...
c*********************************************************************72
c
cc MAIN is the main program for FEM_TO_GMSH.
c
c  Discussion:
c
c    FEM_TO_GMSH converts a 1D, 2D or 3D mesh from FEM to GMSH format.
c
c  Usage:
c
c    fem_to_gmsh prefix
c
c    where 'prefix' is the common filename prefix:
c
c    * 'prefix'_nodes.txt contains the node coordinates,
c    * 'prefix'_elements.txt contains the element node connectivity.
c    * 'prefix'.msh will contain the Gmsh version of the data.
c
c  Licensing:
c
...
c
     ...
     &  '  Convert a 1D, 2D or 3D mesh from FEM to GMSH format.'
      ...
      write ( *, '(a)' ) 
     &  '  Read "prefix"_elements.txt, 3 or 6 node element definitions.'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) 
     ...
     &  element_num )
       ...
       if ( m .eq. 1 ) then
        ...
        if ( element_order .eq. 4 ) then
          ...
          write ( *, '(a)' ) 'FEM_TO_GMSH - Fatal error!'
          write ( *, '(a)' ) 
     &      '  3D mesh data must use 4, 10 or 20 nodes.'
          stop 1
        end if
