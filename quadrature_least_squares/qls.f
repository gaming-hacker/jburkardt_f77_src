...
c*********************************************************************72
c
cc WEIGHTS_LS computes weights for a least squares quadrature rule.
c
c  Discussion:
c
c    We suppose that we wish to estimate integrals of functions f(x)
c    over the interval [a,b], given values of the function at N points X,
c    and that we wish to do this in such a way that our estimate is
c    based on the least squares polynomial approximant p(x) of degree D.
c
c    The resulting quadrature rule will only be exact if N = D + 1
c    and f(x) is a polynomial of degree D or less.
c
c    This function is based in part on ideas of Mac Hyman, of Tulane
c    University.
c
c  Method:
c
c    The approximant p(x) to f(x) has coefficients C(1:D+1).
c    The values of p(x) at the abscissas are found by V * C = P.
c    The values of f(x) at the abscissas are in the vector F.
c    We want to minimize ||P-F|| = ||V * C - F||.
c    We set up the D+1 x D+1 system V' * V * C = V' * F.
c
c    Now, for quadrature, we have Q * C = W * F,
c    so formally Q * inv ( V' * V ) * V' = W.
c
c    Hence, to solve for the weights W, we solve two Vandermonde systems:
c
c    1) Solve   Q' = V' * S  for S, an N vector.
c    2) Solve   S  = V  * R  for R, a D+1 vector.
c    3) Compute W  = R' * V' for W, an N vector.
c
c    so that, by construction (and writing inv() for matrices which
c    actually don't have inverses):
c
c      W  = R' * V'
c      W' = V  * R
c      W' = V  * inv ( V )      * S
...
c      W  = Q  * inv ( V' * V )              * V'
c
c    Thus, knowing the values Q and the abscissas X, we can construct
c    V and solve for W, and the value of W depends (as it should) only
c    on N and X, but not on f(x).
c
c    Having W, we can estimate the integral of any f(x)
c    if we have its values F(1:n) at the points X(1:n), by the formula:
c
c      I(f) \approx Q(f) = sum ( 1 <= i <= n ) W(i) * F(i)
c
c    A simple test is to use N points associated with a known quadrature
c    rule, and specify D = N-1.  Assuming the rule is exact for polynomials
c    of degree D, this procedure should determine the same weights as 
c    those used for the given quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, integer D, the desired degree of the polynomial 
c    approximant.  0 <= D.
c
c    Input, double precision A, B, the left and right endpoints of 
c    the interval.
c
c    Input, integer N, the number of data points or abscissas.
c
c    Input, double precision X(N), the data points.
c
...
c
      ...
      integer i
      integer j
      double precision q(d+1)
      double precision r(d+1)
      double precision s(n)
      double precision v(n,d+1)
      double precision vt(d+1,n)
      double precision w(n)
      double precision x(n)
c
c  Integrate the basis functions x^0, x^1, ..., x^d over [a,b].
c
      bi = b
      ai = a
      do i = 1, d + 1
...
c
c  Form the N by D+1 Vandermonde matrix V.
c
      do i = 1, n
        v(i,1) = 1.0D+00
        do j = 2, d + 1
...
!
      call r8mat_transpose ( n, d + 1, v, vt )
