...
c*********************************************************************72
c
cc SQUARE_GRID: grid points over the interior of a square in 2D.
c
c  Discussion:
c
c    In 2D, a logically rectangular grid is to be created.
c    In the I-th dimension, the grid will use S(I) points.
c    The total number of grid points is
c      N = product ( 1 <= I <= 2 ) S(I)
c
...
c    N = product ( 1 <= I <= 2 ) NS(I).
c
c    Input, integer NS(2), the number of points along
c    each dimension.
c
c    Input, double precision A(2), B(2), the endpoints for each dimension.
c
c    Input, integer C(2), the grid centering for each dimension.
c    1 <= C(*) <= 5.
c
c    Output, double precision X(2,N) = X(2*S(1)*S(2)), the points.
c
      implicit none
