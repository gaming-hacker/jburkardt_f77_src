...
!
      ...
      integer mxitsv
      integer n
      character * ( 8 ) name(prob_max)
      integer nout
      integer nprob
      integer pu
      double precision rs(5,prob_max)
      logical rstart
      double precision v(5000)
...
!
      ...
      write ( *, '(a)' )
     &  '  * means a finite difference jacobian was used.'
      write ( *, '(a)' ) ' '

      if (mod(k,56) .eq. 1) then
        write(pu, 110)
      end if
...
!
      ...
      subroutine nltest ( n, p, nex, title, jac, rstart, xscal1,
     &  xscal2 )
...
!
!    Input, character * ( * ) TITLE, the title of the problem.
!
!    Input, integer JAC.
...
!
      ...
      double precision v(5000)
      character * 20 version
      double precision x(p_max)
      double precision x0scal
      integer xscal1
      integer xscal2
...
!
        ...
        if ( jac .eq. 1 ) then
          call nl2sol ( n, p, x, testr, testj, iv, v, uiparm, urparm,
     &      testr )
...
!    Input, integer P, the number of variables.
!
!    Input, double precision X(P), the point at which the jacobian is to be evaluated.
!
!    Input, integer NFCALL, is the invocation count of TESTR.  It is
!    not needed by this routine.
!
!    Output, double precision JAC(N,P), the jacobian matrix at X.
!
!    Input, integer UIPARM(1), contains the value of NEX, the index
!    of the problem being solved.
!
!    Input, double precision URPARM(*), is a user parameter vector, which is
!    not needed by this routine.
!
...
!
      ...
      double precision e
      double precision expmin
      integer i
      integer j
      double precision jac(n,p)
      integer k
      integer nex
      integer nfcall
      double precision r2
      ...
      double precision tip1
      double precision tpi
      double precision tpim1
      double precision tpip1
      double precision twopi
      parameter ( twopi = 2.0D+00 * 3.141592653589793D+00 )
      double precision u
      external ufparm
      double precision uftolg
      integer uiparm(1)
      double precision ukow(11)
...
!  Singular.
!
      else if ( nex .eq. 3 ) then

        jac(1,1) = 1.0D+00
        jac(1,2) = 10.0D+00
        jac(2,3) = sqrt ( 5.0D+00 )
        jac(2,4) = -jac(2,3)
        jac(3,2) = 2.0D+00 * ( x(2) - 2.0D+00 * x(3) )
...
!
        ...
        jac(5,4) = jac(5,2)
        jac(6,2) = sqrt ( 0.2D+00 )
        jac(7,4) = jac(6,2)
!
!  Zangwill.
!
      else if ( nex .eq. 5 ) then
...
!  Branin.
!
      else if ( nex .eq. 7 ) then

        jac(1,1) = 4.0D+00
        jac(1,2) = 4.0D+00
        jac(2,1) = 3.0D+00 + (x(1) - 2.0D+00 )
     &    * ( 3.0D+00 * x(1) - 2.0D+00 * x(2) - 2.0D+00 ) + x(2) * x(2)
        jac(2,2) = 1.0D+00 + 2.0D+00
     &    * ( 2.0D+00 * x(1) - x(2) * x(2) ) - ( x(1) - x(2) )**2
!
!  Beale.
!
      else if ( nex .eq. 8 ) then
...
!  Box.
!
      else if ( nex .eq. 10 ) then

        if ( expmin .eq. 0.0D+00 ) then
          expmin = 1.999D+00 * log ( d1mach ( 1 ) )
        end if
...
!  Osborne 1.
!
      else if ( nex .eq. 22 ) then

        do i = 1, 33
          ti = 10.0D+00 * dble ( 1 - i )
          jac(i,1) = -1.0D+00
          jac(i,2) = -exp ( x(4) * ti )
          jac(i,3) = -exp ( x(5) * ti )
...
!
      else if ( nex .eq. 23 ) then

        if ( uftolg .eq. 0.0D+00 ) then
          uftolg = 1.999D+00 * log ( d1mach ( 1 ) )
        end if
...
!    on some of these test problems, for 30 .le. NEX.
!
!  Modified:
!
!    28 March 2006
...
!    Input, integer P, the number of variables.
!
!    Input, double precision X(P), the point at which the residual vector
!    is to be evaluated.
!
...
!    which is not needed here.
!
!    Output, double precision R(N), the residual vector at X.
!
      implicit none
      ...
      double precision w
      double precision x(p)
      double precision ybard(15)
      double precision ykow(11)
      double precision ymeyer(16)
      double precision yosb1(33)
      double precision yosb2(65)
      double precision z
...
!
        ...
        if ( x(1) .le. 0.0D+00 .and. x(2) .le. 0.0D+00 ) then
          theta = theta + 1.0D+00
        end if
...
!
        ...
        if ( expmax .eq. 0.0D+00 ) then
          expmax = 1.999D+00 * log ( d1mach ( 2 ) )
        end if
        ...
        if ( min ( x(1), x(2), x(3) ) .le. -expmax ) then
          nfcall = -1
          return
        end if
...
!
        ...
        r(1) = -13.0D+00 + x(1) -  2.0D+00
     &    * x(2) + 5.0D+00 * x(2)**2 - x(2)**3
        r(2) = -29.0D+00 + x(1) - 14.0D+00
     &    * x(2) +           x(2)**2 + x(2)**3
!
!  Watson6.
!  Watson9.
...
!
!    Output, double precision X(P), the initial value for the solution vector.
!
