...
c
   ...
   60     continue
   70     amu=offmax
          if(amu.ne. 0.0E+00 ) go to 80
c         if(amu.eq. 0.0E+00 )
c         then
            amu = 1.00E+00
            go to 90
c         else
   80       amu=amu* ( 1.00E+00 + tol )
c         end if
c       end if
c
c a=a + mu*i
...
c
      if(addmax.le. 0.0E+00 ) go to 220
c     if(addmax.gt. 0.0E+00 )
c     then
c
...
c
        call choldc(nr,n,a, 0.0E+00, tol, addmax )
c     end if
c
...
c         calculate double dogleg curve (ssd)
          ...
          eta= 0.2E+00 + ( 0.8E+00 *alpha*alpha)
     &      /(-beta * r4vec_dot ( n, g, p ) )

          do i=1,n
            v(i)=eta*sx(i)*p(i) - ssd(i)
          end do

          if (dlt .eq. ( -1.00E+00 )) dlt = min(cln, stepmx)
...
c
  120       dot1 = r4vec_dot ( n, v, ssd )
            dot2 = r4vec_dot ( n, v, v )
...
c
      third = 1.0E+00 / 3.0E+00
      do i = 1, n
         stepi = rnoise**third * max(abs(x(i)), 1.0E+00 / sx(i))
         xtempi = x(i)
         x(i) = xtempi + stepi
         call fcn (n, x, fplus)
         x(i) = xtempi - stepi
         call fcn (n, x, fminus)
         x(i) = xtempi
         g(i) = (fplus - fminus) / ( 2.0E+00 * stepi)
      end do
...
c
      do j=1,n
        stepsz=sqrt(rnoise)*max(abs(xpls(j)), 1.0E+00 /sx(j))
        xtmpj=xpls(j)
        xpls(j)=xtmpj+stepsz
...
c
      call d2fcn(nr,n,x,a)
      do 40 j=1,n
        hs=max(abs(g(j)), 1.0E+00 )/max(abs(x(j)),typsiz(j))
...
c
          ...
          phip0 = - ( r4vec_norm_l2 ( n, wrk0 ) )**2 / rnwtln
...
c
   70   continue
        ...
        if(amu.ge.amulo .and. amu.le.amuup) go to 80
          amu=max(sqrt(amulo*amuup),amuup * 1.0E-03 )
          if(iprint.gt. 1)write(lounit,1050) amu
   80   continue
c
c  copy (h,udiag) to l
...
c       factor h=l(l+)
c
        call choldc(nr,n,a, 0.0E+00, sqrt(epsm),addmax)
c
...
c
  140     continue
          ...
          if(phi.lt. 0.0E+00 ) amuup=min(amuup,amu)
...
c
   ...
   20 continue
      ...
      rln = 0.0E+00
      do i=1,n
        rln=max(rln,abs(p(i))/max(abs(x(i)), 1.0E+00 /sx(i)))
      end do
...
c
   40 continue
      ...
      if(fpls.gt. f+slp * 1.0E-04 * almbda) go to 60
c
c solution found
c
        iretcd=0
        if(almbda.eq. 1.0E+00 .and. sln.gt. 0.99E+00 * stepmx) then
          mxtake=.true.
        end if
        go to 40
c
...
c
   70     if(almbda.ne. 1.0E+00 ) go to 80
c
c first backtrack: quadratic fit
c
            tlmbda=-slp/( 2.0E+00 *(fpls-f-slp))
            go to 110
c
...
c
   90         tlmbda=(-b-sign ( 1.0E+00, a )*sqrt(disc)) 
     &          / ( 3.0E+00 * a )
  100       if(tlmbda.gt. 0.5E+00 * almbda) tlmbda= 0.5E+00 *almbda
  110     plmbda=almbda
          pfpls=fpls

          if(tlmbda.ge. almbda * 0.1E+00 ) go to 120
            almbda=almbda * 0.1E+00
            go to 130
  120       almbda=tlmbda
  130 go to 40
...
c
      do i=1,n
        if(typsiz(i).eq. 0.0E+00 ) typsiz(i) = 1.0E+00
        if(typsiz(i).lt. 0.0E+00 ) typsiz(i)=-typsiz(i)
        sx(i)=1.0E+00 / typsiz(i)
      end do
c
c  check maximum step size
c
      if (stepmx .gt. 0.0E+00 ) go to 30
      stpsiz = 0.0E+00
      do i = 1, n
        stpsiz = stpsiz + x(i)*x(i)*sx(i)*sx(i)
      end do
      stpsiz = sqrt(stpsiz)
      stepmx = max ( 1.0E+03 * stpsiz, 1.0E+03 )
   30 continue
c
c  check function scale
c
      if(fscale.eq. 0.0E+00 ) fscale = 1.0E+00
      if(fscale.lt. 0.0E+00 ) fscale = -fscale
c
c  check gradient tolerance
c
      if(gradtl.lt. 0.0E+00 ) go to 50
c
...
c
     ...
     &     dlt,method,iexp,iagflg,iahflg,stepmx,msg,iprint,lounit)
      if(msg.lt.0) return
      rnf=max( 10.0E+00 **(-ndigit),epsm)
      analtl=max( 1.0E-02, sqrt(rnf))
...
c
c a(n,n)       --> workspace
c                  a real matrix used to store the hessian and its
...
c
      ...
      rgx = 0.0E+00
      do i=1,n
        relgrd=abs(gpls(i))*max(abs(xpls(i)), 1.0E+00 /sx(i))/d
        rgx=max(rgx,relgrd)
      end do
...
c
      rsx=0.0E+00
      do i=1,n
        relstp=abs(xpls(i)-x(i))/max(abs(xpls(i)), 1.0E+00 /sx(i))
        rsx=max(rsx,relstp)
      end do
      jtrmcd=2
...
c
   ...
   10 continue
      ...
      if(u(k).ne. 0.0E+00 .or. k.eq. 1) go to 20
c     if(u(k).eq. 0.0E+00 .and. k.gt. 1)
c     then
        k=k-1
        go to 10
c     end if
...
10    continue
      ...
      function r4vec_dot ( n, v1, v2 )

c*********************************************************************72
c
cc R4VEC_DOT finds the dot product of a pair of R4VEC's.
c
c  Discussion:
c
...
c
c    Output, real R4VEC_DOT, the dot product.
c
      implicit none
...
c
      ...
      den1 = r4vec_dot ( n, x, y )

      snorm2 = r4vec_norm_l2 ( n, s )
      ynrm2 = r4vec_norm_l2 ( n, y )

      if ( den1 .lt. sqrt(epsm)*snorm2*ynrm2) go to 160
        call mvmltu(nr,n,a,s,u)
        den2 = r4vec_dot ( n, u, u )
c
...
c  undefined).
c
      if(iretcd.eq. 4) fplsp = 0.0E+00
      if(iprint.gt. 1)write(lounit,1100) iretcd,fpls,fplsp,dltf,slp
      if(iretcd.ne. 3 .or. (fpls.lt.fplsp.and.dltf.le. 1.0E-04*slp))
     &                                                     go to 30
c
...
c  fpls too large
c
   30   if(dltf.le. 1.0E-04 *slp) go to 80
          if(iprint.gt. 1)write(lounit,1010)
          rln = 0.0E+00
          do i=1,n
            rln=max(rln,abs(sc(i))/max(abs(xpls(i)), 1.0E+00 /sx(i)))
          end do
...
c
   50       iretcd=2
            dltmp=-slp*dlt/( 2.0E+00 *(dltf-slp))
            if(iprint.gt. 1)write(lounit,1120) dltmp
            if(dltmp.ge. 0.1E+00 * dlt) go to 60
              dlt= 0.1E+00 * dlt
              go to 70
   60         dlt=dltmp
   70       continue
...
c
  110     do 130 i = 1, n
             ...
             dltfp = dltfp + 2.0E+00 * temp
  130     continue
c
c         end if
c
  140     dltfp = slp + dltfp / 2.0E+00
          if(iprint.gt. 1)write(lounit,1130) dltfp,nwtake
          if(iretcd.eq. 2 .or. (abs(dltfp-dltf).gt. 0.1E+00 *abs(dltf))
     &         .or. nwtake .or. (dlt.gt. 0.99E+00 *stepmx)) go to 160
c         if(iretcd.ne. 2 .and. (abs(dltfp-dltf) .le. 0.1E+00 *abs(dltf))
c    &         .and. (.not.nwtake) .and. (dlt.le. 0.99E+00 *stepmx))
c         then
c
...
c
  160       continue
            ...
            iretcd=0
            if(dlt.gt. 0.99E+00 *stepmx) mxtake=.true.
            if(dltf.lt. 0.1E+00 *dltfp) go to 170
c           if(dltf.ge. 0.1E+00 *dltfp)
c           then
c
