...
c*********************************************************************72
c
cc MAIN is the main program for TRIANGULATION_SVG.
c
c  Discussion:
c
c    TRIANGULATION_SVG plots a triangulated set of nodes.
c
c  Usage:
c
c    triangulation_svg prefix
c
c    where
c
c    * 'prefix'_nodes.txt contains the node coordinates;
c    * 'prefix'_elements.txt contains the element definitions 
c    * 'prefix'.svg, the SVG plot of the triangulation.
c
c  Licensing:
c
...
c
     ...
     &  node_num, element_filename, node_filename, plot_filename )
c
c  Terminate.
c
      write ( *, '(a)' ) ''
...
c
      call triangulation_plot ( plot_filename, node_num, node_xy, 
...
c
     ...
     &  d, month(m), y, h, ':', n, ':', s, '.', mm, ampm
      ...
      subroutine triangulation_plot ( filename, node_num, node_xy, 
     &  element_order, element_num, element_node )

c*********************************************************************72
c
cc TRIANGULATION_PLOT plots a triangulation in SVG format.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, character * ( * ) FILENAME, the name of the output file.
c
c    Input, integer NODE_NUM, the number of points.
c
c    Input, double precision NODE_XY(2,NODE_NUM), the nodes.
c
c    Input, integer ELEMENT_ORDER, the order of the element.
c
c    Input, integer ELEMENT_NUM, the number of elements.
c
c    Input, integer ELEMENT_NODE(ELEMENT_ORDER,ELEMENT_NUM), lists, for each element,
c    the indices of the points that form the vertices of the element.
c
      implicit none
...
c
     ...
     &  j4_max, '"'
      ...
      write ( output, '(a)' ) '  <desc>'
      write ( output, '(a)' ) 
     &  '    Triangulation created by triangulation_svg.f'
      write ( output, '(a)' ) '  </desc>'
      ...
      do j = 1, element_num
        ...
        if ( element_order .eq. 3 ) then
          do i = 1, 3
            ...
            call r8_to_i4 ( x_min, x_max, node_xy(1,node), i4_min, 
     &        i4_max, i4 )
            call r8_to_i4 ( y_max, y_min, node_xy(2,node), j4_min, 
     &        j4_max, j4 )
            write ( string, '(i3,a,i3)' ) i4, ',', j4
            call s_blank_delete ( string )
            write ( output, '(6x,a)' ) trim ( string )
          end do
        else if ( element_order .eq. 4 ) then
          do i = 1, 3
            node = element_node(i,j)
            call r8_to_i4 ( x_min, x_max, node_xy(1,node), i4_min, 
     &        i4_max, i4 )
