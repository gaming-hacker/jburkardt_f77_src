...
c
      ...
      subroutine dcft2d_test ( )

c*********************************************************************72
c
cc dcft2d_test tests dcft2d().
c
c  Discussion:
c
c    Plot image and transform of 8 by 8 unit source in 64 by 64 
c    (otherwise zero) array.
c
c  Modified:
c
c    06 July 2021
c
      ...
      logical forwd
      integer i
      complex*16 image(n,n)
      complex*16 image2(n,n)
      integer j
      integer lda
      complex*16 w(3*n+8)
...
C Arithmetic statement function for Runge's function. 
      ...
      write ( *, '(a)' ) '  DCFFTF computes it.'
       ...
       X0 = -1.0D0 
       PI = ASIN(1.0D0)*2.0D0 
       SQTM1 = CMPLX(0.0D0,-1.0D0, kind = 8 )
...
C First sample point at -1, last at 1-DEL
              XJ = (-1.0D0) + J*DEL 
              COEFF(J) = CMPLX(RUNGE(XJ),0.0D0, kind = 8 ) 
          end do
...
C
      ...
      if (data .lt. xmin) then
         ...
         test06_inbin = 2 + int ( (data-xmin)/width )
      endif

      return
      end
      subroutine dpchez_test ( )

c*********************************************************************72
c
cc DPCHEZ_TEST tests DPCHEZ.
c
c  Modified:
c
c    06 February 2012
...
C
      ...
      parameter ( mchalf = 8 )
       ...
       DOUBLE PRECISION  A(mchalf),B(mchalf),R(MCOEF),
     *WSAVE(3*MCOEF+15),TN,ER,PI,DEL,XJ,DFTA(mchalf),
     *DFTB(mchalf),F,C(mchalf),S(mchalf),X,RUNGE,X0,AZERO 
C
C Arithmetic statement function for Runge's function. 
       RUNGE(X) = 1.0D0/(1.0D0+25.0D0*X*X) 
...
c*********************************************************************72
c
cc F might or might not define an ODE for DDRIV1.
c
c  Discusssion:
c
c    Because of the BAD form of DDRIV1, we need to include a subroutine
c    called F in any program that calls NMS.  
c
c    If we are calling DDRIV1, then F is the right hand side of an ODE.
c
c    Otherwise, F can be any thing, as long as the name F appears.
c
