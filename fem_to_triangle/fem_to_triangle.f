...
c*********************************************************************72
c
cc MAIN is the main program for FEM_TO_TRIANGLE.
c
c  Discussion:
c
c    FEM_TO_TRIANGLE converts a 2D mesh from FEM to TRIANGLE format.
c
c  Usage:
c
c    fem_to_triangle prefix
c
c    where 'prefix' is the common filename prefix:
c
c    * 'prefix'_nodes.txt contains the FEM node coordinates,
c    * 'prefix'_elements.txt contains the FEM element connectivities.
c    * 'prefix'.node will contains the TRIANGLE node coordinates,
c    * 'prefix'.ele will contains the TRIANGLE element connectivities.
c
c  Licensing:
c
...
c
     ...
     &  trim ( fem_node_filename ) //'".'
      ...
      call i4mat_header_read ( fem_element_filename, element_order, 
     &  element_num )
     ...
     &  trim ( fem_element_filename ) // '".'
      ...
      call main_sub ( element_num, element_order, 
     &  fem_element_filename, fem_node_filename, m, node_num,  
...
c
      ...
      subroutine main_sub ( element_num, element_order, 
     &  fem_element_filename, fem_node_filename, m, node_num, 
     &  triangle_element_filename, triangle_node_filename )

c*********************************************************************72
c
cc MAIN_SUB completes the MAIN program after allocating some memory.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      integer element
      double precision element_att(0,0)
      ...
      integer i
      double precision node_att(0,0)
      integer node_att_num
      integer node_marker(0,0)
     ...
     &  trim ( fem_node_filename ) //'".'
      ...
      call r8mat_transpose_print_some ( m, node_num, node_x, 1, 1, m,
     &  5, '  Initial portion of node coordinate data:' )

      call i4mat_data_read ( fem_element_filename, element_order, 
...
c
c  Force 1-based indexing.
c
      call mesh_base_one ( node_num, element_order, element_num, 
     &  element_node )
c
c  Write the TRIANGLE node and element files.
c
      node_att_num = 0
      node_marker_num = 0
     ...
     &  node_att_num, node_marker_num, node_x, node_att, node_marker )
      ...
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) '  Created "' // 
     &  trim ( triangle_node_filename ) // '"'

      element_att_num = 0

      call triangle_element_write ( triangle_element_filename, 
     &  element_num, element_order, element_att_num, element_node, 
     &  element_att )

      write ( *, '(a)' ) '  Created "' // 
     &  trim ( triangle_element_filename ) // '"'
