...
c
c    integer STATUS, used to communicate between the user and the routine.  
c    The user only sets STATUS to zero on the first call, to indicate 
c    that this is a startup call.
c
c    double precision VALUE, the function value at ARG, as requested
c    by the routine on the previous call.
c
c  Output:
c
c    double precision ARG, the currently considered point.  For the next call,
...
c
c    integer STATUS, used to communicate between the user and the routine.
c    The routine returns STATUS positive to request that the function be 
c    evaluated at ARG, or returns STATUS as 0, to indicate that the 
c    iteration is complete and that ARG is the estimated zero.
c
      implicit none
