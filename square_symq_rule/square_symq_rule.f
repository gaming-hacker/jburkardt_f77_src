...
c  Parameters:
c
c    Input, integer DEGREE, the degree of the quadrature (the maximum degree of
c    the polynomials of two variables that are integrated
c    exactly.  1 <= DEGREE <= 20.
c
c    Output, integer N, the number of nodes in the full rule.
c
      implicit none
     ...
     &   24,  33,  33,  44,  44,  55,  55,  68,  68,  81 /
      ...
      if ( 1 .le. degree .and. degree .le. 20 ) then
        n = n_save(degree)
      else
        write ( *, '(a)' ) ''
...
c
     ...
     &     0.28284271247461904D+01/
      ...
      call r8mat_row_copy ( 2, ns, 1, xs, x )
      call r8mat_row_copy ( 2, ns, 2, ys, x )
      call r8vec_copy ( ns, ws, w )
...
c*********************************************************************72
c
cc SQUARE_SYMQ returns a symmetric quadrature rule for the square.
c
c  Discussion:
c
...
c
c    The nodes are symmetrically arranged.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the number of nodes.
c    This can be determined by a call to RULE_FULL_SIZE(DEGREE).
c
c    Output, double precision X(2,N), the coordinates of the nodes.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc SQUARE_SYMQ_GNUPLOT: GNUPLOT plot of the symmetric square quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
