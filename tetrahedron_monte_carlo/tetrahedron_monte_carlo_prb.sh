#!/bin/bash
#
gfortran -c -g tetrahedron_monte_carlo_prb.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling tetrahedron_monte_carlo_prb.f"
  exit
fi
rm compiler.txt
#
gfortran tetrahedron_monte_carlo_prb.o -L$HOME/libf77 -ltetrahedron_monte_carlo
if [ $? -ne 0 ]; then
