...
c*****************************************************************************80
c
cc MAIN is the main program for HERMITE_RULE.
c
c  Discussion:
c
c    This program computes a Gauss-Hermite quadrature rule 
c    and writes it to a file.
c
c    The integral to be approximated has the form
c
c      C * Integral ( -oo < x < +oo ) f(x) rho(x) dx
c
...
c    * A, the center point, often 0.0;
c    * B, a scale factor, often 1.0 or 0.5;
c    * SCALE, is 1 if the weights are to be normalized;
c    * FILENAME, the root name of the output files.
c
c    If SCALE = 0, then the factor C in front of the integrand is 1.
c    If SCALE is nonzero, then the factor C is sqrt ( B ) / sqrt ( PI ).
c    which means that the function f(x)=1 will integrate to 1.
c
c  Licensing:
c
...
c
c  Set some variables internally.
c
      alpha = 0.0D+00
      beta = 0.0D+00
c
...
c
      if ( 4 .le. arg_num ) then
        iarg = 4
        call getarg ( iarg, string )
        call s_to_i4 ( string, scale )
      else
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 
     &    '  Enter SCALE, the normalization option (0/1):'
        read ( *, * ) scale
      end if
c
c  Get FILENAME.
...
c
c  Because FORTRAN77 can't allocate memory easily, we have to
c  call a subroutine.
c
      call main_sub ( a, alpha, b, beta, filename, n, scale )
c
c  Terminate.
c
      write ( *, '(a)' ) ' '
      ...
      subroutine main_sub ( a, alpha, b, beta, filename, n, scale )

c*********************************************************************72
c
cc MAIN_SUB is part of the MAIN program that needs allocated memory.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, character * ( * ) FILENAME, the root name of the quadrature files.
c
c    Input, integer N, the number of points in the rule.
c
c    Input, integer SCALE, the normalization option.
c
      implicit none
...
c
c  Normalize the rule.
c  This way, the weights add up to 1.
c
      if ( scale .eq. 1 ) then
        do i = 1, n
...
c
c  Set the R values to suggest an infinite interval.
c
      r(1) = - r8_huge ( )
      r(2) =   r8_huge ( ) 
c
