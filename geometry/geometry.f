...
c*********************************************************************72
c
cc ELLIPSE_AREA3 returns the area of an ellipse in 2D.
c
c  Discussion:
c
...
c
c    Output, double precision ELLIPSE_AREA3, the area of the ellipse.
c
      implicit none

      double precision ellipse_area3
      double precision r1
      double precision r2
      double precision r8_pi
      parameter ( r8_pi = 3.141592653589793D+00 )
...
c
      ...
      t = r8vec_diff_dot_product ( dim_num, p1, p, p1, p2 ) / bot
...
c
      ...
      t = r8vec_diff_dot_product ( dim_num, p, p1, p2, p1 ) / bot
c
...
c*********************************************************************72
c
cc LINE_PAR_POINT_NEAR_2D: nearest point on parametric line to given point, 2D.
c
c  Discussion:
c
c    The parametric form of a line in 2D is:
...
c      Y = Y0 + G * T
c
c    We may normalize by choosing F*F + G*G = 1, and F nonnegative.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c    to be measured.
c
c    Output, double precision PN(2), the point on the parametric line which
c    is nearest to P.
c
      implicit none
...
c*********************************************************************72
c
cc LINE_PAR_POINT_NEAR_3D: nearest point on parametric line to given point, 3D.
c
c  Discussion:
c
c    The parametric form of a line in 3D is:
...
c      Z = Z0 + H * T
c
c    We may normalize by choosing F*F + G*G + H*H = 1, and F nonnegative.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c  Parameters:
c
c    Input, double precision F, G, H, X0, Y0, Z0, the parametric 
c    line parameters.
c
c    Input, double precision P(3), the point whose distance from the line is
c    to be measured.
c
c    Output, double precision PN(3), the point on the parametric line which
c    is nearest to P.
c
      implicit none
...
c*********************************************************************72
c
cc LINE_PAR2EXP_3D converts a parametric line to explicit form in 3D.
c
c  Discussion:
c
c    The parametric form of a line in 3D is:
...
c  Parameters:
c
c    Input, double precision F, G, H, X0, Y0, Z0, the parametric line parameters.
c
c    Output, double precision P1(3), P2(3), two points on the line.
c
      implicit none
...
c*********************************************************************72
c
cc POLYGON_CONTAINS_POINT_2D finds if a point is inside a polygon.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c
c    Output, logical INSIDE, is TRUE if the point is inside 
c    the polygon.
c
      implicit none
      ...
      do i = 1, n
        ...
        if ( min ( py1, py2 ) .lt. p(2) ) then
          if ( p(2) .le. max ( py1, py2 ) ) then
            if ( p(1) .le. max ( px1, px2 ) ) then
              if ( py1 .ne. py2 ) then
                xints = ( p(2) - py1 ) * ( px2 - px1 ) 
     &            / ( py2 - py1 ) + px1
              end if
              if ( px1 .eq. px2 .or. p(1) .le. xints ) then
                inside = .not. inside
              end if
            end if
          end if
...
c*********************************************************************72
c
cc POLYGON_CONTAINS_POINT_2D_2: is a point inside a convex polygon in 2D.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c*********************************************************************72
c
cc POLYGON_CONTAINS_POINT_2D_3 finds if a point is inside a simple polygon in 2D.
c
c  Discussion:
c
...
c*********************************************************************72
c
cc SPHERE01_TRIANGLE_VERTICES_TO_MIDPOINTS: midsides of a spherical triangle.
c
c  Discussion:
c
...
c
      ...
      do j1 = 1, 3
        do j2 = j1+1, 4
          k = k + 1
          edge_length(k) = r8vec_diff_norm ( dim_num, 
...
c*********************************************************************72
c
cc TRIANGLE_CONTAINS_POINT_2D_1 finds if a point is inside a triangle in 2D.
c
c  Discussion:
c
