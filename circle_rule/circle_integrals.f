...
c
      ...
      subroutine monomial_value ( m, n, x, expon, value )
...
c
c    Input, integer EXPON(M), the exponents.
c
...
c
      ...
      do i = 1, m
        if ( 0 .ne. expon(i) ) then
          do j = 1, n
            value(j) = value(j) * x(i,j) ** expon(i)
          end do
        end if
      end do
