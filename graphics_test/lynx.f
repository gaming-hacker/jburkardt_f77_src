...
c*********************************************************************72
c
cc LYNX uses DISLIN to draw an XY plot of the Lynx harvest per year.
c
c  Discussion:
c
c    This example reads an XY data file containing an unknown number of
c    records, draws the data points and connectes them.
c
c  Licensing:
c
...
c
      ...
      write ( *, '(a)' ) 
     &  '  Use DISLIN routines to plot (X,Y) data as a curve.'
c
c  Read the data.
c
      call r4mat_data_read ( 'lynx.txt', m, n, xy )
c
...
c
      call setfil ( 'lynx.png' )
c
c  Choose the page size and orientation.
c  'USA' is 2790 plot units wide and 2160 plot units high.
c  'L' requests LANDSCAPE orientation.
c
      call setpag ( 'usal' )
c
...
c
      call simplx ( )
c
c  Set the axis origin 500 plot units to the right, and 1900 plot units DOWN.
c
...
c
c  Use "-1" digits after the decimal points on X and Y tick marks.
c
      call labdig ( -1, 'x' )
      call labdig ( -1, 'y' )
c
c  Use tick marks to mark 5 intervals between major tick marks.
c
      call ticks ( 5, 'xy' )
c
c  Relate the physical coordinates to the axes.
c
      call graf ( 1821.0, 1934.0, 1830.0, 10.0, 0.0, 7000.0, 0.0, 
     &  1000.0 )
c
