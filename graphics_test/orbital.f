...
c*********************************************************************72
c
cc ORBITAL uses DISLIN to display a contour plot of Z(X,Y) data.
c
c  Licensing:
c
...
c
      ...
      write ( *, '(a)' ) 
     &  '  Use DISLIN to make a contour plot of Z(X,Y) data.'
c
c  Read the data.
c
      call r4mat_data_read ( 'orbital.txt', m, nn, xyz )
c
c  Split the data.
c  The contouring routine expects that data is along fixed X and Y coordinates,
c  and so the X and Y data is to be given as vectors, not arrays.
c
      k = 1
      do i = 1, n
        x(i) = xyz(1,k)
      ...
      call r4vec_max ( n, y, ymax )
      call r4vec_min ( n, y, ymin )
c
c  Z is a table.
c  The first dimension should contain values for constant X.
c
      k = 0
      do j = 1, n
        do i = 1, n
      ...
      call r4mat_max ( n, n, z, zmax )
      call r4mat_min ( n, n, z, zmin )
c
...
c
      call pagera ( )
c
c  Use the SIMPLX font.
c
      call simplx ( )
c
c  Set the axis origin in plot units to the right, and plot units DOWN.
c
...
c
      ...
      do level = 1, level_num
        ...
        level_value = ( real ( level_num + 1 - level ) * zmin   
     &                + real (                 level ) * zmax ) 
     &                / real ( level_num + 1         )
...
c
      call endgrf ( )
c
c  RETURN FROM LEVEL 2 TO LEVEL 1.
c
c  Close DISLIN.
c
      call disfin ( )
...
c
     ...
     &    '  the file "', trim ( input_filename ), '".'
      ...
      subroutine r4mat_max ( m, n, a, amax )
...
c
c    Output, real AMAX, the maximum entry of A.
c
      implicit none
      ...
      subroutine r4mat_min ( m, n, a, amin )
...
c    Input, real A(M,N), the matrix.
c
c    Output, real AMIN, the minimum entry of A.
c
      implicit none
