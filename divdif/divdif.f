...
c
      ...
      subroutine dif_basis_deriv ( nd, xd, xdp, ddp )

c*********************************************************************72
c
cc DIF_BASIS_DERIV: Lagrange basis derivative difference tables.
c
c  Discussion:
c
c    Given ND points XD, a Lagrange basis polynomial L(J)(X) is associated
c    with each point XD(J).
c
c    This function computes a table DDP(*,*) whose J-th column contains
c    the difference table for the first derivative of L(J)(X).
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, double precision XD(ND), the X values upon which the 
c    Lagrange basis polynomials are to be based.
c
c    Output, double precision XDP(ND-1), the X values upon with
c    the derivative difference table is based.  In fact, these are
c    all 0.
c
c    Output, double precision DDP(ND-1,ND), the divided difference 
c    tables for all the Lagrange basis polynomials.  Column J of DDP
c    contains the table for basis polynomial associated with XD(J).
c
      implicit none
...
c
c  Process the vectors one column at a time.
c
      do j = 1, nd
c
c  Set the data.
c
        ...
        yd(j) = 1.0D+00
c
c  Compute the divided difference table.
c
        call data_to_dif ( nd, xd, yd, dd )
c
c  Compute the divided difference table for the derivative.
c
        call dif_deriv_table ( nd, xd, dd, xdp, ddp(1:nd-1,j) )

      end do

      return
      end
      subroutine dif_basis_derivk ( nd, xd, k, xdp, ddp )

c*********************************************************************72
c
cc DIF_BASIS_DERIV: Lagrange basis K-th derivative difference tables.
c
c  Discussion:
c
c    Given ND points XD, a Lagrange basis polynomial L(J)(X) is associated
...
c    Lagrange basis polynomials are to be based.
c
c    Input, integer K, the index of the derivative.
c
c    Output, double precision XDP(ND-K), the X values upon with
c    the derivative difference table is based.  In fact, these are
c    all 0.
c
c    Output, double precision DDP(ND-K,ND), the divided difference 
c    tables for all the Lagrange basis polynomials.  Column J of DDP
c    contains the table for basis polynomial associated with XD(J).
c
      implicit none
      ...
      double precision dd(nd)
      double precision ddp(nd-k,nd)
      integer i
      integer j
      double precision xd(nd)
      double precision xdp(nd-k)
      double precision yd(nd)
c
c  Process the vectors one column at a time.
c
...
c  Compute the divided difference table for the derivative.
c
        call dif_derivk_table ( nd, xd, dd, k, xdp, ddp(1:nd-k,j) )
...
c
      ...
      subroutine dif_deriv_table ( nd, xd, yd, xdp, ydp )
...
c
c    Output, double precision XDP(ND-1), the abscissas for the divided
c    difference table for the derivative.
c
c    Output, double precision YDP(ND-1), the divided difference
...
c
      ...
      subroutine dif_derivk_table ( nd, xd, dd, k, xdk, ddk )

c*********************************************************************72
c
cc DIF_DERIVK_TABLE computes the divided difference table for K-th derivative.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    difference table.
c
c    Input, double precision DD(ND), the divided difference table.
c
c    Input, integer K, the index of the derivative.
c    0 <= K
c
c    Input, double precision XDK(ND-K), the abscissas for the divided
c    difference table for the derivative.
c
c    Output, double precision DDK(ND-K), the divided difference 
c    table for the derivative.
c
      implicit none
      ...
      double precision dd(nd)
      double precision dd_temp(nd)
      double precision ddk(nd-k)
      integer i
      integer j
      integer ndk
      double precision xd(nd)
      double precision xd_temp(nd)
      double precision xdk(nd-k)

      if ( k .lt. 0 ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'DIF_DERIVK_TABLE - Fatal error!'
        write ( *, '(a)' ) '  K < 0.'
        stop
      end if
...
c
      ...
      call dif_shift_zero ( nd, xd_temp, dd_temp )
c
c  Repeatedly differentiate.
c 
      do j = 1, k
