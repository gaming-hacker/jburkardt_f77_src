...
c*********************************************************************72
c
cc BASHFORTH_SET sets abscissas and weights for Adams-Bashforth quadrature.
c
c  Discussion:
c
c    Adams-Bashforth quadrature formulas are normally used in solving
c    ordinary differential equations, and are not really suitable for
c    general quadrature computations.  However, an Adams-Bashforth formula
c    is equivalent to approximating the integral of F(Y(X)) between X(M)
c    and X(M+1), using an explicit formula that relies only on known values
c    of F(Y(X)) at X(M-N+1) through X(M).  For this reason, the formulas
c    have been included here.
c
c    Suppose the unknown function is denoted by Y(X), with derivative
c    F(Y(X)), and that approximate values of the function are known at a
c    series of X values, which we write as X(1), X(2), ..., X(M).  We write
c    the value Y(X(1)) as Y(1) and so on.
c
c    Then the solution of the ODE Y'=F(X,Y) at the next point X(M+1) is
c    computed by:
c
c      Y(M+1) = Y(M) + Integral ( X(M) < X < X(M+1) ) F(Y(X)) dx
c             = Y(M) + H * Sum ( 1 <= I <= N ) W(I) * F(Y(M+1-I)) approximately.
c
c    In the documentation that follows, we replace F(Y(X)) by F(X).
c
c    The integral:
c
c      integral ( 0 <= x <= 1 ) f(x) dx.
c
c    The quadrature formula:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    The Adams-Bashforth formulas require equally spaced data.
c
c    Here is how the formula is applied in the case with non-unit spacing:
c
c      Integral ( A <= X <= A+H ) F(X) dx =
c      H * Sum ( 1 <= I <= N ) W(I) * F ( A - (I-1)*H ),
c      approximately.
c
c    The reference lists the second coefficient of the order 8 Adams-Bashforth
c    formula as
c      w(2) =  -1162169.0D+00 / 120960.0D+00
c    but this should be
c      w(2) =  -1152169.0D+00 / 120960.0D+00
c
c  Licensing:
c
...
c    N should be between 1 and 10, 12, 14, 16, 18 or 20.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
c    W(1) is the weight at X(1) = 0,
c    W(2) the weight at X(2) = -1,
c    and so on.  The weights are rational, and should sum to 1.  Some
c    weights may be negative.
c
      implicit none
      ...
      if ( n .eq. 1 ) then
        ...
        write ( *, '(a)' ) 
     &    '  Legal values are 1 through 10, 12, 14, 16, 18 or 20.'
        stop 1
...
c*********************************************************************72
c
cc BDF_SET sets weights for ODE backward differentiation.
c
c  Discussion:
c
c    GAMMA * Y(N+1) = Sum ( 1 <= I <= N ) ALPHA(I) * Y(N+1-I)
c                     + dx * BETA * Y'(X(N+1),Y(N+1))
c
c    This is equivalent to the backward differentiation corrector formulas.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N should be between 1 and 6.
c
c    Output, double precision ALPHA(N), BETA, GAMMA, the weights.
c
      implicit none
...
c*********************************************************************72
c
cc BDF_SUM: an explicit backward difference quadrature rule for [0,1].
c
c  Discussion:
c
c    The integral:
...
c
c      sum ( 1 <= i <= n ) w(i) * bdf^(i-1) f ( 0 )
c
c    The integral from 0 to 1 is approximated using data at X = 0,
c    -1, -2, ..., -N+1.  This is a form of extrapolation, and
c    the approximation can become poor as N increases.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, external FUNC, the name of the function which evaluates
c    the integrand.  The function must have the form
c      function func ( x ).
c
c    Input, integer N, the order.
c
...
c
      ...
      do i = 2, n
        do j = n, i, -1
          diftab(j) = diftab(j-1) - diftab(j)
        end do
      end do

      result = r8vec_dot_product ( n, w, diftab )
...
c*********************************************************************72
c
cc BDFC_SET sets weights for backward differentiation corrector quadrature.
c
c  Discussion:
c
c    A backward differentiation corrector formula is defined for a set
c    of evenly spaced abscissas X(I) with X(1) = 1 and X(2) = 0.  Assuming
c    that the values of the function to be integrated are known at the
c    abscissas, the formula is written in terms of the function value at
c    X(1), and the backward differences at X(1) that approximate the
c    derivatives there.
c
c    The integral:
c
...
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * bd^(i-1) f ( 1 ).
c
c    Here, "BD^(I-1) F ( 1 )" denotes the (I-1)st backward difference
c    of F at X = 1, using a spacing of 1.  In particular,
c
c    BD^0 F(1) = F(1)
c    BD^1 F(1) = F(1) - F(0)
c    BD^2 F(1) = F(1) - 2 * F(0) + F(-1 )
c
c    The relationship between a backward difference corrector and the
c    corresponding Adams-Moulton formula may be illustrated for the
c    BDF corrector of order 4:
c
c      BD^0 F(1) - 1/2 * BD^1 F(1) - 1/12 * BD^2 F(1) - 1/24 * BDF^3 F(1)
c      =            F(1)
c        -  1/2 * ( F(1) -         F(0) )
c        - 1/12 * ( F(1) - 2     * F(0) +        F(-1) )
c        - 1/24 * ( F(1) - 3     * F(0) + 3    * F(-1) -        F(-2) )
c      =   9/24 *   F(1) + 19/24 * F(0) - 5/24 * F(-1) + 1/24 * F(-2)
c
c    which is the Adams-Moulton formula of order 4.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Simeon Fatunla,
c    Numerical Methods for Initial Value Problems in Ordinary
...
c
c    Input, integer N, the order.
c    N should be between 1 and 19.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc BDFP_SET sets weights for backward differentiation predictor quadrature.
c
c  Discussion:
c
c    A backward differentiation predictor formula is defined for a set
c    of evenly spaced abscissas X(I) with X(1) = 1 and X(2) = 0.  Assuming
c    that the values of the function to be integrated are known at the
c    abscissas, the formula is written in terms of the function value at
c    X(2), and the backward differences at X(2) that approximate the
c    derivatives there.  A backward differentiation predictor formula
c    is equivalent to an Adams-Bashforth formula of the same order.
c
c    The integral:
c
c      integral ( 0 <= x <= 1 ) f(x) dx
...
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * bd^(i-1) f ( 0 ),
c
c    Here, "BD^(I-1) F ( 0 )" denotes the (I-1)st backward difference
c    of F at X = 0, using a spacing of 1.  In particular,
c
c    BD^0 F(0) = F(0)
c    BD^1 F(0) = F(0) - F(-1)
c    BD^2 F(0) = F(0) - 2 * F(-1) + F(-2 )
c
c    The relationship between a backward difference predictor and the
c    corresponding Adams-Bashforth formula may be illustrated for the
c    BDF predictor of order 3:
c
c      BD^0 F(0) + 0.5 * BD^1 F(0) + 5/12 * BD^2 F(0)
c      =            F(0)
c        + 1/2  * ( F(0) -         F(1) )
c        + 5/12 * ( F(0) - 2     * F(-1) +      F(-2) )
c      =  23/12 *   F(0) - 16/12 * F(-1) + 5/12 F(-2)
c
c    which is the Adams-Bashforth formula of order 3.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc CHEBYSHEV_SET sets abscissas and weights for Chebyshev quadrature.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    The Chebyshev rule is distinguished by using equal weights.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N may only have the values 1, 2, 3, 4, 5, 6, 7 or 9.
c    There are NO other Chebyshev rules with real abscissas.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
c
      ...
      if ( n .eq. 1 ) then
        ...
        x(1) =   - sqrt ( ( 1.0D+00 + 2.0D+00 / sqrt ( 5.0D+00 ) ) 
     &    / 3.0D+00 )
        x(2) =   - sqrt ( ( 1.0D+00 - 2.0D+00 / sqrt ( 5.0D+00 ) ) 
     &    / 3.0D+00 )
        x(3) =     sqrt ( ( 1.0D+00 - 2.0D+00 / sqrt ( 5.0D+00 ) ) 
     &    / 3.0D+00 )
        x(4) =     sqrt ( ( 1.0D+00 + 2.0D+00 / sqrt ( 5.0D+00 ) ) 
     &    / 3.0D+00 )
...
c*********************************************************************72
c
cc CHEBYSHEV1_COMPUTE computes a Gauss-Chebyshev type 1 quadrature rule.
c
c  Discussion:
c
c    The integral:
c
c      integral ( -1 <= x <= 1 ) f(x) / sqrt ( 1 - x * x ) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c*********************************************************************72
c
cc CHEBYSHEV1_SET sets a Chebyshev Type 1 quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c*********************************************************************72
c
cc CHEBYSHEV2_COMPUTE computes a Gauss-Chebyshev type 2 quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c
      ...
      do i = 1, n
        angle = r8_pi * dble ( n + 1 - i ) / dble ( n + 1 )
        w(i) = r8_pi / dble ( n + 1 ) * ( sin ( angle ) )**2
        x(i) = cos ( angle )
      end do
...
c*********************************************************************72
c
cc CHEBYSHEV2_SET sets a Chebyshev Type 2 quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c*********************************************************************72
c
cc CHEBYSHEV3_COMPUTE computes a Gauss-Chebyshev type 3 quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    The N = 1 rule is exceptional.  It consists of a single
c    point at 0, with weight PI.
c
c    For rules with N = 2 or greater, the following remarks apply:
c
c    If N points are used, then Gauss-Chebyshev quadrature
c    will compute the integral exactly, whenever F(X) is a polynomial
c    of degree 2*N-3 or less.
c
c    The abscissas include -1 and 1.
c
c    The first and last weights are 0.5 * PI / ( N - 1),
c    and all other weights are PI / ( N - 1 ).
c
c    If the order is doubled, the abscissas of the new rule include
c    all the points of the old rule.  This fact can be used to
c    efficiently implement error estimation.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c  Take care of the special case N = 1.
c
      if ( n .eq. 1 ) then
        x(1) = 0.0D+00
        w(1) = r8_pi
      ...
      do i = 1, n
        ...
        angle = dble ( n - i ) * r8_pi / dble ( n - 1 )
        x(i) = cos ( angle )

      end do

      w(1) = r8_pi / dble ( 2 * ( n - 1 ) )
      do i = 2, n - 1
        w(i) = r8_pi / dble ( n - 1 )
      end do
      w(n) = r8_pi / dble ( 2 * ( n - 1 ) )
...
c*********************************************************************72
c
cc CHEBYSHEV3_INTEGRAL evaluates a monomial Chebyshev type 3 integral.
c
c  Discussion:
c
c    The integral:
...
!
!    Output, double precision X(N), the abscissas.
!
!    Output, double precision W(N), the weights.
!
      implicit none
...
c  Discussion:
c
c    This method uses a direct approach.  The paper by Waldvogel
c    exhibits a more efficient approach using Fourier transforms.
c
c    The integral:
c
c      integral ( -1 <= x <= 1 ) f(x) dx
...
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    The abscissas for the rule of order N can be regarded
c    as the cosines of equally spaced angles between 180 and 0 degrees:
c
c      X(I) = cos ( ( N - I ) * PI / ( N - 1 ) )
c
c    except for the basic case N = 1, when
c
c      X(1) = 0.
c
c    A Clenshaw-Curtis rule that uses N points will integrate
c    exactly all polynomials of degrees 0 through N-1.  If N
c    is odd, then by symmetry the polynomial of degree N will
c    also be integrated exactly.
c
c    If the value of N is increased in a sensible way, then
c    the new set of abscissas will include the old ones.  One such
c    sequence would be N(K) = 2*K+1 for K = 0, 1, 2, ...
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do i = 1, n
        theta(i) = dble ( n - i ) * r8_pi / dble ( n - 1 )
      end do
...
c*********************************************************************72
c
cc CLENSHAW_CURTIS_SET sets a Clenshaw-Curtis quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c    the new set of abscissas will include the old ones.  One such
c    sequence would be N(K) = 2*K+1 for K = 0, 1, 2, ...
c    Thus, in the table below, the abscissas for order 9 include
c    those for order 5.
c
c  Licensing:
c
...
c
c    Input, integer N, the order.
c    N must be between 1 and 17, 33, 65 or 129.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc FEJER1_COMPUTE computes a Fejer type 1 quadrature rule.
c
c  Discussion:
c
c    This method uses a direct approach.  The paper by Waldvogel
...
c
     ...
     &    / dble ( 2 * n )
      ...
      do i = 1, n
        ...
        do j = 1, ( n / 2 )
          w(i) = w(i) 
     &      - 2.0D+00 * cos ( 2.0D+00 * dble ( j ) * theta(i) ) 
     &      / dble ( 4 * j * j - 1 )
        end do
      end do

      do i = 1, n
        w(i) = 2.0D+00 * w(i) / dble ( n )
      end do
...
c*********************************************************************72
c
cc FEJER1_SET sets abscissas and weights for Fejer type 1 quadrature.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N should be between 1 and 10.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc FEJER2_COMPUTE computes a Fejer type 2 quadrature rule.
c
c  Discussion:
c
c    This method uses a direct approach.  The paper by Waldvogel
...
c
      ...
      do i = 1, n
        theta(i) = dble ( n + 1 - i ) * r8_pi / dble ( n + 1 )
      end do
      ...
      do i = 1, n
        ...
        do j = 1, ( ( n - 1 ) / 2 )
          w(i) = w(i) - 2.0D+00 
     &      * cos ( 2.0D+00 * dble ( j ) * theta(i) ) 
     &      / dble ( 4 * j * j - 1 )
        end do

        if ( 2 .lt. n ) then
          p = 2.0D+00 * dble ( ( ( n + 1 ) / 2 ) ) - 1.0D+00
...
c*********************************************************************72
c
cc FEJER2_SET sets abscissas and weights for Fejer type 2 quadrature.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GEGENBAUER_COMPUTE computes a Gauss-Gegenbauer quadrature rule.
c
c  Discussion:
c
c    The integral:
c
c      integral ( -1 <= x <= 1 ) (1-x*x)^alpha * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    Thanks to Janiki Raman for pointing out a problem in an earlier
c    version of the code that occurred when ALPHA was -0.5.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    Input, integer N, the order.
c
c    Input, double precision ALPHA, the exponent of (1-X*X) in the weight.
c    -1.0 < ALPHA.
c
c    Output, double precision X(N), the abscissas.
c
...
c
      ...
      do i = 3, n
        ...
        c(i) = dble ( i - 1 ) 
     &        * ( alpha + alpha + dble ( i - 1 ) ) / 
     &        ( ( alpha + alpha + dble ( 2 * i - 1 ) ) 
     &        * ( alpha + alpha + dble ( 2 * i - 3 ) ) )
     ...
     &      / r8_gamma ( alpha + alpha + 2.0D+00 )
      ...
      cc = delta * 2.0D+00**( 2.0D+00 * alpha + 1.0D+00 ) 
     ...
     &      ( ( 6.28D+00 + alpha ) * dble ( n * n ) )
          ...
          r1 = ( 1.0D+00 + 0.235D+00 * alpha ) 
     &      / ( 0.766D+00 + 0.119D+00 * alpha )
     ...
     &      dble ( n * n ) ) )
          ...
          r1 = ( 1.0D+00 + 0.37D+00 * alpha ) 
     &      / ( 1.67D+00 + 0.28D+00 * alpha )
...
c*********************************************************************72
c
cc GEGENBAUER_INTEGRAL: integral of a monomial with Gegenbauer weight.
c
c  Discussion:
c
c    The integral:
c
c      integral ( -1 <= x <= +1 ) x^expon (1-x*x)^alpha dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GEGENBAUER_RECUR finds the value and derivative of a Gegenbauer polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GEGENBAUER_ROOT improves an approximate root of a Gegenbauer polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    Input, integer N, the order of the polynomial.
c
c    Input, double precision ALPHA, the exponent of (1-X^2) in the weight.
c    -1.0 < ALPHA.
c
c    Output, double precision DP2, the value of J'(N)(X).
c
...
c*********************************************************************72
c
cc GEN_HERMITE_DR_COMPUTE computes a generalized Gauss-Hermite rule.
c
c  Discussion:
c
c    The integral:
c
c      integral ( -oo .lt. x .lt. +oo ) |x|^alpha exp(-x^2) f(x) dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      double precision alpha
      double precision alpha_laguerre
      ...
      double precision s
      double precision w(n)
      double precision w_laguerre(n/2)
      double precision x(n)
      double precision x_laguerre(n/2)
c
c  Generate the related generalized Laguerre rule.
c
      if ( n .eq. 1 ) then
      ...
      call gen_laguerre_ss_compute ( n_laguerre, alpha_laguerre, 
     &  x_laguerre, w_laguerre )

      if ( mod ( n, 2 ) .eq. 0 ) then

        do i = 1, n_laguerre
          x(i) = - sqrt ( x_laguerre(n_laguerre+1-i) )
        end do

        do i = n_laguerre + 1, n_laguerre + n_laguerre
          x(i) = sqrt ( x_laguerre(i-n_laguerre) )
        end do

        do i = 1, n_laguerre
          w(i) = 0.5D+00 * w_laguerre(n_laguerre+1-i)
        end do

        do i = n_laguerre + 1, n_laguerre + n_laguerre
          w(i) = 0.5D+00 * w_laguerre(i-n_laguerre)
        end do
        ...
        do i = n_laguerre + 2, n_laguerre + n_laguerre + 1
          x(i) = sqrt ( x_laguerre(i-n_laguerre-1) )
        end do

        do i = 1, n_laguerre
          w(i) = 0.5D+00 * w_laguerre(n_laguerre+1-i) 
     &      / x_laguerre(n_laguerre+1-i)
        end do
        ...
        s = 0.0D+00
        do i = 1, n_laguerre
          s = s + w_laguerre(i) / x_laguerre(i)
        end do

        w(n_laguerre+1) = r8_gamma ( arg ) - s

        do i = n_laguerre + 2, n_laguerre + n_laguerre + 1
          w(i) = 0.5D+00 * w_laguerre(i-n_laguerre-1) 
     &      / x_laguerre(i-n_laguerre-1)
        end do
...
c*********************************************************************72
c
cc GEN_HERMITE_EK_COMPUTE computes a generalized Gauss-Hermite quadrature rule.
c
c  Discussion:
c
...
c*********************************************************************72
c
cc GEN_HERMITE_INTEGRAL evaluates a monomial generalized Hermite integral.
c
c  Discussion:
c
c    The integral:
c
c      integral ( -oo < x < +oo ) x^n |x|^alpha exp(-x^2) dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, integer EXPON, the exponent of the monomial.
c    0 <= EXPON.
c
c    Input, double precision ALPHA, the exponent of |X| in the
c    weight function.  -1.0 < ALPHA.
c
c    Output, double precision VALUE, the value of the integral.
c
      implicit none
...
c*********************************************************************72
c
cc GEN_LAGUERRE_EK_COMPUTE: generalized Gauss-Laguerre quadrature rule.
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 <= x < +oo ) exp ( - x ) * x^alpha * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c    The integral:
c
c      integral ( 0 <= x < +oo ) x^alpha * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * exp ( x(i) ) * f ( x(i) )
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GEN_LAGUERRE_INTEGRAL evaluates a monomial genearlized Laguerre integral.
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 <= x < +oo ) x^n * x^alpha * exp ( -x ) dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GEN_LAGUERRE_SS_COMPUTE: generalized Gauss-Laguerre quadrature rule.
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 .le. x .lt. +oo ) exp ( - x ) * x^alpha * f(x) dx
c
c    The quadrature rule:
c
...
c    The integral:
c
c      integral ( 0 .le. x .lt. +oo ) x^alpha * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 .le. i .le. n ) w(i) * exp ( x(i) ) * f ( x(i) )
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      cc = r8_gamma ( alpha + 1.0D+00 ) * r8vec_product ( n - 1, c(2) )
...
c
     ...
     &      ( 1.0D+00 + 2.4D+00 * dble ( n ) + 1.8D+00 * alpha )

        else if ( i .eq. 2 ) then

          xval = xval + ( 15.0D+00 + 6.25D+00 * alpha ) / 
     &      ( 1.0D+00 + 0.9D+00 * alpha + 2.5D+00 * dble ( n ) )
...
c
      ...
      subroutine gen_laguerre_ss_recur ( p2, dp2, p1, x, n, alpha, 
     &  b, c )

c*********************************************************************72
c
cc GEN_LAGUERRE_SS_RECUR evaluates a generalized Laguerre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc GEN_LAGUERRE_SS_ROOT seeks roots of a generalized Laguerre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      call imtqlx ( n, x, bj, w )
c
c  If N is odd, force the center X to be zero.
c
...
c*********************************************************************72
c
cc HERMITE_GK16_SET sets a Hermite Genz-Keister 16 rule.
c
c  Discussion:
c
c    The integral:
c
c      integral ( -oo <= x <= +oo ) f(x) exp ( - x * x ) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    A nested family of rules for the Hermite integration problem
c    was produced by Genz and Keister.  The structure of the nested
c    family was denoted by 1+2+6+10+16, that is, it comprised rules
c    of successive orders O = 1, 3, 9, 19, and 35.
c
c    The precisions of these rules are P = 1, 5, 15, 29, and 51.
c
c    Consider, however, the special cases where a rule of precision
c    at least 7, 17, 31 or 33 is desired.  Ordinarily, this would
c    suggest using the nested rule of order 9, 19, 51 or 51 respectively.
c    In these cases, however, the order of the rule that is used exceeds
c    the precision requested.  Hence, it is possible simply to select
c    a subset of the points in the higher precision rule and get a
c    rule of lower order and the desired precision.  This accounts for
c    the four extra rules in this family.
c
c    The entire list of rules is therefore:
c
c    L   P   N
c
c    0   1   1  <-- Full rule 0
c    1   5   3  <-- Full rule 1
c    2   7   7  <-- Partial rule
c    3  15   9  <-- Full rule 2
c    4  17  17  <-- Partial rule
c    5  29  19  <-- Full rule 3
c    6  31  31  <-- Partial rule
c    7  33  33  <-- Partial rule
c    8  51  35  <-- Full rule 4
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N must be 1, 3, 7, 9, 17, 19, 31, 33 or 35.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc HERMITE_GK18_SET sets a Hermite Genz-Keister 18 rule.
c
c  Discussion:
c
c    The integral:
...
c    A nested family of rules for the Hermite integration problem
c    was produced by Genz and Keister.  The structure of the nested
c    family was denoted by 1+2+6+10+18, that is, it comprised rules
c    of successive orders O = 1, 3, 9, 19, and 37.
c
c    The precisions of these rules are P = 1, 5, 15, 29, and 55.
c
c    Some of the data in this function was kindly supplied directly by
c    Alan Genz on 24 April 2011.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc HERMITE_GK22_SET sets a Hermite Genz-Keister 22 rule.
c
c  Discussion:
c
c    The integral:
...
c    A nested family of rules for the Hermite integration problem
c    was produced by Genz and Keister.  The structure of the nested
c    family was denoted by 1+2+6+10+22, that is, it comprised rules
c    of successive orders O = 1, 3, 9, 19, and 41.
c
c    The precisions of these rules are P = 1, 5, 15, 29, and 63.
c
c    Some of the data in this function was kindly supplied directly by
c    Alan Genz on 24 April 2011.
c
...
c
c    Input, integer N, the order.
c    Legal values are 1, 3, 9, 19, and 41.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc HERMITE_GK24_SET sets a Hermite Genz-Keister 24 rule.
c
c  Discussion:
c
c    The integral:
...
c    A nested family of rules for the Hermite integration problem
c    was produced by Genz and Keister.  The structure of the nested
c    family was denoted by 1+2+6+10+24, that is, it comprised rules
c    of successive orders O = 1, 3, 9, 19, and 43.
c
c    The precisions of these rules are P = 1, 5, 15, 29, and 67.
c
c    Some of the data in this function was kindly supplied directly by
c    Alan Genz on 24 April 2011.
c
...
c
c    Input, integer N, the order.
c    Legal values are 1, 3, 9, 19, and 43.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c    The integral:
c
c      integral ( -oo < x < +oo ) x^n exp(-x^2) dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c*********************************************************************72
c
cc HERMITE_PROBABILIST_SET: probabilist Hermite quadrature.
c
c  Discussion:
c
c    The integral:
...
c    The weight:
c
c      rho(x) = exp ( - x * x / 2 ) / sqrt ( 2 * pi ) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c*********************************************************************72
c
cc HERMITE_SET sets abscissas and weights for Hermite quadrature.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    Mathematica can numerically estimate the abscissas of the rule
c    of order N to P digits by the command:
c
c      NSolve [ HermiteH [ n, x ] .eq. 0, x, p ]
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N must be between 1 and 20, 31/32/33, 63/64/65, 127/128/129.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc HERMITE_1_SET sets abscissas and weights for Hermite quadrature.
c
c  Discussion:
c
c    This routine is for the case with unit density:
c      integral ( -oo < x < +oo ) f(x) dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc HERMITE_SS_COMPUTE computes a Gauss-Hermite quadrature rule.
c
c  Discussion:
c
c    The code uses an algorithm by Stroud and Secrest.
c
c    The abscissas are the zeros of the N-th order Hermite polynomial.
c
c    The integral:
...
c
      ...
      cc = sqrt ( r8_pi ) * r8_gamma ( dble ( n ) ) 
     &  / ( 2.0D+00 ** ( n - 1 ) )

      s = ( 2.0D+00 * dble ( n ) + 1.0D+00 )**( 1.0D+00 / 6.0D+00 )
      ...
      do i = 1, ( n + 1 ) / 2
        ...
        if ( i .eq. 1 ) then
          ...
          xval = xval - 1.14D+00 * ( ( dble ( n ) )**0.426D+00 ) / xval
...
c
c  Reverse the abscissas.
c  Because of symmetry, the weights are unchanged,
c  and the abscissas simply change sign.
c
      do i = 1, n
        x(i) = - x(i)
      end do
...
c*********************************************************************72
c
cc HERMITE_SS_RECUR finds the value and derivative of a Hermite polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Output, double precision P2, the value of H(N)(X).
c
c    Output, double precision DP2, the value of H'(N)(X).
c
c    Output, double precision P1, the value of H(N-1)(X).
c
c    Input, double precision X, the point at which polynomials are evaluated.
c
c    Input, integer N, the order of the polynomial.
...
c*********************************************************************72
c
cc HERMITE_SS_ROOT improves an approximate root of a Hermite polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc KRONROD_SET sets abscissas and weights for Gauss-Kronrod quadrature.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    A Kronrod rule is used in conjunction with a lower order
c    Gauss rule, and provides an efficient error estimation.
c
c    The error may be estimated as the difference in the two integral
c    approximations.
c
c    The efficiency comes about because the Kronrod rule uses the abscissas
c    of the Gauss rule, thus saving on the number of function evaluations
c    necessary.  If the Kronrod rule were replaced by a Gauss rule of
c    the same order, a higher precision integral estimate would be
c    made, but the function would have to be evaluated at many more
c    points.
c
c    The Gauss Kronrod pair of rules involves an ( N + 1 ) / 2
c    point Gauss-Legendre rule and an N point Kronrod rule.
c    Thus, the 15 point Kronrod rule should be paired with the
c    Gauss-Legendre 7 point rule.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N may be 15, 21, 31 or 41, corresponding to Gauss-Legendre rules of
c    order 7, 10, 15 or 20.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc LAGUERRE_EK_COMPUTE: Laguerre quadrature rule by the Elhay-Kautsky method.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc LAGUERRE_INTEGRAL evaluates a monomial Laguerre integral.
c
c  Discussion:
c
c    The integral:
...
c*********************************************************************72
c
cc LAGUERRE_SET sets abscissas and weights for Laguerre quadrature.
c
c  Discussion:
c
c    The abscissas are the zeroes of the Laguerre polynomial L(N)(X).
c
c    The integral:
c
c      integral ( 0 <= x .lt. +oo ) exp ( -x ) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c    The integral:
c
c      integral ( 0 <= x .lt. +oo ) f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * exp ( x(i) ) * f ( x(i) )
c
c    Mathematica can numerically estimate the abscissas for the
c    n-th order polynomial to p digits of precision by the command:
c
c      NSolve [ LaguerreL[n,x] .eq. 0, x, p ]
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      if ( n .eq. 1 ) then
        ...
        write ( *, '(a,i8)' ) '  Illegal value of N = ', n
        write ( *, '(a)' ) 
     &    '  Legal values are 1 to 20, 31/32/33, 63/64/65 ' //
     &    'or 127/128/129.'
        stop 1
...
c*********************************************************************72
c
cc LAGUERRE_1_SET sets abscissas and weights for Laguerre quadrature.
c
c  Discussion:
c
c    This routine is specialized for the case where the density function is 1.
c
c    The integral:
c      I(f) = integral ( 0 <= x < +oo ) f(x) dx
c    The quadrature rule:
...
c*********************************************************************72
c
cc LAGUERRE_SS_COMPUTE computes a Gauss-Laguerre quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c
      ...
      do i = 1, n
        c(i) = dble ( i - 1 ) * ( dble ( i - 1 ) )
      end do

      cc = r8vec_product ( n - 1, c(2) )
...
c
        if ( i .eq. 1 ) then
          ...
          xval = 3.0D+00 / ( 1.0D+00 + 2.4D+00 * dble ( n ) )

        else if ( i .eq. 2 ) then

          xval = xval + 15.0D+00 / ( 1.0D+00 + 2.5D+00 * dble ( n ) )
...
c*********************************************************************72
c
cc LAGUERRE_SS_RECUR finds the value and derivative of a Laguerre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc LAGUERRE_SS_ROOT improves an approximate root of a Laguerre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc LAGUERRE_SUM carries out Laguerre quadrature over [ A, +oo ).
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 <= x <= +oo ) exp ( -x ) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c  Parameters:
c
c    Input, external FUNC, the name of the FORTRAN function which
c    evaluates the integrand.  The function must have the form
c      function func ( x ).
c
c    Input, double precision A, the beginning of the integration interval.
c
c    Input, integer N, the order.
c
c    Input, double precision X(N), the abscissas.
...
c*********************************************************************72
c
cc LEGENDRE_COS2_SET sets a Gauss-Legendre rule for COS(X) * F(X) on [0,PI/2].
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 <= x <= pi/2 ) cos(x) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c    The integral:
c
c      integral ( 0 <= x <= pi/2 ) sin(x) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( pi/2 - x(i) )
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Gwynne Evans,
c    Practical Numerical Integration,
c    Wiley, 1993,
c    ISBN: 047193898X,
c    LC: QA299.3E93.
c
...
c
c    Input, integer N, the order.
c    N must be between 2, 4, 8 or 16.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc LEGENDRE_COS_SET: Gauss-Legendre rule for COS(X) * F(X) on [-PI/2,PI/2].
c
c  Discussion:
c
c    The integral:
c
c      integral ( -pi/2 <= x <= pi/2 ) cos(x) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c    The integral:
c
c      integral ( 0 <= x <= pi ) sin(x) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) + pi/2 )
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, integer N, the order.
c    N must be between 1, 2, 4, 8 or 16.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc LEGENDRE_GW_COMPUTE: Legendre quadrature rule by the Golub-Welsch method.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      do i = 1, n - 1
        beta(i) = 1.0D+00 / dble ( 2 * i )
      end do

      do i = 1, n - 1
        beta(i) = 0.5D+00 
     &   / sqrt ( ( 1.0D+00 - beta(i) ) * ( 1.0D+00 + beta(i) ) )
      end do
c
...
c
c  X values are eigenvalues.
c
      do i = 1, n
        x(i) = d(i)
      end do
c
c  W is related to first eigenvector.
c
      do i = 1, n
        w(i) = 2.0D+00 * z(1,i)**2
      end do
...
c*********************************************************************72
c
cc LEGENDRE_LOG_COMPUTE: Gauss-Legendre rules for - LOG(X) * F(X) on [0,1].
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 .le. x .le. 1 ) - log(x) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 .le. i .le. n ) w(i) * f ( x(i) )
c
c    Note that this rule is reasonably accurate for values of N up to 13.
c    However, for larger values of N, the weight calculation loses so much
c    accuracy that the formula is not reliable.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Author:
c
c    Original FORTRAN77 version by Federico Paris, Jose Canas.
c    This FORTRAN77 version by John Burkardt.
c
c  Reference:
c
c    Donald Anderson,
c    Gaussian Quadrature Formulae for the integral from 0 to 1 of
c    -ln(X) f(X) dx,
c    Mathematics of Computation,
c    Volume 19, Number 91, Number 3, July 1965, pages 477-481.
c
...
c
      do k = 2, n
        ...
        do i = 0, k - 1
          do j = 0, k - 1
            tk = tk + coef(i,i1) * coef(j,i1) 
     &        / ( dble ( i + j + 1 ) )**2
            uk = uk + coef(i,i1) * coef(j,i1) 
     &        / ( dble ( i + j + 2 ) )**2
          end do
        end do
        rk = uk / tk
     ...
     &      * ( coef(i-1,i1) - rk * coef(i,i1) ) - sk * coef(i,i2)
        ...
        do i = 1, k
          call legendre_log_roots ( k, coef(0,i0), root(i-1,ir1), 
     &      root(i,ir1), tol, root(i,ir0) )
        end do
        root(  0,ir0) = 0.0D+00
...
c
c  Compute the abscissas and weights.
c
      do i = 1, n
        x(i) = root(i,ir1)
      end do
      ...
      do i = 1, n
        ...
        v2 = coef(j,  i2)
        do j = 1, n - 1
          v1 = v1 + dble ( j + 1 ) * coef(j+1,i1) * x(i)**j
...
c*********************************************************************72
c
cc LEGENDRE_LOG_ROOTS is a root finder for LEGENDRE_LOG_COMPUTE.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Input, double precision COEF(N+1), the coefficients of the polynomial.
c
c    Input, double precision X00, XFF, two points at which the polynomial
c    has opposite sign.
c
c    Input, double precision TOL, a tolerance for the size of the
c    uncertainty interval in the value of XI.
c
c    Output, double precision XI, the approximate root.
c
      implicit none

      integer n

      double precision coef(n+1)
      double precision errac
      double precision tol
      double precision v0
      double precision vf
      double precision vi
...
c*********************************************************************72
c
cc LEGENDRE_LOG_SET sets a Gauss-Legendre rule for - LOG(X) * F(X) on [0,1].
c
c  Discussion:
c
c    The integral:
c
c      integral ( 0 <= x <= 1 ) - log(x) * f(x) dx
c
c    The quadrature rule:
c
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
...
c
c    Input, integer N, the order.
c    N must be between 1 through 8, or 16.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc LEGENDRE_LOG_VALUE evaluates a polynomial for LEGENDRE_LOG_ROOT.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    Input, integer N, the degree of the polynomial.
c
c    Input, double precision COEF(N+1), the polynomial coefficients.
c
c    Input, double precision X, the evaluation point.
c
c    Output, double precision V, the value of the polynomial at X.
c
      implicit none
...
c*********************************************************************72
c
cc LEGENDRE_POLYNOMIAL_VALUE evaluates a Legendre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, integer M, the number of values.
c
c    Input, integer N, the order of the polynomial.
c
c    Input, double precision X(M), the evaluation points.
c
c    Output, double precision P(M), the value of P(N)(X(1:M)).
c
      implicit none

      integer m

      double precision dp0(m)
      double precision dp1(m)
      double precision dp2(m)
      integer i
      integer j
      integer n
      double precision p(m)
      double precision p0(m)
      double precision p1(m)
      double precision p2(m)
      ...
      do j = 2, n
        ...
        do i = 1, m
          ...
          p2(i) = ( dble ( 2 * j - 1 ) * x(i) * p1(i) 
     &            + dble (   - j + 1 )          * p0(i) ) 
     &            / dble (     j )

          dp2(i) = 
     &      ( dble ( 2 * j - 1 ) * ( p1(i) + x(i) * dp1(i) ) 
     &      - dble (     j - 1 ) * dp0(i) ) 
     &      / dble (     j )
...
c*********************************************************************72
c
cc LEGENDRE_RECUR finds the value and derivative of a Legendre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Output, double precision P2, the value of P(N)(X).
c
c    Output, double precision DP2, the value of P'(N)(X).
c
c    Output, double precision P1, the value of P(N-1)(X).
c
c    Input, double precision X, the point at which polynomials are evaluated.
c
c    Input, integer N, the order of the polynomial.
...
c*********************************************************************72
c
cc LEGENDRE_ROOT improves an approximate root of a Legendre polynomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc LEGENDRE_SQRTX2_01_SET: Gauss-Legendre rule for F(X) / SQRT(X) on [0,1].
c
c  Discussion:
c
c    The integral:
c
c      Integral ( 0 .le. X .le. 1 ) F(X) / SQRT ( X ) dx
c
c    The quadrature rule:
c
c      Sum ( 1 .le. I .le. N ) W(I) * F ( X(I) )
...
c
      ...
      integer n2
      double precision w(n)
      double precision w2(2*n+1)
      double precision x(n)
      double precision x2(2*n+1)
...
c*********************************************************************72
c
cc LEGENDRE_SQRTX_01_SET sets a Gauss-Legendre rule for SQRT(X) * F(X) on [0,1].
c
c  Discussion:
c
c    The integral:
c
c      Integral ( 0 .le. X .le. 1 ) SQRT ( X ) * F(X) dx
c
c    The quadrature rule:
c
c      Sum ( 1 .le. I .le. N ) W(I) * F ( X(I) )
...
c*********************************************************************72
c
cc LEGENDRE_X0_01_SET sets a Gauss-Legendre rule for F(X) on [0,1].
c
c  Discussion:
c
c    The integral:
...
c*********************************************************************72
c
cc LEGENDRE_X1_01_SET sets a Gauss-Legendre rule for X * F(X) on [0,1].
c
c  Discussion:
c
c    The integral:
c
c      Integral ( 0 <= X <= 1 ) X * F(X) dx
c
c    The quadrature rule:
c
c      Sum ( 1 <= I <= N ) W(I) * F ( X(I) )
...
c*********************************************************************72
c
cc LEGENDRE_X1_SET sets a Gauss-Legendre rule for ( 1 + X ) * F(X) on [-1,1].
c
c  Discussion:
c
c    The integral:
...
c*********************************************************************72
c
cc LEGENDRE_X2_01_SET sets a Gauss-Legendre rule for X * X * F(X) on [0,1].
c
c  Discussion:
c
c    The integral:
c
c      Integral ( 0 <= X <= 1 ) X * X * F(X) dx
c
c    The quadrature rule:
c
c      Sum ( 1 <= I <= N ) W(I) * F ( X(I) )
...
c*********************************************************************72
c
cc LEGENDRE_X2_SET sets a Gauss-Legendre rule for ( 1 + X )^2 * F(X) on [-1,1].
c
c  Discussion:
c
c    The integral:
c
c      Integral ( -1 <= X <= 1 ) ( 1 + X ) * ( 1 + X ) * F(X) dx
c
c    The quadrature rule:
c
c      Sum ( 1 <= I <= N ) W(I) * F ( X(I) )
...
c*********************************************************************72
c
cc LOBATTO_COMPUTE computes a Lobatto quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c      Sum ( 1 .le. I .le. N ) W(I) * F ( X(I) )
c
c    The quadrature rule will integrate exactly all polynomials up to
c    X^(2*N-3).
c
c    The Lobatto rule is distinguished by the fact that both endpoints
c    (-1 and 1) are always abscissas.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    LC: QA47.A34.
c
c    Claudio Canuto, Yousuff Hussaini, Alfio Quarteroni, Thomas Zang,
c    Spectral Methods in Fluid Dynamics,
c    Springer, 1993,
c    ISNB13: 978-3540522058,
c    LC: QA377.S676.
c
c    Arthur Stroud, Don Secrest,
c    Gaussian Quadrature Formulas,
...
c
      ...
      tolerance = 100.0D+00 * r8_epsilon ( )
c
c  Initial estimate for the abscissas is the Chebyshev-Gauss-Lobatto nodes.
c
      do i = 1, n
...
10    continue
      ...
      test = 0.0D+00
      do i = 1, n
        test = max ( test, abs ( x(i) - xold(i) ) )
      end do

      if ( tolerance .lt. test ) then
     ...
     &           / ( dble ( n ) * p(i,n) )
      ...
      do i = 1, n
        w(i) = 2.0D+00 / ( dble ( ( n - 1 ) * n ) * p(i,n)**2 )
      end do
...
c*********************************************************************72
c
cc LOBATTO_SET sets abscissas and weights for Lobatto quadrature.
c
c  Discussion:
c
c    The integral:
...
c*********************************************************************72
c
cc MOULTON_SET sets weights for Adams-Moulton quadrature.
c
c  Discussion:
c
c    Adams-Moulton quadrature formulas are normally used in solving
c    ordinary differential equations, and are not suitable for general
c    quadrature computations.  However, an Adams-Moulton formula is
c    equivalent to approximating the integral of F(Y(X)) between X(M)
c    and X(M+1), using an implicit formula that relies on known values
c    of F(Y(X)) at X(M-N+1) through X(M), plus the unknown value at X(M+1).
c
c    Suppose the unknown function is denoted by Y(X), with derivative F(Y(X)),
c    and that approximate values of the function are known at a series of
c    X values, which we write as X(1), X(2), ..., X(M).  We write the value
c    Y(X(1)) as Y(1) and so on.
c
c    Then the solution of the ODE Y' = F(X,Y) at the next point X(M+1) is
c    computed by:
c
c      Y(M+1) = Y(M) + Integral ( X(M) .lt. X .lt. X(M+1) ) F(Y(X)) dx
c             = Y(M) + H * Sum ( 1 <= I <= N ) W(I) * F(Y(M+2-I)) approximately.
c
c    Note that this formula is implicit, since the unknown value Y(M+1)
c    appears on the right hand side.  Hence, in ODE applications, this
c    equation must be solved via a nonlinear equation solver.  For
c    quadrature problems, where the function to be integrated is known
c    beforehand, this is not a problem, and the calculation is explicit.
c
c    In the documentation that follows, we replace F(Y(X)) by F(X).
c
c
c    The Adams-Moulton formulas require equally spaced data.
c
c    Here is how the formula is applied in the case with non-unit spacing:
c
c      Integral ( A <= X <= A+H ) F(X) dx =
c      H * Sum ( 1 <= I <= N ) W(I) * F ( A - (I-2)*H ),
c      approximately.
c
c    The integral:
c
...
c
c    Input, integer N, the order.
c    N must be between 1 and 10, 12, 14, 16, 18 or 20.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
c    W(1) is the weight at X = 1, W(2) the weight at X = 0, and so on.
c    The weights are rational.  The weights are not symmetric, and
c    some weights may be negative.  They should sum to 1.
c
      implicit none
...
c*********************************************************************72
c
cc NC_COMPUTE computes a Newton-Cotes quadrature rule.
c
c  Discussion:
c
c    The integral:
c
c      Integral ( X_MIN <= X <= X_MAX ) F(X) dx
c
c    The quadrature rule:
c
c      Sum ( 1 <= I <= N ) W(I) * F ( X(I) ).
c
c    For the CLOSED rule, the abscissas include the end points.
c    For an OPEN rule, the abscissas do not include the end points.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    Input, integer N, the order.
c
c    Input, double precision X_MIN, X_MAX, the endpoints of the interval.
c
c    Input, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c
        yvala = d(n) / dble ( n )
        do j = n - 1, 1, -1
          yvala = yvala * x_min + d(j) / dble ( j )
        end do
        yvala = yvala * x_min

        yvalb = d(n) / dble ( n )
        do j = n - 1, 1, -1
          yvalb = yvalb * x_max + d(j) / dble ( j )
        end do
        yvalb = yvalb * x_max
...
c*********************************************************************72
c
cc NCC_COMPUTE: Newton-Cotes Closed quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= I <= N ) W(I) * F ( X(I) ).
c
c    For the CLOSED rule, the abscissas are equally spaced and include
c    the end points.
c
c  Licensing:
c
...
c*********************************************************************72
c
cc NCC_COMPUTE_POINTS: Newton-Cotes Closed points
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc NCC_COMPUTE_WEIGHTS: Newton-Cotes Closed weights.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc NCC_SET sets abscissas and weights for Newton-Cotes closed quadrature.
c
c  Discussion:
c
c    The Newton-Cotes closed rules use equally spaced abscissas, and
c    hence may be used with tabulated function data.
c
c    The rules are called "closed" because they include the endpoints.
c    As a favor, we include an order 1 rule, the midpoint rule, even
c    though this does not satisfy the requirement that the endpoints
c    be included!
c
c    The higher order rules involve negative weights.  These can produce
c    loss of accuracy due to the subtraction of large, nearly equal quantities.
c
c    N = 1 is the midpoint rule (and is not really an NCC rule!)
c    N = 2 is the trapezoidal rule.
c    N = 3 is Simpson's rule.
c    N = 4 is Simpson's 3/8 rule.
c    N = 5 is Bode's rule.
c
c    The Kopal reference for N = 12 lists
c      W(6) = 15494566.0D+00 / 43545600.0D+00
c    but this results in a set of coeffients that don't add up to 2.
c    The correct value is
c      W(6) = 15493566.0D+00 / 43545600.0.
c
c    The integral:
c
c      Integral ( -1 <= X <= 1 ) F(X) dx
...
c      Sum ( 1 <= I <= N ) W(I) * F ( X(I) )
c
c    In Mathematica, the Newton-Cotes closed weights of order N in
c    the interval [-1,+1] can be computed by:
c
c      Needs["NumericalDifferentialEquationAnalysis`"]
c      NewtonCotesWeights [ n, -1, 1, QuadratureType -> Closed ]
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Johnson,
c    Quarterly Journal of Mathematics,
c    Volume 46, Number 52, 1915.
c
c    Zdenek Kopal,
c    Numerical Analysis,
c    John Wiley, 1955,
...
c
c      6137698213 / 150885504000
c     42194238652 / 150885504000
c   - 23361540993 / 150885504000
c    116778274403 / 150885504000
c  - 113219777650 / 150885504000
c    154424590209 / 150885504000
c   - 32067978834 / 150885504000
c   - 32067978834 / 150885504000
c    154424590209 / 150885504000
c  - 113219777650 / 150885504000
c    116778274403 / 150885504000
...
c
c       90241897 / 2501928000
c      710986864 / 2501928000
c    - 770720657 / 2501928000
c     3501442784 / 2501928000
c   - 6625093363 / 2501928000
c    12630121616 / 2501928000
c  - 16802270373 / 2501928000
c    19534438464 / 2501928000
c  - 16802270373 / 2501928000
c    12630121616 / 2501928000
c   - 6625093363 / 2501928000
c     3501442784 / 2501928000
...
c
c     105930069 / 3099672576
c     796661595 / 3099672576
c   - 698808195 / 3099672576
c    3143332755 / 3099672576
c  - 4688522055 / 3099672576
c    7385654007 / 3099672576
c  - 6000998415 / 3099672576
c    3056422815 / 3099672576
c    3056422815 / 3099672576
c  - 6000998415 / 3099672576
c    7385654007 / 3099672576
c  - 4688522055 / 3099672576
...
c
c       15043611773 / 488462349375
c      127626606592 / 488462349375
c    - 179731134720 / 488462349375
c      832211855360 / 488462349375
c   - 1929498607520 / 488462349375
c     4177588893696 / 488462349375
c   - 6806534407936 / 488462349375
c     9368875018240 / 488462349375
c  - 10234238972220 / 488462349375
c     9368875018240 / 488462349375
c   - 6806534407936 / 488462349375
c     4177588893696 / 488462349375
c   - 1929498607520 / 488462349375
...
c
c       55294720874657 / 1883051089920000
c      450185515446285 / 1883051089920000
c    - 542023437008852 / 1883051089920000
c     2428636525764260 / 1883051089920000
c   - 4768916800123440 / 1883051089920000
c     8855416648684984 / 1883051089920000
c  - 10905371859796660 / 1883051089920000
c    10069615750132836 / 1883051089920000
c   - 3759785974054070 / 1883051089920000
c   - 3759785974054070 / 1883051089920000
c    10069615750132836 / 1883051089920000
c  - 10905371859796660 / 1883051089920000
c     8855416648684984 / 1883051089920000
...
c
c       203732352169 / 7604556960000
c      1848730221900 / 7604556960000
c    - 3212744374395 / 7604556960000
c     15529830312096 / 7604556960000
c   - 42368630685840 / 7604556960000
c    103680563465808 / 7604556960000
c  - 198648429867720 / 7604556960000
c    319035784479840 / 7604556960000
c  - 419127951114198 / 7604556960000
c    461327344340680 / 7604556960000
c  - 419127951114198 / 7604556960000
c    319035784479840 / 7604556960000
c  - 198648429867720 / 7604556960000
c    103680563465808 / 7604556960000
...
c
c       69028763155644023 / 2688996956405760000
c      603652082270808125 / 2688996956405760000
c    - 926840515700222955 / 2688996956405760000
c     4301581538450500095 / 2688996956405760000
c  - 10343692234243192788 / 2688996956405760000
c    22336420328479961316 / 2688996956405760000
c  - 35331888421114781580 / 2688996956405760000
c    43920768370565135580 / 2688996956405760000
c  - 37088370261379851390 / 2688996956405760000
c    15148337305921759574 / 2688996956405760000
c    15148337305921759574 / 2688996956405760000
c  - 37088370261379851390 / 2688996956405760000
c    43920768370565135580 / 2688996956405760000
c  - 35331888421114781580 / 2688996956405760000
...
c*********************************************************************72
c
cc NCO_COMPUTE computes a Newton-Cotes Open quadrature rule.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= I <= N ) W(I) * F ( X(I) ).
c
c    For the OPEN rule, the abscissas do not include the end points.
c
c    For this common type of Newton-Cotes Open rule, the abscissas for
c    rule N are found by computing the abscissas for the closed rule of
c    order N+2 and dropping the two endpoints.
c
c    (An alternative "open half" rule of order N can be defined, which divides
c    the interval into N equal subintervals, and uses the midpoint
c    of each subinterval as the abscissa.)
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc NCO_COMPUTE_POINTS: points for a Newton-Cotes Open quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc NCO_COMPUTE_WEIGHTS: weights for a Newton-Cotes Open quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc NCO_SET sets abscissas and weights for open Newton-Cotes quadrature.
c
c  Discussion:
c
c    The open Newton-Cotes rules use equally spaced abscissas, and
c    hence may be used with equally spaced data.
c
c    The rules are called "open" because the abscissas do not include
c    the interval endpoints.
c
c    For this common type of open Newton-Cotes rule, the abscissas for
c    rule N are found by computing the abscissas for the closed rule of
c    order N+2 and dropping the two endpoints.
c
c    (An alternative "open half" rule of order N can be defined, which divides
...
c    of each subinterval as the abscissa.)
c
c    Most of the rules involve negative weights.  These can produce loss
c    of accuracy due to the subtraction of large, nearly equal quantities.
c
c    The integral:
c
c      Integral ( -1 <= X <= 1 ) F(X) dx
...
c
c    Input, integer N, the order.
c    N must be between 1 and 7, and 9.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc NCOH_COMPUTE computes a Newton-Cotes Open Half quadrature rule.
c
c  Discussion:
c
c    The input value N is used to define N equal subintervals of [-1,+1].
c    The I-th abscissa is the center of the I-th subinterval.
c
c    The integral:
c
c      Integral ( X_MIN .le. X .le. X_MAX ) F(X) dx
c
c    The quadrature rule:
c
c      Sum ( 1 .le. I .le. N ) W(I) * F ( X(I) ).
...
c*********************************************************************72
c
cc NCOH_COMPUTE_POINTS: points for a Newton-Cotes Open Half quadrature rule.
c
c  Discussion:
c
c    The input value N is used to define N equal subintervals of [-1,+1].
...
c*********************************************************************72
c
cc NCOH_COMPUTE_WEIGHTS: weights for a Newton-Cotes Open Half quadrature rule.
c
c  Discussion:
c
c    The input value N is used to define N equal subintervals of [-1,+1].
...
c*********************************************************************72
c
cc NCOH_SET sets abscissas and weights for Newton-Cotes "open half" quadrature.
c
c  Discussion:
c
c    The Newton-Cotes rules use equally spaced abscissas, and
c    hence may be used with equally spaced data.
c
c    The rules are called "open" because the abscissas do not include
c    the interval endpoints.
c
c    For this uncommon type of Newton-Cotes open rule, the abscissas for
c    rule N are found by dividing the interval into N equal subintervals,
c    and using the midpoint of each subinterval as the abscissa.
c
c    Most of the rules involve negative weights.  These can produce loss
c    of accuracy due to the subtraction of large, nearly equal quantities.
c
...
c      Sum ( 1 <= I <= N ) W(I) * F ( X(I) )
c
c    In Mathematica, the Newton-Cotes "open half" weights and abscissas
c    can be computed by the commands:
c
c      Needs["NumericalDifferentialEquationAnalysis`"]
c      NewtonCotesWeights [ n, -1, 1, QuadratureType -> Open ]
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c*********************************************************************72
c
cc PATTERSON_SET sets abscissas and weights for Gauss-Patterson quadrature.
c
c  Discussion:
c
c    The integral:
...
c      sum ( 1 <= i <= n ) w(i) * f ( x(i) )
c
c    The zeroth rule, of order 1, is the standard Gauss-Legendre rule.
c
c    The first rule, of order 3, is the standard Gauss-Legendre rule.
c
c    The second rule, of order 7, includes the abscissas of the previous
c    rule.
c
c    Rules are available of orders 1, 3, 7, 15, 31, 63, 127, 255, and 511.
c
c    These rules constitute a nested family.  The rules can integrate exactly
c    any polynomial of degree 1, 5, 11, 23, 47, 95, 191, 383 or 767, 
c    respectively.
c
c    The data for N = 511 was supplied by Dirk Laurie, and is derived
c    from a NAG Library function d01arf.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    LC: QA299.3.K98.
c
c    NAG Library Documentation,
c    D01ARF,
c    The Numerical Algorithms Group.
c
c    Thomas Patterson,
c    The Optimal Addition of Points to Quadrature Formulae,
c    Mathematics of Computation,
...
c
c    Input, integer N, the order.
c    N must be 1, 3, 7, 15, 31, 63, 127, 255 or 511.
c
c    Output, double precision X(N), the abscissas.
c
c    Output, double precision W(N), the weights.
...
c*********************************************************************72
c
cc RADAU_COMPUTE computes a Radau quadrature rule.
c
c  Discussion:
c
c    The Radau rule is distinguished by the fact that the left endpoint
c    (-1) is always an abscissa.
c
c    The integral:
c
c      Integral ( -1 .le. X .le. 1 ) F(X) dx
...
c    LC: QA377.S676.
c
c    Francis Hildebrand,
c    Section 8.11,
c    Introduction to Numerical Analysis,
c    Dover, 1987,
c    ISBN13: 978-0486653631,
c    LC: QA300.H5.
c
c    Arthur Stroud, Don Secrest,
c    Gaussian Quadrature Formulas,
...
c
c  Initial estimate for the abscissas is the Chebyshev-Gauss-Radau nodes.
c
      do i = 1, n
        x(i) = - cos ( 2.0D+00 * r8_pi * dble (     i - 1 ) 
     &                                 / dble ( 2 * n - 1 ) )
      end do
...
10    continue
     ...
     &                 / dble (     j )
        ...
        do i = 2, n
          x(i) = xold(i) - ( ( 1.0D+00 - xold(i) ) / dble ( n ) ) 
     &      * ( p(i,n) + p(i,n+1) ) / ( p(i,n) - p(i,n+1) )
        end do
      ...
      w(1) = 2.0D+00 / dble ( n * n )
      do i = 2, n
        w(i) = ( 1.0D+00 - x(i) ) / ( dble ( n ) * p(i,n) )**2
      end do
...
c*********************************************************************72
c
cc RADAU_SET sets abscissas and weights for Radau quadrature.
c
c  Discussion:
c
c    The Radau rule is distinguished by the fact that the left endpoint
...
c
c    If the weight function W(X) is not 1, then the weight vector W will
...
c*********************************************************************72
c
cc SUM_SUB carries out a composite quadrature rule.
c
c  Discussion:
c
c    SUM_SUB assumes the original rule was written for [XLO,XHI].
c
c    The integral:
c
c      Integral ( A .le. X .le. B ) F(X) dx
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, external FUNC, the name of the function which
c    evaluates the integrand.  The function must have the form
c      function func ( x ).
c
c    Input, double precision A, B, the lower and upper limits of integration.
c
c    Input, integer NSUB, the number of equal subintervals into
c    which the finite interval (A,B) is to be subdivided for
c    higher accuracy.  NSUB must be at least 1.
c
c    Input, integer N, the order.
c    N must be at least 1.
c
c    Input, double precision XLO, XHI, the left and right endpoints of
c    the interval over which the quadrature rule was defined.
c
c    Input, double precision X(N), the abscissas of a quadrature
c    rule for the interval [XLO,XHI].
c
c    Input, double precision W(N), the weights.
c
c    Output, double precision RESULT, the approximate value of the integral.
...
c*********************************************************************72
c
cc SUM_SUB_GK carries out a composite Gauss-Kronrod rule.
c
c  Discussion:
c
c    The integral:
...
c
c      H = ( B - A ) / NSUB
c      XMID(J) = A + 0.5 * H * ( 2 * J - 1 )
c
c      Sum ( 1 <= J <= NSUB )
c        Sum ( 1 <= I <= NK )
c          WK(I) * F ( XMID(J) + 0.5 * H * XK(I) )
c
c    The orders of the Gauss-Legendre and Kronrod rules must satisfy
c    NK = 2 * NG + 1.
c
c    The Kronrod rule uses the abscissas of the Gauss-Legendre rule,
c    plus more points, resulting in an efficient and higher order estimate.
c
c    The difference between the Gauss-Legendre and Kronrod estimates
c    is taken as an estimate of the error in the approximation to the
c    integral.
c
c  Licensing:
c
...
c    higher accuracy.  NSUB must be at least 1.
c
c    Input, integer NG, the order of the Gauss-Legendre rule.
c    NG must be at least 1.
c
c    Input, double precision WG(NG), the weights of the
c    Gauss-Legendre rule.
c
c    Output, double precision RESULTG, the approximate value of the
c    integral based on the Gauss-Legendre rule.
c
c    Input, integer NK, the order of the Kronrod rule.
c    NK must be at least 1.
c
c    Input, double precision XK(NK), the abscissas of the
c    Kronrod rule.
c
c    Input, double precision WK(NK), the weights of the
c    Kronrod rule.
c
c    Output, double precision RESULTK, the approximate value of the
c    integral based on the Kronrod rule.
c
c    Output, double precision ERROR, an estimate of the approximation
c    error.  This is computed by taking the sum of the absolute values of
c    the differences between the Gauss-Legendre and Kronrod rules
c    over each subinterval.  This is usually a good estimate of
c    the error in the value RESULTG.  The error in the Kronrod
c    estimate RESULTK is usually much smaller.
c
      implicit none
      ...
      double precision wk(nk)
      double precision xk(nk)
...
c*********************************************************************72
c
cc SUMMER carries out a quadrature rule over a single interval.
c
c  Formula:
c
c    RESULT = sum ( 1 <= I <= N ) W(I) * FUNC ( X(I) )
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      subroutine summer_gk ( func, ng, wg, resultg, nk, xk, wk, 
     &  resultk )

c*********************************************************************72
c
cc SUMMER_GK carries out Gauss-Kronrod quadrature over a single interval.
c
c  Discussion:
c
c    The abscissas for the Gauss-Legendre rule of order NG are
c    not required, since they are assumed to be the even-indexed
c    entries of the corresponding Kronrod rule.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    integral, based on the Gauss-Legendre rule.
c
c    Input, integer NK, the order of the Kronrod rule.  NK
c    must equal 2 * NG + 1.
c
c    Input, double precision XK(NK), the abscissas of the Kronrod rule.
c
c    Input, double precision WK(NK), the weights of the Kronrod rule.
c
c    Output, double precision RESULTK, the approximate value of the integral,
c    based on the Kronrod rule.
c
      implicit none
