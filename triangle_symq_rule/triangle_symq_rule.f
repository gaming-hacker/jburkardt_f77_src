...
c*********************************************************************72
c
cc KJACOPOLS evaluates Jacobi polynomials.
c
c  Discussion:
c
c    This routine evaluates Jacobi polynomials.
c
c  Licensing:
c
...
c
c    Input, double precision A, B, the parameter values.
c
...
c*********************************************************************72
c
cc KJACOPOLS2 evaluates Jacobi polynomials and derivatives.
c
c  Discussion:
c
c    This routine evaluates Jacobi polynomials and  derivatives.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c    Output, double precision POLS(N+1), the polynomial values.
c
c    Output, double precision DERS(N+1), the polynomial derivative values.
c
      implicit none
      ...
      double precision b
      double precision beta
      double precision ders(n+1)
      double precision dk
      double precision dkm1
      double precision dkp1
...
c*********************************************************************72
c
cc KLEGEYPOLS3 evaluate scaled Legendre polynomials and derivatives.
c
c  Discussion:
c
c    This routine evaluates a sequence of scaled Legendre polynomials
c    P_n(x/y) y^n, with the parameter y in [0,1], together with their
c    derivatives with respect to the parameters x and y.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c    Input, double precision X, the evaluation point.
c
c    Input, double precision Y, the parameter value.
c
c    Input, integer N, the highest degree to be evaluated.
c
c    Output, double precision POLS(N+1), the polynomial values.
c
c    Output, double precision DERSX(N+1), the derivatives with respect to X.
c
c    Output, double precision DERSY(N+1), the derivatives with respect to Y.
c
      implicit none

      integer n

      double precision dersx(n+1)
      double precision dersy(n+1)
      double precision dk
      double precision dkm1
      double precision dkp1
      integer k
      ...
      double precision y
      double precision yk
      double precision ykm1
      double precision ykp1
...
c*********************************************************************72
c
cc ORTHO2EVA0 evaluates the orthonormal polynomials on the triangle.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c  Parameters:
c
c    Input, integer MMAX, the maximum order to which the polynomials are
c    to be evaluated.
c
c    Input, double precision Z(2), the coordinates of the evaluation point.
c
c    Output, double precision POLS((mmax+1)*(mmax+2)/2), the orthogonal
c    polynomials evaluated at the point Z.
c
c    Workspace, double precision F1(*), F2(*).
c
      implicit none
      ...
      r12 = -1.0D+00 / sqrt3
      r21 = - 1.0D+00 / 3.0D+00
      r22 = 1.0D+00 / 9.0D+00 * ( 6.0D+00 ) * sqrt3

      a = z(1)
      b = z(2)
c
c  Map the reference triangle to the right
c  triangle with the vertices (-1,-1), (1,-1), (-1,1)
c
      x = r11 + r12 * b + a
      y = r21 + r22 * b
c
c  Evaluate the Koornwinder's polynomials via the three term recursion.
c
...
c
     ...
     &      * ( 1 + ( m - n ) + ( m - n ) ) ) / sqrt3 )
      ...
      subroutine ortho2eva30 ( mmax, z, pols, dersx, dersy, f1, f2,
     &  f3, f4, f5, f6 )

c*********************************************************************72
c
cc ORTHO2EVA30: orthonormal polynomials and derivatives on triangle.
c
c  Discussion:
c
...
c    polynomials evaluated at the point Z.
c
c    Output, double precision DERSX((mmax+1)*(mmax+2)/2), the derivatives
c    with respect to X of the polynomials returned in POLS.
c
c    Output, double precision DERSY((mmax+1)*(mmax+2)/2), the derivatives
c    with respect to Y of the polynomials returned in POLS.
c
c    Workspace, double precision F1(*), F2(*), F3(*), F4(*), F5(*), F6(*).
c
      implicit none

      double precision a
      double precision b
      double precision dersx(*)
      double precision dersy(*)
      double precision f1(*)
      double precision f2(*)
      double precision f3(*)
      double precision f4(*)
...
c*********************************************************************72
c
cc ORTHO2EVA3: orthogonal polynomial values and derivatives, reference triangle.
c
c  Discussion:
c
c     This routine evaluates at the user-supplied point Z
c     a collection of polynomials (of X,Y) orthogonal on the
c     reference triangle, together with their derivatives with
c     respect to X and Y.
c
c     The reference triangle has vertices
c       (0,2/sqrt(3)), (-1,-1/sqrt(3), (1,-1/sqrt(3)).
c
c     The polynomials evaluated by this routine are all
c     orthogonal polynomials up to order mmax, arranged in the
c     increasing order.
c
c     This routine is based on the Koornwinder's representation
...
c    to be evaluated.
c
c    Input, double precision Z(2), the location where the polynomials are
c    to be evaluated; normally, expected to be inside (including boundary) the
...
c
c    Output, double precision DERSX((mmax+1)*(mmax+2)/2), the derivatives
c    with respect to X of the polynomials returned in array POLS.
c
c    Output, double precision DERSY((mmax+1)*(mmax+2)/2), the derivatives
c    with respect to Y of the polynomials returned in array POLS.
c
c    Workspace, double precision W(mmax+1+(mmax+1)**2).
c
      implicit none
...
c*********************************************************************72
c
cc ORTHO2EVA evaluates orthogonal polynomials on the reference triangle.
c
c  Discussion:
c
c    This routine evaluates at the user-supplied point Z
...
c    reference triangle.
c
c    The reference triangle has the vertices
c      (0,2/sqrt(3)), (-1,-1/sqrt(3)), (1,-1/sqrt(3))
c
c    The polynomials evaluated by this routine are all
c    orthogonal polynomials up to order mmax, arranged in the
...
c  Parameters:
c
c    Input, integer MMAX, the maximum order to which the polynomials
c    are to be evaluated;
c
c    Input, double precision Z(2), the location where the polynomials are
c    to be evaluated; normally, expected to be inside (including boundary)
...
c*********************************************************************72
c
cc QUAECOPY copies a quadrature rule into user arrays Z and W.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c  Parameters:
c
c    Input, double precision XS(KK), YS(KK), the point coordinates to copy.
c
c    Input, double precision WS(KK), the weights to copy.
c
c    Output, double precision Z(2,KK), the copied point coordinates.
c
c    Output, double precision W(KK), the copied weights.
c
c    Input, integer KK, the number of values to copy.
c
      implicit none
      ...
      integer i
      double precision w(kk)
      double precision ws(kk)
      double precision xs(kk)
      double precision ys(kk)
      double precision z(2,kk)
...
c*********************************************************************72
c
cc QUAECOPY2 copies a quadrature rule into a user arrays X, Y, and W.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c    Input, double precision WS(KK), the weights to copy.
c
c    Output, double precision XNEW(KK), YNEW(KK), the copied point coordinates.
c
c    Output, double precision W(KK), the copied weights.
c
c    Input, integer KK, the number of values to copy.
c
      ...
      double precision w(kk)
      double precision ws(kk)
      double precision xnew(kk)
      double precision xs(kk)
      double precision ynew(kk)
      double precision ys(kk)
...
c*********************************************************************72
c
cc QUAEINSIDE checks whether a point is inside a triangle.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c  Parameters:
c
c    Input, integer IITYPE, indicates the check to perform.
c    * 0 check whether the point is inside the whole triangle
c    * 1 check whether the point is inside the bottom 1/3 of the triangle
c    * 2 check whether the point is inside the lower-left 1/6 of the triangle
c
c    Input, double precision XSOUT, YSOUT, the coordinates of the point.
c
c    Output, integer NBOOL.
c    * 1, the point is inside.
c    * 2, the point is outside.
c
      implicit none
...
c
      if ( iitype .eq. 2 ) then
        ...
        if ( xsout .lt. -1.0D+00 .or. 0.0D+00 .lt. xsout ) then
          nbool = 0
        end if
c
c  hx
c
        if ( ysout .lt. -1.0D+00 / s - 1.0D-30
     &    .or. xsout / s .lt. ysout ) then
          nbool = 0
        end if
c
c  The 1/3 of triangle.
c
      else if ( iitype .eq. 1 ) then

        nbool = 0

        if ( xsout .le. 0.0D+00 .and. 
     &       -1.0D+00 .le. xsout .and. 
     &       -1.0D+00 / s .le. ysout .and.
     &       ysout .le. xsout / s ) then
          nbool = 1
        end if

        if ( iitype .eq. 1 ) then

          if  ( 0.0D+00 .le. xsout .and. 
     &          xsout .le. 1.0D+00 .and. 
     &          -1.0D+00 / s .le. ysout .and. 
     &          ysout .le. - xsout / s ) then
            nbool = 1
          end if
...
c
c  The entire triangle.
c
      else if ( iitype .eq. 0 ) then

        nbool = 1

        if ( ysout .lt. -1.0D+00 / s ) then
          nbool = 0
        end if

        if ( s * xsout + 2.0D+00 / s .lt. ysout ) then
          nbool = 0
        end if
...
c*********************************************************************72
c
cc QUAENODES2 expands nodes from 1/6 to 1/3 of the triangle.
c
c  Discussion:
c
c    This routine only expands to 1/3 of the triangle, assuming the points are
...
c  Parameters:
c
c    Input, integer NPTSOUT, the number of points expanded.
c
c    Input, double precision XSOUT(NPTSOUT), YSOUT(NPTSOUT), WSOUT(NPTSOUT),
c    the coordinates and weights of the points to be expanded.
c
c    Output, integer NPTSOUTPUT, the number of points in the expanded set.
c
c    Output, double precision XS2(NPTSOUTOUT), YS2(NPTSOUTOUT), WS2(NPTSOUTOUT),
c    the coordinates and weights of the expanded set of points.
c
      implicit none
      ...
      integer i
      integer ntot
      double precision ws2(*)
      double precision wsout(nptsout)
      double precision xs2(*)
      double precision xsout(nptsout)
      double precision ys2(*)
      double precision ysout(nptsout)
...
c*********************************************************************72
c
cc QUAENODES expands nodes to the reference triangle.
c
c  Discussion:
c
c    This routine expands nodes to the reference triangle
c    assuming that the points are already in the lower-left 1/6 of the
c    triangle.
c
c  Licensing:
c
...
c*********************************************************************72
c
cc QUAEQUAD returns a symmetric quadrature formula for a reference triangle.
c
c  Discussion:
c
c    This routine constructs (or rather, retrieves)
...
c  Parameters:
c
c    Input, integer ITYPE, the configuration of the quadrature nodes.
c    * 0, all nodes on the reference triangle
c    * 1, quadrature nodes on the bottom 1/3 of the reference triangle
c    * 2, quadrature nodes on lower-left 1/6 of the reference triangle
c
c    Input, integer MMAX, the degree of the quadrature (the maximum degree of
c    the polynomials of two variables that are integrated
c    exactly. 1 <= MMAX <= 50.
c
c    Output, double precision ZS(2,NUMNODES), the nodes.
c
c    Output, double precision WHTS(NUMNODES), the weights.
c
c    Input, integer NUMNODES, the number of nodes in the quadrature rule.
c
      implicit none
      ...
      integer mmax
      integer nptsout
      double precision whts(numnodes)
      double precision wsout(numnodes)
      double precision xnew(numnodes)
      double precision xsout(numnodes)
      double precision ynew(numnodes)
      double precision ysout(numnodes)
      double precision zs(2,numnodes)

      if ( mmax .lt. 1 .or. 50 .lt. mmax ) then
        write ( *, '(a)' ) ''
      ...
      if ( itype .lt. 0 .or. 2 .lt. itype ) then
        write ( *, '(a)' ) ''
        write ( *, '(a)' ) 'QUAEQUAD - Fatal error!'
...
c
c  Retrieve the compressed rule.
c
...
c
     ...
     &    ysout, wsout )
        ...
        call quaecopy ( xsout, ysout, wsout, zs, whts, nptsout )
c
c  Simply copy the nodes; they are already in the lower-left
...
c*********************************************************************72
c
cc QUAEQUAD0 returns the requested quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c    exactly.  1 <= MMAX <= 50.
c
c    Input, integer KK, the number of points in the compressed rule.
c
c    Output, double precision XNEW(KK), YNEW(KK), the
c    coordinates of the nodes.
c
c    Output, double precision W(KK), the weights.
c
      implicit none
...
c
        ...
        else if ( nbool1 .eq. 1 ) then
          xnew(i) = -xnew(i)
          ynew(i) = ynew(i)
        else if ( nbool0 .eq. 1 ) then
          x0 = xnew(i)
          y0 = ynew(i)
          call quaerotate ( x0, y0, x1, y1 )
          xnew(i) = x1
...
c*********************************************************************72
c
cc QUAEROTATE applies a rotation.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c  Parameters:
c
c    Input, double precision XIN, YIN, the coordinates of the point.
c
c    Output, double precision XOUT, YOUT, the coordinates of the point
c    after rotation.
c
      implicit none
...
c*********************************************************************72
c
cc REF_TO_KOORN maps points from the reference to Koornwinder's triangle.
c
c  Discussion:
c
c    The reference triangle has vertices:
...
c      (  0, +2/sqrt(3) )
c
c    Koornwinder's triangle has vertices:
c
c      ( -1, -1 )
c      ( +1, -1 )
c      ( -1, +1 )
...
c  Parameters:
c
c    Input, double precision R(2), the coordinates of a point in the
c    reference triangle.
c
c    Output, double precision U(2), the coordinates of the point in
c    the Koornwinder triangle.
c
      implicit none
      ...
      a10 = -1.0D+00 / 3.0D+00
      a11 =  1.0D+00
      a12 = -1.0D+00 / dsqrt ( 3.0D+00 )
 
      a20 = - 1.0D+00 / 3.0D+00
      a21 =   0.0D+00
      a22 =   2.0D+00 * dsqrt ( 3.0D+00 ) / 3.0D+00
...
c*********************************************************************72
c
cc REF_TO_TRIANGLE maps points from the reference triangle to a triangle.
c
c  Discussion:
c
c    The reference triangle has vertices:
...
c  Parameters:
c
c    Input, double precision TVERT1(2), TVERT2(2), TVERT3(2), the coordinates
c    of the vertices of the triangle.  These vertices will be taken
c    to be the images of (0,0), (1,0) and (0,1) respectively.
c
c    Input, double precision R(2), the coordinates of a point in the
c    reference triangle.
c
c    Output, double precision T(2), the coordinates of the point in
c    the triangle.
c
      implicit none
      ...
      double precision rvert1(2)
      double precision rvert2(2)
      double precision rvert3(2)
      double precision tvert1(2)
      double precision tvert2(2)
      double precision tvert3(2)
...
c*********************************************************************72
c
cc RULE_COMPRESSED_SIZE returns the compressed size of the requested quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c    exactly.  1 <= MMAX <= 50.
c
c    Output, integer NPTS, the number of nodes in the compressed rule.
c
      implicit none
     ...
     &  73, 75, 78, 82, 84 /
      ...
      if ( 1 .le. mmax .and. mmax .le. 50 ) then
        npts = nnodes(mmax)
      else
        write ( *, '(a)' ) ''
...
c*********************************************************************72
c
cc RULE_FULL_SIZE returns the full size of the requested quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c    exactly.  1 <= MMAX <= 50.
c
c    Output, integer NPTS, the number of nodes in the full rule.
c
      implicit none
...
c  Parameters:
c
c    Output, double precision X(*), Y(*), the coordinates of the nodes.
c
c    Output, double precision W(*), the weights.
c
      implicit none
     ...
     &     0.21934566882541541013653648363283D+00/
      ...
      call r8vec_copy ( n, xs, x )
      call r8vec_copy ( n, ys, y )
...
c*********************************************************************72
c
cc RULE27 returns the rule of degree 29.
c
c  Discussion:
c
c    Order 27 (141 pts)
...
c*********************************************************************72
c
cc RULE31 returns the rule of degree 31.
c
c  Discussion:
c
c    Order 31 (181 pts)
...
c*********************************************************************72
c
cc RULE32 returns the rule of degree 32.
c
c  Discussion:
c
c    Order 32 (193 pts)
...
c*********************************************************************72
c
cc RULE33 returns the rule of degree 33.
c
c  Discussion:
c
c    Order 33 (204 pts)
...
c*********************************************************************72
c
cc RULE34 returns the rule of degree 34.
c
c  Discussion:
c
c    Order 34 (214 pts)
...
c*********************************************************************72
c
cc RULE35 returns the rule of degree 35.
c
c  Discussion:
c
c    Order 35 (228 pts)
...
c*********************************************************************72
c
cc RULE36 returns the rule of degree 36.
c
c  Discussion:
c
c    Order 36 (243 pts)
...
c*********************************************************************72
c
cc RULE37 returns the rule of degree 37.
c
c  Discussion:
c
c    Order 37 (252 pts)
...
c*********************************************************************72
c
cc RULE38 returns the rule of degree 38.
c
c  Discussion:
c
c    Order 38 (267 pts)
...
c*********************************************************************72
c
cc RULE39 returns the rule of degree 39.
c
c  Discussion:
c
c    Order 39 (282 pts)
...
c*********************************************************************72
c
cc RULE40 returns the rule of degree 40.
c
c  Discussion:
c
c    Order 40 (295 pts)
...
c*********************************************************************72
c
cc RULE41 returns the rule of degree 41.
c
c  Discussion:
c
c    Order 41 (309 pts)
...
c*********************************************************************72
c
cc RULE42 returns the rule of degree 42.
c
c  Discussion:
c
c    Order 42 (324 pts)
...
c*********************************************************************72
c
cc RULE43 returns the rule of degree 43.
c
c  Discussion:
c
c    Order 43 (339 pts)
...
c*********************************************************************72
c
cc RULE44 returns the rule of degree 44.
c
c  Discussion:
c
c    Order 44 (354 pts)
...
c*********************************************************************72
c
cc RULE45 returns the rule of degree 45.
c
c  Discussion:
c
c    Order 45 (370 pts)
...
c*********************************************************************72
c
cc RULE46 returns the rule of degree 46.
c
c  Discussion:
c
c    Order 46 (385 pts)
...
c*********************************************************************72
c
cc RULE47 returns the rule of degree 47.
c
c  Discussion:
c
c    Order 47 (399 pts)
...
c*********************************************************************72
c
cc RULE48 returns the rule of degree 48.
c
c  Discussion:
c
c    Order 48 (423 pts)
...
c*********************************************************************72
c
cc RULE49 returns the rule of degree 49.
c
c  Discussion:
c
c    Order 49 (435 pts)
...
c*********************************************************************72
c
cc RULE50 returns the rule of degree 50.
c
c  Discussion:
c
c    Order 50 (453 pts)
...
c*********************************************************************72
c
cc SIMPLEX_TO_TRIANGLE maps points from the simplex to a triangle.
c
c  Discussion:
c
c    The simplex has vertices:
c
c      (  0, 0 )
c      (  1, 0 )
c      (  0, 1 )
...
c    to be the images of (0,0), (1,0) and (0,1) respectively.
c
c    Input, double precision S(2), the coordinates of the point in the simplex.
c
c    Output, double precision T(2), the coordinates of the point in
c    the triangle.
c
...
c  Parameters:
c
c    Input, double precision VERT1(2), VERT2(2), VERT3(2), the vertices of
c    the triangle.
c
...
c*********************************************************************72
c
cc TRIANGLE_TO_REF maps points from any triangle to the reference triangle.
c
c  Discussion:
c
c    The reference triangle has vertices:
...
c    to be the images of (0,0), (1,0) and (0,1) respectively.
c
c    Input, double precision T(2), the coordinates of a point in the triangle.
c
c    Output, double precision R(2), the coordinates of a point in the
c    reference triangle.
c
      implicit none
...
c*********************************************************************72
c
cc TRIANGLE_TO_SIMPLEX maps points from any triangle to the simplex.
c
c  Discussion:
c
c    The simplex has vertices:
...
c    Input, double precision T(2), the coordinates of a point in the triangle.
c
c    Output, double precision S(2), the coordinates of the
...
c
     ...
     &       - ( tvert3(1) - tvert1(1) ) * ( tvert2(2) - tvert1(2) ) )
      ...
      subroutine trianmap ( numnodes, vert1, vert2, vert3, rnodes,
     &  whts )

c*********************************************************************72
c
cc TRIANMAP maps rules from the reference triangle to the user triangle.
c
c  Discussion:
c
c    This routine maps the quadrature nodes on the reference
c    triangle into a user-defined triangle specified by its vertices.
c
c    The weights are rescaled accordingly, so that
...
c  Parameters:
c
c    Input, integer NUMNODES, the number of nodes.
c
c    Input, double precision VERT1(2), VERT2(2), VERT3(2), the vertices of
c    the triangle on which the quadrature rule is to be constructed.
c
c    Input/output, double precision RNODES(2,NUMNODES), the nodes.
c
c    Input/output, double precision WHTS(NUMNODES), the weights.
c
      implicit none
...
c*********************************************************************72
c
cc TRIASIMP maps a point from the reference triangle to the simplex.
c
c  Discussion:
c
c    Map the reference triangle with vertices
c      (-1,-1/sqrt(3)), (1,-1/sqrt(3)), (0,2/sqrt(3))
...
c  Parameters:
c
c    Input, double precision X, Y, the coordinates of a point in the
c    reference triangle.
c
c    Output, double precision UOUT, VOUT, the coordinates of the corresponding
c    point in the simplex.
c
      implicit none
     ...
     &  - 0.5D+00 * scale * ( y + scale )
      ...
      subroutine triasymq ( n, vert1, vert2, vert3, rnodes, weights,
     &  numnodes )

c*********************************************************************72
c
cc TRIASYMQ returns a symmetric quadrature formula for a user triangle.
c
c  Discussion:
c
c    This routine constructs (or rather retrieves) a symmetric
c    quadrature formula on the user-defined triangle in the plane
c    specified by its vertices (vert1,vert2,vert3).
c
...
c  Parameters:
c
c    Input, integer N, the degree of the quadrature (must not exceed 50).
c    Note that the total number of nodes to be created is numnodes.
c
c    Input, double precision VERT1(2), VERT2(2), VERT3(2), the vertices of
c    the triangle on which the quadrature rule is to be constructed.
c
c    Output, double precision RNODES(2,NUMNODES), the nodes in the plane
c    (all inside the user-supplied triangle)
c
c    Output, double precision WEIGHTS(NUMNODES), the quadrature weights.
c
c    Input, integer NUMNODES, the number of nodes.
c
      implicit none
      ...
      subroutine triasymq_gnuplot ( vert1, vert2, vert3, numnodes,
     &  rnodes, header )

c*********************************************************************72
c
cc TRIASYMQ_GNUPLOT: set up a GNUPLOT plot of the triangle quadrature rule.
c
c  Licensing:
c
c    This code is distributed under the GNU GPL license.
...
c  Parameters:
c
c    Input, double precision VERT1(2), VERT2(2), VERT3(2), the vertices
c    of the triangle.
c
c    Input, double precision NUMNODES, the number of nodes.
c
c    Input, double precision RNODES(2,NUMNODES), the coordinates of
c    the nodes.
c
