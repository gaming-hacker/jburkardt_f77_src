...
c*****************************************************************************80
c
cc MAIN is the main program for TETRAHEDRON_EXACTNESS.
c
c  Discussion:
c
c    This program investigates the polynomial exactness of a quadrature
c    rule for a tetrahedron.
c
c  Licensing:
c
...
c
     ...
     &  '  R file = "' // trim ( quad_r_filename ) // '".'
      write ( *, '(a,i8)' ) 
     &  '  Maximum total degree to check = ', degree_max
c
c  Read the X header.
c
      ...
      if ( dim_num .ne. 3 ) then
        ...
        write ( *, '(a)' ) 
     &    '  The quadrature abscissas must be 3 dimensional.'
        stop 1
      end if
c
c  Read the W header.
...
c
     ...
     &    '  The quadrature region file should have the'
      ...
      if ( point_num3 .ne. 4 ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'TETRAHEDRON_EXACTNESS - Fatal error!'
        write ( *, '(a)' ) 
     &    '  The quadrature region file should have 4 lines.'
        stop 1
      end if

      call tetrahedron_exactness_sub ( degree_max, point_num,
...
c
      ...
      subroutine tetrahedron_exactness_sub ( degree_max, point_num,
     &  quad_r_filename, quad_w_filename, quad_x_filename )

c*****************************************************************************80
c
cc TETRAHEDRON_EXACTNESS_SUB completes the work of MAIN.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      character * ( 255 ) quad_w_filename
      character * ( 255 ) quad_x_filename
      double precision r(3,4)
      integer t
      double precision volume
      double precision w(point_num)
      double precision x(3,point_num)
      double precision x_ref(3,point_num)
c
c  Read the X file.
c
      call r8mat_data_read ( quad_x_filename, 3, point_num, x )
c
c  Read the W file.
c
...
c  Read the R file.
c
      call r8mat_data_read ( quad_r_filename, 3, 4, r )
c
c  Rescale the weights.
c
      call tetrahedron_volume ( r, volume )

      do j = 1, point_num
        w(j) = ( 1.0D+00 / 6.0D+00 ) * w(j) / volume
      end do
c
c  Translate the abscissas from the reference tetrahedron to
c  the unit tetrahedron.
c
      call tetrahedron_order4_physical_to_reference ( r, point_num, x, 
     &  x_ref )
c
...
10      continue
          ...
          call tet01_monomial_quadrature ( 3, expon, point_num, 
     &      x_ref, w, quad_error )
...
c*****************************************************************************80
c
cc TET01_MONOMIAL_INTEGRAL integrates a monomial over the unit tetrahedron.
c
c  Discussion:
c
...
c   value = value * dble ( i ) / dble ( k )
      ...
      subroutine tet01_monomial_quadrature ( dim_num, expon, point_num, 
     &  x, w, quad_error )

c*****************************************************************************80
c
cc TET01_MONOMIAL_QUADRATURE applies quadrature to a monomial in a tetrahedron.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      subroutine tetrahedron_order4_physical_to_reference ( tetra, n, 
     &  phy, ref )

c*****************************************************************************80
c
cc TETRAHEDRON_ORDER4_PHYSICAL_TO_REFERENCE maps physical to reference points.
c
c  Discussion:
c
...
c  Parameters:
c
c    Input, double precision TETRA(3,4), the coordinates of the vertices.
...
c
      do i = 1, 3
        a(i,1) = tetra(i,2) - tetra(i,1)
        a(i,2) = tetra(i,3) - tetra(i,1)
        a(i,3) = tetra(i,4) - tetra(i,1)
      end do
c
