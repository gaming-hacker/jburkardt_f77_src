...
c*********************************************************************72
c
cc NEWTON_RC solves a small system of nonlinear equations.
c
c  Discussion:
c
c    NEWTON_RC uses Newton's method, with the jacobian approximated via
c    finite differences.
c
c    NEWTON_RC uses "reverse communication".  That is, NEWTON_RC does not call 
c    any user subroutines.  Instead, the user repeatedly calls NEWTON_RC, 
c    and the user and NEWTON_RC communicate via values of the parameter IDO.
c
c    To begin using NEWTON_RC, set IDO=0, set X to an approximate root,
c    set FX(1) to a small positive tolerance, and call NEWTON_RC.
c
c    NEWTON_RC will return with IDO=2.  Evaluate the residual at the
c    point X that is returned by NEWTON_RC, store that value in FX,
c    and call NEWTON_RC back with IDO=2.
c
c    NEWTON_RC will return with IDO=1.  Again, evaluate the residual
c    at the point X returned by NEWTON_RC, store that value in FX<
c    and call NEWTON_RC back with IDO=1.
c
c    This process will be repeated until NEWTON_RC returns with
...
c    residual of the nonlinear equations if NEWTON_RC returned
c    with IDO=1 or 2.
c
...
c
      if ( ido .eq. 0 ) then
        ...
        if ( maxn .lt. n ) then
          ...
          write ( *, '(a,i6)' ) '  Maximum legal N = ', maxn
          ido=-1
          return
        end if
...
c
        ...
        if ( j .lt. n ) then
          ...
          if(info.ne.0)then
            write ( *, '(a)' ) ' '
            write ( *, '(a)' ) 'NEWTON_RC - Fatal error!'
            write ( *, '(a)' ) '  Estimated jacobian is singular.'
            ido=-1
            return
          end if
