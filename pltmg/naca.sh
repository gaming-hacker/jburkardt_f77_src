...
rm compiler.txt
#
gfortran -c naca.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling naca.f"
  exit
fi
rm compiler.txt
#
...
rm compiler.txt
#
gfortran atest.o naca.o mgmpi_stubs.o mgvio_stubs.o xgui_stubs.o \
  -L$HOME/libf77 -lpltmg_double -lrpcsvc
if [ $? -ne 0 ]; then
  echo "Errors linking and loading "
  echo "  atest.o + naca.o + mgmpi_stubs.o + mgvio_stubs.o + xgui_stubs.o"
  exit
fi
rm atest.o
rm naca.o
...
mv naca $HOME/bin
#
echo "The pltmg naca test program has been created."
