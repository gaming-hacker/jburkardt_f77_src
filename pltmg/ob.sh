...
rm compiler.txt
#
gfortran -c ob.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling ob.f"
  exit
fi
rm compiler.txt
#
...
rm compiler.txt
#
gfortran atest.o ob.o mgmpi_stubs.o mgvio_stubs.o xgui_stubs.o \
  -L$HOME/libf77 -lpltmg_double -lrpcsvc
if [ $? -ne 0 ]; then
  echo "Errors linking and loading "
  echo "  atest.o + ob.o + mgmpi_stubs.o + mgvio_stubs.o + xgui_stubs.o"
  exit
fi
rm atest.o
rm ob.o
...
mv ob $HOME/bin
#
echo "The pltmg ob test program has been created."
