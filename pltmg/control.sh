...
rm compiler.txt
#
gfortran -c control.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling control.f"
  exit
fi
rm compiler.txt
#
...
rm compiler.txt
#
gfortran atest.o control.o mgmpi_stubs.o mgvio_stubs.o xgui_stubs.o \
  -L$HOME/libf77 -lpltmg_double -lrpcsvc
if [ $? -ne 0 ]; then
  echo "Errors linking and loading "
  echo "  atest.o + control.o + mgmpi_stubs.o + mgvio_stubs.o + xgui_stubs.o"
  exit
fi
rm atest.o
rm control.o
...
mv control $HOME/bin
#
echo "The pltmg control test program has been created."
