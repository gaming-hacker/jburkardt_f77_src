...
rm compiler.txt
#
gfortran -c burger.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling burger.f"
  exit
fi
rm compiler.txt
#
...
rm compiler.txt
#
gfortran atest.o burger.o mgmpi_stubs.o mgvio_stubs.o xgui_stubs.o \
  -L$HOME/libf77 -lpltmg_double -lrpcsvc
if [ $? -ne 0 ]; then
  echo "Errors linking and loading "
  echo "  atest.o + burger.o + mgmpi_stubs.o + mgvio_stubs.o + xgui_stubs.o"
  exit
fi
rm atest.o
rm burger.o
...
mv burger $HOME/bin
#
echo "The pltmg burger test program has been created."
