...
c*********************************************************************72
c
cc MAIN is the main program for LEGENDRE_EXACTNESS.
c
c  Discussion:
c
c    This program investigates a standard Gauss-Legendre quadrature rule
c    by using it to integrate monomials over [-1,+1], and comparing the
...
c
      ...
      double precision w2(order_max)
      double precision x(order_max)
      double precision x2(order_max)
...
c
c  Generate a second rule for comparison.
c
      if ( order .eq. 1 ) then
        x2(1) = 0.0D+00
        w2(1) = 2.0D+00
        ...
        do i = 1, order - 1
          w2(i) = w2(i) + 1.0D+00 / dble ( order - 1 )
        end do
        do i = 2, order
          w2(i) = w2(i) + 1.0D+00 / dble ( order - 1 )
        end do
...
c
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '  A Gauss-Legendre rule exactly'
      write ( *, '(a,i8)' )
     ...
     &  '         (This rule)                (Trapezoid)'
      ...
      do degree = 0, degree_max
        ...
        call monomial_quadrature_legendre ( degree, order, w, x, 
     &    quad_error )

        call monomial_quadrature_legendre ( degree, order, w2, x2, 
     &    quad_error2 )

        write ( *, '(2x,f24.16,2x,f24.16,2x,i2)' ) 
...
10      continue
      ...
      subroutine legendre_integral ( expon, value )
...
c
      ...
      subroutine monomial_quadrature_legendre ( expon, order,
     &  w, x, quad_error )

c*********************************************************************72
c
cc MONOMIAL_QUADRATURE_LEGENDRE applies a quadrature rule to a monomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c  Get the exact value of the integral of the monomial.
c
