...
c
c    Anton Reinhard corrected the assignments:
c      a2(i) = ( t(i+1) - t(i)   ) / 6.0D+00
c      a3(i) = ( t(i+1) - t(i-1) ) / 3.0D+00
c      a4(i) = ( t(i)   - t(i-1) ) / 6.0D+00
c    to
c      a2(i) = ( t(i)   - t(i-1) ) / 6.0D+00
c      a3(i) = ( t(i+1) - t(i-1) ) / 3.0D+00
c      a4(i) = ( t(i+1) - t(i)   ) / 6.0D+00
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    3: Not-a-knot: the third derivative is continuous at T(2).
c
...
c    3: Not-a-knot: the third derivative is continuous at T(N-1).
c
...
c    Input, integer N, the order of the matrix.
c
c    Input, double precision A1(N), A2(N), A3(N), A4(N), A5(N), the nonzero
