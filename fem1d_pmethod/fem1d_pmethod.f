...
c  Local Parameters:
c
c    Local, double precision A(0:NP), the squares of the norms of the 
c    basis functions.
c
c    Local, double precision ALPHA(NP), BETA(NP), the basis function 
!    recurrence coefficients.
c
c    Local, double precision F(0:NP).
...
c
c    Local, integer NP, the highest degree polynomial to use.
c
c    Local, integer NPRINT, the number of printout points.
c
c    Local, integer NQUAD, the number of points in the quadrature rule.
c
c    Local, integer PROBLEM, indicates the problem being solved.
c    1, U=1-x**4, P=1, Q=1, F=1.0+12.0*x**2-x**4.
c    2, U=cos(0.5*pi*x), P=1, Q=0, F=0.25*pi*pi*cos(0.5*pi*x).
c
c    Local, double precision WQUAD(NQUAD), the quadrature weights.
c
c    Local, double precision XQUAD(NQUAD), the quadrature abscissas.
c
      implicit none
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a,i4)' ) '  Highest degree polynomial to use is ', np
      write ( *, '(a,i4)' ) '  Number of output points = ', nprint
...
c    basis functions.
c
c    Output, double precision ALPHA(NP), BETA(NP), the basis function 
!    recurrence coefficients.
c
     ...
     &    + x * qq ( x, problem ) * ( 1.0D+00 - x * x )**2
      ...
      do i = 2, np + 1
        ...
        do iq = 1, nquad
          ...
          x = xquad(iq)
c
c  Three term recurrence for Q and Q'.
c
          qm1 = 0.0D+00
          q = 1.0D+00
          qm1x = 0.0D+00
          ...
          t = 1.0D+00 - x**2
c
c  The basis function PHI = ( 1 - x^2 ) * q.
c
c     s = pp * ( phi(i) )' * ( phi(i) )' + qq * phi(i) * phi(i)
c
          s = pp ( x, problem ) * ( t * qx - 2.0D+00 * x * q )**2 
     &      + qq ( x, problem ) * ( t * q )**2
c
c     u = pp * ( x * phi(i) )' * phi(i)' + qq * x * phi(i) * phi(i)
c
          u = pp ( x, problem ) 
...
c
c     v = pp * ( x * phi(i) )' * phi(i-1) + qq * x * phi(i) * phi(i-1)
c
          v = pp ( x, problem ) 
     &      * ( x * t * qx + ( 1.0D+00 - 3.0D+00 * x**2 ) * q ) 
...
c
c  SS(i) = <   phi(i), phi(i)   > = integral ( S )
c  SU(i) = < x phi(i), phi(i)   > = integral ( U )
c  SV(i) = < x phi(i), phi(i-1) > = integral ( V )
c
          ss = ss + s * wquad(iq)
          su = su + u * wquad(iq)
          sv = sv + v * wquad(iq)
...
c  Parameters:
c
c    Input, double precision ALPHA(NP), BETA(NP), the basis function 
!    recurrence coefficients.
c
...
c
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '    X        U computed    U exact     Error'
      write ( *, '(a)' ) ' '
      el2 = 0.0D+00
      ...
      el2 = sqrt ( el2 )
      write ( *, '(a)' ) ' '
      write ( *, '(a,g14.6)' ) '  Little L2 error =', el2
      write ( *, '(a)' ) ' '
