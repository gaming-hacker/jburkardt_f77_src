...
c*********************************************************************72
c
cc TRANSF transforms between constrained and unconstrained problems.
c
c  Discussion:
c
c    The routine implements the change of variables for transforming 
c    unconstrained problems into constrained ones and vice versa.
c
c  Reference:
c
c    J Dennis, N Echebest, M Guardarucci, J Martinez, H Scolnik, C Vacchino,
c    A curvilinear search using tridiagonal secant updates for unconstrained optimization,
c    SIAM Journal on Optimization,
c    Volume 1, Number 3, pages 333-357, August 1991.
c
c  Parameters:
c
c     Input:
...
c
      ...
      if ( mar == 1 ) then
c
c  Analyze if the user-supplied data is coherent.
c
...
c
        do i = 1, n
          ...
          if ( jbound(i) == 3 ) then
            wa2 = dabs ( bu(i) - bl(i) )
            wa3 = tol * dmax1 ( 0.1D+00, dabs ( bl(i) ) )
            if ( wa3 < wa2 ) then
...
c*********************************************************************72
c
cc TRANSF2 changes the current iterate if the unconstrained
...
c        nfu: the number of function evaluations.
c
c   Output :
...
c
      do i=1,n
        ...
        if(gp(i).gt.0.d0)then
          if(jbound(i) == 3.or.jbound(i) == 2)then
            ci=bl(i)+1.d1*dmax1(dabs(bl(i)),1.d0)
            bet=0.9d0
            if(jbound(i) == 3)ci=bu(i)
        ...
        if(gp(i).lt.0.d0) then
          if(jbound(i) == 3.or.jbound(i) == 1)then
            ci=bu(i)-1.d1*dmax1(dabs(bu(i)),1.d0)
            bet=0.9d0
            if(jbound(i) == 3)ci=bl(i)
...
c  Compute x, iterate in the unconstrained problem corresponding to y.    
c
      do i=1,n
...
c*********************************************************************72
c
cc GRADX computes the gradient of the transformed problem.
c
c  Parameters:
c
c     Input :
...
c*********************************************************************72
c
cc HESSIX computes the Hessian of the transformed problem.
c
c  Parameters:
c
c    Input :
...
c*********************************************************************72
c
cc FACTOR computes the Householder decomposition of the Hessian matrix.
c
c  Discussion:
c
c    The Hessian matrix is decomposed as H = Q' * T * Q.
c
c  Parameters:
c
c     Input:
...
c*********************************************************************72
c
cc DHOLDER finds the tridiagonal matrix T of
...
c*********************************************************************72
c
cc TRI1 solves a positive definite symmetric tridiagonal linear system.
c
c  Parameters:
c
c    Input :
...
c*********************************************************************72
c
cc MATMUL computes either Ax or A'x where A is a matrix, A' its transpose,
c     and x a vector.
c
c  Parameters:
c
...
c*********************************************************************72
c
cc AUTOVA computes the least eigenvalue of the tridiagonal matrix Tk.
c
...
30    return
      ...
      subroutine funct ( fcn, n, x0, bl, bu, d, e, qt, w, al, f, x1, 
     &  y, qs, wa, ibound, jbound, nfu )

c*********************************************************************72
c
cc FUNCT computes the value of the objective
...
c
      ...
      double precision function ddotsa ( n, x, ix, y, iy )                

c*********************************************************************72
c
cc DDOTSA computes the inner product of vectors sx,sy
c
...
30    mp1=m+1                                                         
      ...
      do i=mp1,n,5                                                   
              ddotsa = ddotsa+x(i)*y(i)+x(i+1)*y(i+1)+x(i+2)*y(i+2)
...
c*********************************************************************72
c
cc DNRM2 computes the euclidean norm of vector x.
c
      implicit none
      ...
      RETURN
      END
      subroutine prntout ( fu, n, x0, fopt, eps, ibound, jbound, bl, 
     &  bu, wa, nfu, ngr, nh, nit, ier, ichoice, flname )
     
c*********************************************************************72
c
cc PRNTOUT prints the results to the file specified in file name
c 
c  Parameters:
c
c    Input : 
...
c                2 failure to converge.
c                3 wrong input in a constrained problem. 
c    ichoice  :  ichoice is 0 or 1.  Choose 0 if it is to be involked before
...
c
      ...
      integer nfu, ngr, nit, ier, ichoice, n 
...
c
      ...
      do i = 1, n
          ...
          else if( jbound(i)  ==  2) then
              write(26, 1013) i, x0(i), bl(i), jbound(i)
              if (wa(i) .lt. bl(i))  wa(i) = bl(i)
              
          else if (jbound(i)  ==  1) then 
              write(26, 1014) i, x0(i), bu(i), jbound(i)
              if (wa(i) .gt. bu(i))  wa(i) = bu(i)
              
          else if (jbound(i)  ==  0) then  
