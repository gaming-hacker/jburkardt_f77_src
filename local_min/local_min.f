...
c  Discussion:
c
c    If the function F is defined on the interval (A,B), then local_min
c    finds an approximation X to the point at which F attatains its minimum
c    (or the appropriate limit point), and returns the value of F at X.
c
c    T and EPS define a tolerance TOL = EPS * abs ( X ) + T.
c    F is never evaluated at two points closer than TOL.  
c
c    If F is delta-unimodal for some delta less than TOL, the X approximates
c    the global minimum of F with an error less than 3*TOL.
c
c    If F is not delta-unimodal, then X may approximate a local, but 
c    perhaps non-global, minimum.
c
...
c    B), then, ignoring rounding errors, convergence is superlinear, and 
c    usually of the order of about 1.3247.
c
c    Thanks to Jonathan Eggleston for pointing out a correction to the 
c    golden section step, 01 July 2013.
c
c    Thanks to Vladimir Kodnyanko for pointing out that the line
c      if ( fu .gt. fv .and. v .ne. x .and. v .ne. w ) go to 20
c    should read
c      if ( fu .gt. fv .and. v .ne. x .and. v .ne. w ) go to 10
c    23 January 2019
c
c  Licensing:
c
...
c
c    double precision EPS, a positive relative error tolerance.
...
c  Output:
c
c    double precision X, the estimated value of an abscissa
c    for which F attains a local minimum value in [A,B].
c
c    integer CALLS, the number of calls to F.
c
c    double precision LOCAL_MIN, the value F(X).
c
      implicit none
...
c
      if ( abs ( x - m ) .le.  t2 - 0.5D+00 * ( sb - sa ) ) then
        go to 190
      end if
...
40    continue
      ...
      if ( p .le. q * ( sa - x ) ) go to 60
      if ( p .ge. q * ( sb - x ) ) go to 60
c
...
90    continue
      ...
      if ( tol .le. abs ( d ) ) then
        u = x + d
      else if ( 0.0D+00 .gt. d ) then
        u = x + tol
      else
        u = x - tol
      end if
...
180   continue
      ...
      if ( ( fu .gt. fv ) .and. ( v .ne. x ) .and. ( v .ne. w ) ) 
     &  go to 10
      v = u
      fv = fu
      go to 10
