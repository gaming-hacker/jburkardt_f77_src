...
c*********************************************************************72
c
cc HYPERCUBE_GRID: grid points over the interior of a hypercube in M dimensions.
c
c  Discussion:
c
c    In M dimensional space, a logically rectangular grid is to be created.
c    In the I-th dimension, the grid will use S(I) points.
c    The total number of grid points is
c      N = product ( 1 <= I <= M ) S(I)
c
...
c    N = product ( 1 <= I <= M ) NS(I).
c
c    Input, integer NS(M), the number of points along
c    each dimension.
c
c    Input, double precision A(M), B(M), the endpoints for each dimension.
c
c    Input, integer C(M), the grid centering for each dimension.
c    1 <= C(*) <= 5.
c
c    Output, double precision X(M,N) = X(M*S(1),S(2),...,S(M)), the points.
c
      implicit none
