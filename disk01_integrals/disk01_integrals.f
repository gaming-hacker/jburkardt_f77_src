...
c*********************************************************************72
c
cc DISK01_AREA returns the area of the unit disk in 2D.
c
c  Licensing:
c
...
c  Parameters:
c
c    Output, double precision DISK01_AREA, the area of the unit disk.
c
      implicit none
...
c
      ...
      subroutine disk01_sample ( n, seed, x )

c*********************************************************************72
c
cc DISK01_SAMPLE uniformly samples points from the unit disk in 2D.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c
        call r8vec_normal_01 ( 2, seed, x(1:2,j) )
c
c  Compute the length of the vector.
c
        norm = sqrt ( x(1,j) ** 2 + x(2,j) ** 2 )
c
...
c
        ...
        x(2,j) = x(2,j) / norm
c
c  Now compute a value to map the point ON the circle INTO the circle.
c
