...
c
      ...
      open(unit=iunit,file=filef,form='formatted',status='old')
c
c  Read scalars and vectors describing the fine mesh.
c
      call reads(ibs,iunit,nelemf,neqnf,npf,nparb,nquadf,nxf,nyf,
     &  xbl,xbr)

      call readv(areaf,gf,indxf,isotrif,iunit,nelemf,neqnf,nodef,
     &  npf,nparb,nquadf,splbmp,taubmp,xcf,ycf)
...
c
      ...
      open(unit=iunit,file=filec,form='formatted',status='old')
c
c  Read scalars and vectors describing the coarse mesh.
c
      call reads(ibs,iunit,nelemc,neqnc,npc,nparb,nquadc,nxc,nyc,
     &  xbl,xbr)

      call readv(areac,gc,indxc,isotric,iunit,nelemc,neqnc,nodec,
     &  npc,nparb,nquadc,splbmp,taubmp,xcc,ycc)
...
c
      write(*,*)' '
      write(*,*)'Coarse H is ',10.0/real(nxc-1)
      write(*,*)'Fine H is   ',10.0/real(nxf-1)
      write(*,*)'The refinement factor is ',real(nxf-1)/real(nxc-1)
c
c  Evaluate the FINE mesh solution at the nodes of the COARSE mesh.
c
      call sample2(gf,indxf,isotrif,nelemf,neqnf,nodef,npf,
     &  npc,nxf,nyf,pval,uval,vval,xcf,xcc,ycf,ycc)
c
c  Save the sampled fine data as a standard solution vector GFC on the 
c  coarse grid.
c
      ieqn=0
      do i=1,npc
        ...
        ieqn=ieqn+1
        gfc(ieqn)=vval(i)
        if(indxc(3,i).gt.0)then
          ieqn=ieqn+1
          gfc(ieqn)=pval(i)
        endif
      enddo
c
c  Compute the L2 difference between the coarse solution and the sampled
c  fine solution on the coarse mesh.
c
      call l2dif(areaf,gfc,gc,indxc,isotric,nelemc,neqnc,nodec,npc,
     &  nquadc,xcc,ycc)
c
c  Compute the little L2 difference on the coarse mesh.
c
      call ll2dif(gfc,gc,indxc,neqnc,npc)
c
c  Compute the little L-infinity difference on the coarse mesh.
c
      call llinfdif(gfc,gc,indxc,neqnc,npc)
...
c
c
c  Here is a typical finite element associated with pressure:
c
c      2
c     /|
c    / |
...
c
     ...
     &  -(xc(i3)-xc(i1))*(yc(i2)-yc(i1))
      ...
      subroutine bumpy(ielem,nelem,node,np,xc,xsi,xval,yc,yval)
c
c***********************************************************************
c
c  BUMPY is given the X coordinate of a point along the bump, and the 
c  element in which that point is believed to lie.  BUMPY assumes
c  that the point has reference element coordinates of (XSI,0), and
c  proceeds to estimate the value of XSI using Newton iteration.
c
c
...
c         nodes.
c
c  XSI    Output, double precision XSI, the estimated XSI coordinate
c         of the point on the bump.
c
c  XVAL   Input, double precision XVAL, the X coordinate of a point
c         on the bump.
c
...
c
c
c  AREA   Input, double precision AREA(NQUAD,NELEM).
c
...
c         The number of nodes.  NP=(2*NX-1)*(2*NY-1).
c 
c  NQUAD  Input, integer NQUAD.
c
c         The number of quadrature points used.
c
c         Usually 3, but can also be 7.
c
c  XC     Input, double precision XC(NP), the X coordinates of the
c         nodes.
c
...
c
     ...
     &       node,np,p1,u1,v1,x,xc,xsi,y,yc)

          call uval0(eta,g2,ielem,indx,isotri,nelem,neqn,
     &       node,np,p2,u2,v2,x,xc,xsi,y,yc)
...
c
     ...
     &    +(xc(i1)-xc(i3))*(yq    -yc(i2)) )/c
      ...
      subroutine reads(ibs,iunit,nelem,neqn,np,nparb,nquad,nx,ny,
     &  xbl,xbr)
c
c***********************************************************************
c
c  READS reads the scalar data from a run of the FLOW program.
c
c
c  IBS    Output, integer IBS.
c         1, the bump is modeled by C0 linear splines.
c         2, the bump is modeled by C0 quadratic splines.
c         3, the bump is modeled by C1 cubic splines.
c
c  IUNIT  Input, INTEGER IUNIT, the FORTRAN unit on which the restart
c         information may be read.
c
c  NELEM  Output, integer NELEM, the number of elements in the mesh.
c
c  NEQN   Output, integer NEQN, the number of equations and coefficients
c         associated with the mesh.
c
...
c 
c  NQUAD  Output, integer NQUAD.
c
c         The number of quadrature points used.
c
c         Usually 3, but can also be 7.
c
c  NX     Output, integer NX.
c
...
c 
c  XBL    Output, double precision XBL.
c
c         The X coordinate of the left corner of the bump,
c         for each test solution.
c 
c  XBR    Output, double precision XBR.
c
...
c 
      ...
      double precision xbr
c
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)ibs
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)nelem
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)neqn
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)np
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)nparb
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)nquad
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)nx
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)ny
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)xbl
      read(iunit,'(a)',err=10)string
      read(iunit,*,err=10)xbr
...
10    continue
      ...
      subroutine readv(area,g,indx,isotri,iunit,nelem,neqn,node,np,
     &  nparb,nquad,splbmp,taubmp,xc,yc)
c
c***********************************************************************
c
c  READV reads the vector data from a run of the FLOW program.
c
c
c  AREA   Output, double precision AREA(NQUAD,NELEM).
c
c         AREA contains a common factor multiplying the term associated
c         with a quadrature point in a given element, namely,
c
...
c         Here Ar(IELEM) represents the area of element IELEM.
c
c  G      Output, double precision G(NEQN).
c
...
c         associated with the mesh.
c
c  NODE   Output, integer NODE(6,NELEM).
c
c         NODE(I,J) contains, for an element J, the global node index of 
c         the element node whose local number is I.
c
...
c         Usually 3, but can also be 7.
c
c  SPLBMP Output, double precision SPLBMP(4,NPARB+2,0:NPARB).
c
...
c
c         So there are abscissas there, but not free parameters.
c
c  XC     Output, double precision XC(NP).
c
c         The X coordinates of the nodes.
c
c  YC     Output, double precision YC(NP).
c
...
c
      ...
      do i=1,nquad
        do j=1,nelem
          read(iunit,*,err=10)area(i,j)
        enddo
      enddo

      read(iunit,'(a)',err=10)string
      do i=1,neqn
        read(iunit,*,err=10)g(i)
      enddo

      read(iunit,'(a)',err=10)string
      do i=1,3
        do j=1,np
          read(iunit,*,err=10)indx(i,j)
        enddo
      enddo

      read(iunit,'(a)',err=10)string
      do i=1,nelem
        read(iunit,*,err=10)isotri(i)
      enddo

      read(iunit,'(a)',err=10)string
      do i=1,6
        do j=1,nelem
          read(iunit,*,err=10)node(i,j)
        enddo
      enddo
      ...
      do i=1,4
        do j=1,nparb+2
          do k=0,nparb
            read(iunit,*,err=10)splbmp(i,j,k)
          enddo
        enddo
      enddo
      ...
      read(iunit,'(a)',err=10)string
      do i=1,nparb+2
        read(iunit,*,err=10)taubmp(i)
      enddo

      read(iunit,'(a)',err=10)string
      do i=1,np
        read(iunit,*,err=10)xc(i)
      enddo

      read(iunit,'(a)',err=10)string
      do i=1,np
        read(iunit,*,err=10)yc(i)
      enddo
...
c
      ...
      subroutine sample2(g,indx,isotri,nelem,neqn,node,np,nval,nx,ny,
     &  pval,uval,vval,xc,xval,yc,yval)
c
c***********************************************************************
c
c  SAMPLE2 evaluates a finite element flow solution, whose coefficients
...
c
        ...
        call bumpy(ielem,nelem,node,np,xc,xsi,x,yc,ymin)
c
...
c 
c  NEQN   Input, integer NEQN.
c
c         The number of equations, and functions.
c 
...
c         The number of nodes.  NP=(2*NX-1)*(2*NY-1).
c 
c  P      Output, double precision P, the value of the pressure at the
c         point.
c
c  U,
c  V      Output, double precision U, V, the value of the horizontal
c         and vertical velocities at the point.
c
