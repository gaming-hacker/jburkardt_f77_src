...
c*********************************************************************72
c
cc SGEMM computes C = alpha * A * B and related operations.
c
c  Discussion:
c
...
c
c    Input, real ALPHA, the scalar multiplier 
c    for op ( A ) * op ( B ).
c
c    Input, real A(LDA,KA), where:
...
c
c    Input, real BETA, the scalar multiplier for C.
c
c    Input/output, real C(LDC,N).
...
c
      ...
      if ( ( alpha .eq. 0.0E+00 .or. k .eq. 0 ) .and. 
     &   ( beta .eq. 1.0E+00 ) ) then
        return
      end if
c
