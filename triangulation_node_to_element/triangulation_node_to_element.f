...
c*********************************************************************72
c
cc MAIN is the main program for TRIANGULATION_NODE_TO_ELEMENT.
c
c  Discussion:
c
c    TRIANGULATION_NODE_TO_ELEMENT averages node values to an element value.
c
c    This program is given a triangulation, along with the values of one or
c    more data items associated with each node.
c
c    It produces a file containing the average of the nodal values for 
c    each element.
c
c  Usage:
c
c    triangulation_node_to_element prefix
c
c    where
c
c    * 'prefix'_nodes.txt contains the node coordinates;
c    * 'prefix'_elements.txt contains the element definitions 
c      (this file is optional, and if missing, the elements will be generated
c      by the program);
c    * 'prefix'_values.txt contains the nodal values.
c    * 'prefix'_element_values will contain the averaged element data.
c
c  Licensing:
c
...
c  Parameters:
c
c    Input, character * ( * ) PREFIX, the common file prefix.
c
      implicit none
      ...
      character * ( 255 ) element_value_filename
      double precision, allocatable, dimension ( :, : ) :: element_value
      integer i
      integer iarg
      integer iargc
      integer ierror
...
c
      arg_num = iargc ( )
c
c  First argument is the file prefix.
c
...
c
c  Get sizes of things.
c
      call r8mat_header_read ( node_filename, dim_num, node_num )

      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 
     &  '  Read the header of "' // trim ( node_filename ) //'".'
      write ( *, '(a)' ) ' '
     ...
     &  '  Number of elements ELEMENT_NUM  = ', element_num
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 
     &  '  Read the header of "' // trim ( value_filename ) //'".'
      write ( *, '(a)' ) ' '
      write ( *, '(a,i8)' ) 
     &  '  Spatial dimension VALUE_DIM = ', value_dim
      write ( *, '(a,i8)' ) 
     &  '  Number of values VALUE_NUM  = ', value_num

      call main_sub ( dim_num, element_num, element_order, 
     &  node_num, value_dim, value_num, element_filename, 
     &  element_value_filename, node_filename, value_filename )
c
c  Terminate.
c
      write ( *, '(a)' ) ''
      ...
      subroutine main_sub ( dim_num, element_num, element_order, 
     &  node_num, value_dim, value_num, element_filename, 
     &  element_value_filename, node_filename, value_filename )

c*********************************************************************72
c
cc MAIN_SUB does allocatable arrays the hard way.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      character * ( 255 ) element_value_filename
      double precision element_value(value_dim,element_num)
      integer i
      integer j
      integer k
      integer ni
...
c
     ...
     &  '  Read the data in "' // trim ( node_filename ) //'".'
      ...
      call r8mat_transpose_print_some ( dim_num, node_num, node_xy, 
...
c
     ...
     &  element_num, element_node )
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 
     &  '  Read the data in "' // trim ( element_filename ) //'".'
...
c  Read the values.
c
      call r8mat_data_read ( value_filename, value_dim, value_num, 
     &  value )

      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 
     &  '  Read the data in "' // trim ( value_filename ) //'".'

      call r8mat_transpose_print_some ( value_dim, value_num, value, 
     &  1, 1, value_dim, 5, '  First 5 values:' )
c
c  Create the element_values data.
c
      do j = 1, element_num
        do i = 1, element_order
          ni = element_node ( i, j )
          do k = 1, value_dim
            element_value(k,j) = element_value(k,j) + value(k,ni)
          end do
        end do
      end do
      ...
      do j = 1, element_num
        do k = 1, value_dim
          element_value(k,j) = element_value(k,j) 
     &      / dble ( element_order )
        end do
      end do
c
...
c
      call r8mat_write ( element_value_filename, value_dim, 
...
c
c    This function attempts to detect 9-based node indexing and correct it.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c
     ...
     &  '  The element indexing appears to be 0-based!'
        ...
        write ( *, '(a)' ) ' '
        write ( *, '(a)' )'MESH_BASE_ONE:'
        write ( *, '(a)' )'  Element indexing appears to be 1-based!'
...
10    continue
      ...
      if ( i .lt. n ) then
        ...
        call s_to_r8_old ( s(ilo:), rvec(i), ierror, lchar )
