...
c*********************************************************************72
c
cc MAIN is the main program for CCN_RULE.
c
c  Discussion:
c
c    This program computes a nested Clenshaw Curtis quadrature rule
...
c
      if ( 4 .le. arg_num ) then
        ...
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 
     &    '  Enter FILENAME, the "root name" of the files).'
        read ( *, '(a)' ) filename
      end if
c
c  Input summary.
...
c*********************************************************************72
c
cc CCN_COMPUTE_POINTS: compute Clenshaw Curtis Nested points.
c
c  Discussion:
c
c    We want to compute the following sequence:
c
c    1/2,
c    0, 1
c    1/4, 3/4
c    1/8, 3/8, 5/8, 7/8,
c    1/16, 3/16, 5/16, 7/16, 9/16, 11/16, 13/16, 15/16, and so on.
c
c    But we'd prefer that the numbers in each row be regrouped in pairs
c    that are symmetric about 1/2, with the number above 1/2 coming first.
c    Thus, the last row might become:
c    (9/16, 7/16), (11/16, 5/16), ..., (15/16, 1/16).
c
c    Once we have our sequence, we apply the Chebyshev transformation
c    which maps [0,1] to [-1,+1].
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, integer N, the number of elements to compute.
c
c    Output, double precision X(N), the elements of the sequence.
c
      implicit none
...
c
c  Handle first three entries specially.
c
      if ( 1 .le. n ) then
        x(1) = 0.5D+00
      end if
      ...
      if ( m .lt. n ) then
        ...
        do i = 1, k
          if ( mod ( i, 2 ) .eq. 1 ) then
            x(m+i) = dble ( tu ) / 2.0D+00 / dble ( k )
            tu = tu + 2
          else
            x(m+i) = dble ( td ) / 2.0D+00 / dble ( k )
            td = td - 2
          end if
        end do
...
c
c  Apply the Chebyshev transformation.
c
      do i = 1, n
        x(i) = cos ( x(i) * r8_pi )
      end do
