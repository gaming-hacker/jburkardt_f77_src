c  arby2sub0.f  13 July 1996
c
      program main
c 
c***********************************************************************
c
cc MAIN is the main program for ARBY2.
c
...
c
      ...
      double precision arb(0:maxnrb,0:maxnrb)
      ...
      double precision grb(0:maxnrb)
      double precision grbarb(0:maxnrb)
      double precision grbopt(0:maxnrb)
      double precision grbsav(0:maxnrb)
      double precision grbsen(0:maxnrb)
      double precision grbtay(0:maxnrb)
      double precision grbtmp(0:maxnrb)
      character*20 gridx
      character*20 gridy
      double precision hx
      double precision hy
      ...
      double precision phirb(3,0:maxnrb,15,maxelm)
      double precision rb(maxnfl,0:maxnrb)
      character*20 region
      double precision resfl(maxnfl)
      double precision resflsav(maxnfl)
      double precision resfltmp(maxnfl)
      double precision resrb(0:maxnrb)
      double precision reynld
      double precision reytay
      double precision rfact(0:maxnrb,0:maxnrb)
      double precision rmax
      double precision senfl(maxnfl,0:maxnrb)
      double precision sentmp(maxnfl,0:maxnrb)
      double precision senrb(0:maxnrb,0:maxnrb)
...
c
     ...
     &  costv,dcof,disfil,drey,epsdif,eqn,etaq,gfl,gflafl,
     &  gflrb,gflsav,gflsen,gfltar,gfltay,grb,grbarb,grbsav,
     &  grbsen,
     &  grbtay,grbtmp,gridx,gridy,hx,hy,ibs,ibump,ierror,ifs,igunit,
     &  ihi,ijac,ilo,indx,iopt,ipivfl,ipivrb,isotri,iwrite,jhi,jlo,
     &  ldafl,maxelm,maxnew,maxnfl,maxnp,maxnrb,
...
c
      elseif(leqi(command,'detfprb'))then
        write(*,*)'ARBY - DET FP RB'
        write(*,*)'  Compute determinant of reduced jacobian.'
 
        call fprb(arb,area,grb,maxnrb,nelem,neqnrb,phirb,reynld)
 
        call dfacrb(arb,maxnrb+1,neqnrb+1,ipivrb,info)
 
        call ddetrb(arb,detlog,detman,ipivrb,maxnrb,neqnrb)
...
c
      elseif(leqi(command,'diffprb'))then
        ...
        call diffprb(arb,area,epsdif,grb,grbtmp,maxnrb,nelem,
     &    neqnrb,phirb,resrb,reynld)
c
c  DIFSENFL
c
...
c
      elseif(leqi(command,'difsenrb'))then
        ...
        write(*,*)'ARBY - Dif Sen RB:'
        write(*,*)'  Estimate reduced solution sensitivity '
        write(*,*)'  with respect to parameter ',ipar
        write(*,*)'  via finite differences.'

        call difsenrb(arb,area,dcof,dpar,grb,grbarb,grbtmp,ipar,
     &    ipivrb,iwrite,maxnew,maxnrb,nelem,neqnrb,npar,par,
     &    pararb,partmp,phirb,resrb,senrb,tolnew)
c
c  DisPlot
c
...
c
      elseif(leqi(command,'expand grb'))then
       ...
       call vrb2fl(gfl,grb,maxnfl,neqnfl,neqnrb,rb)
c
c  DISFIL=
c
...
c
      elseif(leqi(command,'fpfl'))then
        write(*,*)'ARBY - FP FL - Evaluate full jacobian.'
        call fpfl(afl,area,eqn,gfl,indx,ldafl,nelem,neqnfl,nlband,
     &    node,np,npar,nparb,nparf,par,phifl)
c
c  FPRB
c
      elseif(leqi(command,'fprb'))then
        write(*,*)'ARBY - FP RB - Evaluate reduced jacobian.'
        do i=1,npar
          pararb(i)=par(i)
        enddo
...
c
      elseif(leqi(command,'fprb=0'))then
        write(*,*)'ARBY - FPRB=0 - Zero out the reduced jacobian.'
        do i=0,maxnrb
          do j=0,maxnrb
            arb(i,j)=0.0
          enddo
...
c
      elseif(leqi(command,'fxrb'))then
        ...
        call fxrb(area,grb,nelem,neqnrb,phirb,resrb,reynld)
c
c  FXIRB
c
      elseif(leqi(command,'fxirb'))then
        ...
        call fxirb(area,eqn,gfl,grb,ifs,indx,maxnfl,nelem,
...
c
      elseif(leqi(command,'getrb'))then
        ...
        call getrb(gfl,gflrb,maxnfl,maxnrb,neqnfl,neqnrb,npar,
...
c
        ...
        call setprb(eqn,indx,maxnfl,nelem,neqnfl,neqnrb,node,np,
     &    phifl,phirb,rb)
c
c  GETSENFL
c
      elseif(leqi(command,'getsenfl'))then
        ...
        if(neqnrb.lt.0)then
          ...
          call getsenfl(afl,area,eqn,gfl,indx,ipivfl,ldafl,maxnfl,
     &      nelem,neqnfl,neqnrb,nlband,node,np,npar,par,phifl,resfl,
     &      senfl)
        endif
c
c  GETSENRB
...
c
c  Map the 0:NEQNRB columns of SENFL into SENRB.
c
        do j=0,neqnrb
          call vfl2rb(senfl(1,j),senrb(0,j),maxnfl,neqnfl,neqnrb,rb)
        enddo
c
...
c
     ...
     &    np,npar,nparb,nparf,par,parafl,phifl,region,resfl,
     &    rmax,splflo,tauflo,tolnew,xc,yc)
...
c
      elseif(leqi(command,'newtrb'))then
        ...
        call newtrb(arb,area,grb,grbarb,ierror,ipivrb,iwrite,maxnew,
     &    maxnrb,nelem,neqnrb,npar,par,pararb,phirb,resrb,rmax,tolnew)
...
c
      elseif(leqi(command,'optdifrb'))then
        ...
        call optdifrb(arb,area,cost,dopt,gfltar,
     &    gfltmp,grb,grbarb,grbopt,grbtmp,ierror,indx,iopt,ipivrb,
     &    ivopt,iwrite,liv,lv,maxelm,maxnew,maxnfl,maxnp,maxnrb,
...
c
      elseif(leqi(command,'picrb'))then
        ...
        call picrb(arb,area,grb,grbtmp,ierror,ipivrb,iwrite,maxsim,
     &    maxnrb,nelem,neqnrb,npar,par,phirb,resrb,rmax,tolsim)
...
c
      elseif(leqi(command,'prfprb'))then
        ...
        call prdmat(arb,ihi,ilo,jhi,jlo,maxnrb,0,neqnrb,0)
c
c  PRDAT
c
...
c
      elseif(leqi(command,'prr'))then
        ...
        call prdmat(rfact,ihi,ilo,jhi,jlo,maxnrb,0,neqnrb,0)
c
c  PRRB
c
      elseif(leqi(command,'prrb'))then
        ...
        call prmatfl(rb,eqn,ihi,ilo,indx,jhi,jlo,maxnfl,neqnfl,
     &    neqnrb,np)
c
...
c
      elseif(leqi(command,'prsenfl'))then
        ...
        call prmatfl(senfl,eqn,ihi,ilo,indx,jhi,jlo,maxnfl,neqnfl,
     &    neqnrb,np)
c
...
c
      elseif(leqi(command,'prsennrm'))then
        ...
        call prsenn(maxnfl,neqnfl,neqnrb,senfl)
c
...
c
      elseif(leqi(command,'prsenrb'))then
        ...
        call prdmat(senrb,ihi,ilo,jhi,jlo,maxnrb,0,neqnrb,0)
c
c  PRXY
c
...
c
      elseif(leqi(command,'reduce gfl'))then
        ...
        call vfl2rb(gfl,grb,maxnfl,neqnfl,neqnrb,rb)
c
c  REGION=
c
...
c
      elseif(leqi(command,'test2'))then
        ...
        call test2(gfl,grb,ihi,ilo,indx,nelem,neqnfl,neqnrb,node,
     &    np,phifl,phirb)
c
c  TEST3
c
      elseif(leqi(command,'test3'))then
        ...
        call test3(maxnfl,maxnrb,neqnfl,neqnrb,rb,rfact,senfl)
c
c  TEST4
c
      elseif(leqi(command,'test4'))then
        ...
        call getsenfl(afl,area,eqn,gfl,indx,ipivfl,ldafl,maxnfl,
     &    nelem,neqnfl,neqnrb,nlband,node,np,npar,par,phifl,
     &    resfl,senfl)
 
        do i=1,neqnfl
          do j=0,neqnrb
            sentmp(i,j)=senfl(i,j)
          enddo
        enddo
     ...
     &    tolnew,xc,yc)
        ...
        do i=1,neqnfl
          do j=0,neqnrb
            if(abs(sentmp(i,j)-senfl(i,j)).ge.difmax)then
              imax=i
              jmax=j
              difmax=abs(sentmp(i,j)-senfl(i,j))
            endif
          enddo
        enddo
...
120   continue
      ...
      subroutine diffprb(arb,area,epsdif,grb,grbtmp,maxnrb,nelem,
     &  neqnrb,phirb,resrb,reynld)
c
c***********************************************************************
c
...
c
c
c  This routine was written on 11 July 1996.
c
c  ARB    Output, double precision ARB(0:MAXNRB,0:MAXNRB).
c         ARB contains the Jacobian or Picard matrix for the reduced
c         Navier Stokes system, stored as a dense 0:NEQNRB by 0:NEQNRB 
c         array.
c
...
c
c  GRB    Input, double precision GRB(0:NEQNRB).
...
c
c  RESRB  Workspace, double precision RESRB(0:NEQNRB).
...
c
        call fxrb(area,grbtmp,nelem,neqnrb,phirb,resrb,reynld)
c
...
c
     ...
     &     eqn(iver).eq.'V0')then
      ...
      subroutine fprb(arb,area,grb,maxnrb,nelem,neqnrb,phirb,reynld)
c
c***********************************************************************
c
...
c
c  Consider reduced basis function IEQNRB.
c
          do ieqnrb=0,neqnrb

            wu0   =phirb(iquad,ieqnrb,10,ielem)
            dwu0dx=phirb(iquad,ieqnrb,11,ielem)
            dwu0dy=phirb(iquad,ieqnrb,12,ielem)
            wv0   =phirb(iquad,ieqnrb,13,ielem)
            dwv0dx=phirb(iquad,ieqnrb,14,ielem)
            dwv0dy=phirb(iquad,ieqnrb,15,ielem)
c
c  Take the derivative with respect to basis function JEQNRB.
c
            do jeqnrb=0,neqnrb
              ...
              arb(ieqnrb,jeqnrb)=arb(ieqnrb,jeqnrb)+ar*
...
c
      ...
      subroutine fxirb(area,eqn,gfl,grb,ifs,indx,maxnfl,nelem,
     &  neqnfl,neqnrb,node,np,npar,nparb,nparf,par,phifl,rb,region,
     &  resfl,resrb,splflo,tauflo,xc,yc)
c
c***********************************************************************
...
c
c  RESRB  Output, double precision RESRB(0:NEQNRB), the residual in the
...
c
      ...
      double precision par(npar)
      double precision phifl(3,6,10,nelem)
      double precision rb(maxnfl,0:neqnrb)
      character*20 region
      double precision resfl(neqnfl)
      double precision resrb(0:neqnrb)
      double precision splflo(nparf+2)
...
c
      call vrb2fl(gfl,grb,maxnfl,neqnfl,neqnrb,rb)
c
c  Get the residual RESFL for the full coefficient set.
c
      call fxfl(area,eqn,gfl,ifs,indx,nelem,neqnfl,node,np,npar,
     &  nparb,nparf,par,phifl,region,resfl,splflo,tauflo,
     &  xc,yc)
c
c  Project the residual RESFL back into the reduced space,
c  arriving at RESRB.
c
      ...
      subroutine fxrb(area,grb,nelem,neqnrb,phirb,resrb,reynld)
c
c***********************************************************************
c
cc FXRB evaluates the reduced basis residual directly (that is,
...
c
c
c  This routine was completely rewritten on 11 July 1996.
c
c
c  AREA   Input, double precision AREA(3,NELEM).
c
...
c             K=15, dWV0rbdY, same as dWVrbdY, with zero BC.
c
c  RESRB  Output, double precision RESRB(0:NEQNRB).
c         RESRB contains the residual in the reduced basis equations, 
...
c
c  Now consider each reduced basis function.
c
          do ieqnrb=0,neqnrb
...
c
      ...
      write(*,*)'HELLO!'
      write(*,*)' '
      write(*,*)'  This is ARBY2, the version of 13 July 1996.'
...
20    continue
      ...
      subroutine newtrb(arb,area,grb,grbarb,ierror,ipivrb,iwrite,maxnew,
     &  maxnrb,nelem,neqnrb,npar,par,pararb,phirb,resrb,rmax,tolnew)
c
c***********************************************************************
c
...
c
c
c  ARB    Workspace, double precision ARB(0:MAXNRB,0:MAXNRB).
c
c         ARB contains the Jacobian matrix for the reduced basis system.
c
c  GRB    Input/output, double precision GRB(0:NEQNRB), the current solution
...
c         be computed.
c
c  IPIVRB Workspace, integer IPIVRB(0:NEQNRB), pivot vector for the solution
...
c             K=15, dWV0rbdY, same as dWVrbdY, with zero BC.
c
c  RESRB  Workspace, double precision RESRB(0:NEQNRB), the residual in the
c         reduced basis equations, evaluated at the coefficient
c         vector GRB.
c
c  TOLNEW Input, double precision TOLNEW, the Newton tolerance.
...
c
      ...
      double precision dmax
      double precision grb(0:neqnrb)
      double precision grbarb(0:neqnrb)
      integer i
      integer idamax
      integer idmax
      integer ierror
      integer info
      integer ipivrb(0:neqnrb)
      integer irmax
      integer iwrite
      integer ixmax
      integer maxnew
...
c
      ...
      irmax=idamax(neqnrb+1,resrb,1)-1
...
c  Solve the linear system A*DX=RES
c
        call dsolrb(arb,maxnrb+1,neqnrb+1,ipivrb,resrb)
 
        idmax=idamax(neqnrb+1,resrb,1)-1
...
c
      ...
      subroutine optdifrb(arb,area,cost,dopt,gfltar,
     &  gfltmp,grb,grbarb,grbopt,grbtmp,ierror,indx,iopt,ipivrb,
     &  ivopt,iwrite,liv,lv,maxelm,maxnew,maxnfl,maxnp,maxnrb,
     &  maxny,maxopt,maxpar,maxparb,maxsim,nelem,neqnfl,neqnrb,np,
     &  npar,nparb,nparf,nprof,numdif,numopt,ny,par,pararb,paropt,
...
c         columns, so that:
c
c           Transpose(RB) * RB = Identity(0:NEQNRB)
c
c
...
c
c         which follows from Transpose(RB) * RB = Identity(0:NEQNRB).
c
...
c
      ...
      double precision grb(0:neqnrb)
      double precision grbarb(0:neqnrb)
      double precision grbopt(0:neqnrb)
      double precision grbtmp(0:neqnrb)
      integer i
      integer ierror
      integer indx(3,maxnp)
...
c
        call picrb(arb,area,grbopt,grbtmp,ierror,ipivrb,iwrite,maxsim,
     &    maxnrb,nelem,neqnrb,npar,paropt,phirb,resrb,rmax,tolsim)
c
...
c
        if(rmax.le.tolnew)then
          ...
          call newtrb(arb,area,grbopt,grbarb,ierror,ipivrb,iwrite,
     &      maxnew,maxnrb,nelem,neqnrb,npar,paropt,pararb,phirb,resrb,
     &      rmax,tolnew)
...
c
      call vrb2fl(gfltmp,grbopt,maxnfl,neqnfl,neqnrb,rb)
c
c  Compute the cost function COST.
c
...
c
      ...
      subroutine test2(gfl,grb,ihi,ilo,indx,nelem,neqnfl,neqnrb,node,
     &  np,phifl,phirb)
c
c***********************************************************************
c
...
c
c  This routine was dashed off on 09 July 1996.
c
      implicit double precision (a-h,o-z)
c
      integer nelem
...
c
     ...
     &      ielem,iquad,nelem,neqnrb,prb,phirb,urb,vrb)
      ...
      subroutine test3(maxnfl,maxnrb,neqnfl,neqnrb,rb,rfact,senfl)
c 
c***********************************************************************
c
...
c
      ...
      write(*,*)' '
      write(*,*)'The maximum difference between RB*RFACT and SENFL'
...
20      continue
      ...
      subroutine ddetrb(arb,detlog,detman,ipivrb,maxnrb,neqnrb)
c
c***********************************************************************
c
...
c
c
c  ARB    Input, double precision ARB(0:MAXNRB,0:NEQNRB).
...
c
c  IPIVRB Input, integer IPIVRB(0:NEQNRB).
c         The pivot vector.
c
c  MAXNRB Input, integer MAXNRB.
...
c
      ...
      subroutine dfacrb(arb,lda,n,ipivrb,info)
c 
c***********************************************************************
c
...
c
c
c  ARB    Input/output, double precision ARB(LDA,N).
c
c         On input, the matrix to be factored.
c
c         On output, contains the LU factors of ARB, in compressed storage.
c
c  LDA    Input, integer LDA, the leading dimension of ARB.
c
c  N      Input, integer N, the order of ARB.
c
c  IPIVRB Output, integer IPIVRB(N), the pivot array.
c
...
c
      ...
      do k=1,n-1
        ...
        l=idamax(n-k+1,arb(k,k),1)+k-1
        ipivrb(k)=l

        if(arb(l,k).eq.0.0)then
...
c
      ...
      subroutine difsenfl(afl,area,dcof,dpar,eqn,gfl,gflafl,
     &  gfltmp,ifs,ijac,indx,ipar,ipivfl,iwrite,ldafl,maxnew,maxnfl,
     &  nelem,neqnfl,neqnrb,nlband,node,np,npar,nparb,nparf,par,
     &  parafl,partmp,phifl,region,resfl,senfl,splflo,tauflo,
     &  tolnew,xc,yc)
...
c***********************************************************************
c  
cc DIFSENFL computes a central difference estimate for the first NEQNRB 
...
c
c  Compute difference NDIF, for NDIF=0 to NEQNRB.
c
      do ndif=0,neqnrb
...
c
     ...
     &        indx,ipivfl,iwrite,ldafl,maxnew,nelem,neqnfl,nlband,node,
     &        np,npar,nparb,nparf,partmp,parafl,phifl,region,resfl,
     &        rmax,splflo,tauflo,tolnew,xc,yc)
...
c
      ...
      subroutine difsenrb(arb,area,dcof,dpar,grb,grbarb,grbtmp,ipar,
     &  ipivrb,iwrite,maxnew,maxnrb,nelem,neqnrb,npar,par,pararb,
     &  partmp,phirb,resrb,senrb,tolnew)
c
c***********************************************************************
c  
...
c
c
c  This routine was written on 12 July 1996.
c
c
c  ARB    Workspace, double precision ARB(0:MAXNRB,0:MAXNRB).
c         ARB contains the Jacobian or Picard matrix for the reduced
...
c         estimate of the state solution.
c
c  GRBARB Output, double precision GRBARB(0:NEQNRB).
c         GRBARB contains the reduced basis coefficients at which
c         the matrix ARB was last evaluated.
c
c  GRBTMP Workspace, double precision GRBTMP(0:NEQNRB).
...
c         for the parameter values PAR and reduced basis coefficients GRB.
c
c  SENRB  Output, double precision SENRB(0:MAXNRB,0:NEQNRB).
...
c
      ...
      double precision rmax
      double precision resrb(0:neqnrb)
      double precision senrb(0:maxnrb,0:neqnrb)
      double precision tolnew
c
      write(*,*)' '
      write(*,*)'DIFSENRB - DEBUG'
      write(*,*)'  I, GRB(I)'
      write(*,*)' '
      do i=0,neqnrb
        write(*,*)i,grb(i)
...
c
      ...
      enddo
c
c  Compute the differences of orders JDIF=0 to NEQNRB.
c
      do jdif=0,neqnrb
        ...
        if(jdif.eq.0)then
          ...
          write(*,*)' '
          write(*,*)'Computing difference column JDIF=',jdif
c
c  Get the difference coefficients DCOF for this order.
c
          call difcof(dcof,dpar,iwrite,jdif)
c
c  Evaluate the solution at several values of the parameter.
c
          do i=0,jdif
...
c
            do j=1,npar
              partmp(j)=par(j)
            enddo
            partmp(ipar)=par(ipar)+(2*i-jdif)*dpar
            write(*,*)'I=',I,' PAR(IPAR)=',partmp(ipar)
c
...
c
            call newtrb(arb,area,grbtmp,grbarb,ierror,ipivrb,iwrite,
     &        maxnew,maxnrb,nelem,neqnrb,npar,partmp,pararb,phirb,resrb,
     &        rmax,tolnew)
...
c
            do j=0,neqnrb
              senrb(j,jdif)=senrb(j,jdif)+dcof(i)*grbtmp(j)
            enddo
...
c
      ...
      subroutine dsolrb(arb,lda,n,ipivrb,b)
c 
c***********************************************************************
c
cc DSOLRB solves the linear system ARB*X=B
c
c  where the matrix ARB represents a reduced basis system, and is a
c  full storage double precision array, and has been factored by
c  DFACRB.
c
c
c  ARB    Input, double precision ARB(LDA,N), the factored matrix,
...
c  N      Input, integer N, the order of ARB.
c
c  IPIVRB Input, integer IPIVRB(N), the pivot vector computed
c         by DFACRB.
c
c  B      Input/output, double precision B(N).
c
...
c
     ...
     &  nprof,ny,yc)
      ...
      subroutine getrb(gfl,gflrb,maxnfl,maxnrb,neqnfl,neqnrb,npar,
     &  par,parrb,rb,rfact,senfl,tau,work)
c
c***********************************************************************
c
...
c
c  GETRB is given:
c    NEQNRB, the number of reduced basis vectors;
...
c    RB, an NEQNFL by 1+NEQNRB orthogonal matrix, whose columns
c     were initially SENFL(0), SENFL(1), ..., SENFL(NEQNRB), and which
c     is essentially the "Q" factor of this matrix,
c    RFACT, the "R" factor in the QR factorization of the matrix.
c
c  This routine was revised on 07 July 1996.
c
c
c  GFL    Input, double precision GFL(NEQNFL), the current solution
c         estimate for the full problem.
...
c
c  RB     Output, double precision rb(maxnfl,0:neqnrb).
...
c         the columns of RB.
c
c  RFACT  Output, double precision RFACT(0:MAXNRB,0:MAXNRB).
...
c
c  WORK   Workspace, double precision WORK(0:MAXNRB), a work vector used by
...
c
      call dgeqrf(neqnfl,neqnrb+1,rb,maxnfl,tau,work,maxnfl,info)
 
      if(info.ne.0)then
        write(*,*)' '
        write(*,*)'GETRB - Fatal error!'
        write(*,*)'  LAPACK QR factor routine DGEQRF returns INFO=',info
        stop
      endif
c
c  Extract the "R" factor from RB, and save it in RFACT.
c
      do i=0,neqnrb
        do j=0,neqnrb
          if(j.lt.i)then
...
c
      call dorgqr(neqnfl,neqnrb+1,neqnrb+1,rb,maxnfl,tau,work,
     &  maxnrb+1,info)
 
      if(info.ne.0)then
        write(*,*)' '
        write(*,*)'GETRB - Fatal error!'
        write(*,*)'  The LAPACK routine DORGQR returns INFO=',info
        stop
      endif

      return
      end
      subroutine getsenfl(afl,area,eqn,gfl,indx,ipivfl,ldafl,maxnfl,
     &  nelem,neqnfl,neqnrb,nlband,node,np,npar,par,phifl,resfl,senfl)
c
c***********************************************************************
c
...
c
     ...
     &  //'  MxNorm(UVSen) MxNorm(PSen)'
      ...
      isen=0
      call uvpnrm(senfl(1,0),indx,neqnfl,np,spnrm,suvnrm)
...
c  of order ISEN.
c
        call reysen(area,eqn,indx,isen,maxnfl,nelem,neqnfl,
...
c***********************************************************************
c
cc INIT initialized program data.
c
      implicit double precision (a-h,o-z)
c
      integer ldafl
...
c
     ...
     &     eqn(iver).eq.'V0')then
      ...
      subroutine picmrb(arb,area,grb,maxnrb,nelem,neqnrb,npar,
     &  par,phirb)
c
c***********************************************************************
c
...
c
c
c  ARB    Output, double precision ARB(0:MAXNRB,MAXNRB).
c         ARB contains the Jacobian or Picard matrix for the reduced 
c         Navier Stokes system, stored as a dense NEQNRB by NEQNRB array.
c
c  AREA   Input, double precision AREA(3,NELEM).
...
c
     ...
     &          +reynld*(urb*dwvjdx+vrb*dwvjdy+dqjdy)*wvi )
      ...
      subroutine picrb(arb,area,grb,grbtmp,ierror,ipivrb,iwrite,maxsim,
     &  maxnrb,nelem,neqnrb,npar,par,phirb,resrb,rmax,
     &  tolsim)
c
c***********************************************************************
c
...
c
c
c  ARB    Workspace, double precision ARB(0:MAXNRB,MAXNRB).
c         ARB contains the Jacobian or Picard matrix for the reduced 
c         Navier Stokes system, stored as a dense NEQNRB by NEQNRB array.
c
c  AREA   Input, double precision AREA(3,NELEM).
...
c
        call picmrb(arb,area,grb,maxnrb,nelem,neqnrb,npar,
     &    par,phirb)
c
c  Factor the matrix.
c
...
c
        call picvrb(area,grb,grbtmp,nelem,neqnrb,npar,
     &    par,phirb,resrb)
c
...
c
      ...
      subroutine picvrb(area,grb,grbtmp,nelem,neqnrb,npar,
     &  par,phirb,resrb)
c
c***********************************************************************
c
...
c
c
c  GRB    Input, double precision GRB(0:NEQNRB), coefficients for the
c         reduced system.
c
...
c
      ...
      itemp=idamax(neqnrb+1,grb,1)
      anrmg=abs(grb(itemp-1))
      enrmg=dnrm2(neqnrb+1,grb,1)
...
c
      ...
      subroutine prmatfl(a,eqn,ihi,ilo,indx,jhi,jlo,maxnfl,neqnfl,
     &  neqnrb,np)
c
c***********************************************************************
c
...
c
c
c  A      Input, double precision A(MAXNFL,0:NEQNRB).
...
c
      ...
      integer np
c
      double precision a(maxnfl,0:neqnrb)
      character*2 eqn(neqnfl)
      integer i
      integer ihi
      integer ihi2
...
c
      if(ilo.lt.1)then
        ilo2=1
        write(*,*)'PrMATFL - ILO reset to 1, input value ',ilo
      else
        ilo2=ilo
      endif
      ...
      if(ihi.gt.np)then
        ihi2=np
        write(*,*)'PrMATFL - IHI reset to ',np,' input value=',ihi
      else
        ihi2=ihi
      endif
      ...
      if(jlo.lt.0)then
        jlo2=0
        write(*,*)'PrMATFL - JLO reset to 0, input value was ',jlo
      else
        jlo2=jlo
      endif
 
      if(jhi.gt.neqnrb)then
        jhi2=neqnrb
        write(*,*)'PrMATFL - JHI reset to ',jhi2,' input value was ',jhi
      else
        jhi2=jhi
      endif
...
c
     ...
     &          (a(i,l),l=llo,lhi)
      ...
      subroutine prsenn(maxnfl,neqnfl,neqnrb,senfl)
c
c***********************************************************************
c
...
c         state equations, and reduced basis coefficients GRB.
c
c  SENFL  Input, double precision SENFL(MAXNFL,0:NEQNRB).
c         SENFL contains the first several order sensitivities of the
c         full solution with respect to the REYNLD parameter.
c
c         SENFL(I,J) contains the J-th sensitivity of the I-th full unknown
...
c
      ...
      subroutine reysen(area,eqn,indx,isen,maxnfl,nelem,neqnfl,
     &  node,np,phifl,resfl,reynld,senfl)
c
c***********************************************************************
c
...
c
      if(isen.gt.maxnrb)then
        write(*,*)' '
        write(*,*)'REYSEN - Fatal error!'
        write(*,*)'  The input value of ISEN is ',isen
        write(*,*)'  but the local value of MAXNRB is ',maxnrb
        stop
      endif
c
...
c
      ...
      subroutine setprb(eqn,indx,maxnfl,nelem,neqnfl,neqnrb,node,np,
     &  phifl,phirb,rb)
c
c***********************************************************************
c
...
c         since there are only three linear basis functions.
c
c  PHIRB  Output, double precision PHIRB(3,0:NEQNRB,15,NELEM).
c         PHIRB contains the values of a finite element basis function
c         or its X or Y derivative, in a given element, at a given
c         quadrature point, for a particular reduced basis function.
c
...
c             K=15, dWV0rbdY, same as dWVrbdY, with zero BC.
c
c  RB     Input, double precision rb(maxnfl,0:neqnrb).
c         RB is the NEQNFL by 1+NEQNRB array of reduced basis vectors.
c
c         RB is generated by computing a finite element solution GFL,
c         which is saved for later reference as "GFLRB".
...
c
c  RESRB  Output, double precision RESRB(0:NEQNRB), the function value
...
c
      ...
      double precision resrb(0:neqnrb)
c
c  Multiply the full vector RESFL by RB transpose.
c
      do i=0,neqnrb
        resrb(i)=0.0
        do j=1,neqnfl
      ...
      subroutine vrb2fl(gfl,grb,maxnfl,neqnfl,neqnrb,rb)
c
c***********************************************************************
c
cc VRB2FL computes the full solution GFL represented by a set of
c  reduced basis coefficients GRB.
c
c  VRB2FL is given:
c    NEQNRB, the number of reduced basis vectors and coefficients;
...
*  =======
*
cc DGER   performs the rank 1 operation
*
*     A := alpha*x*y' + A,
*
