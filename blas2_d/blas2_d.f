...
c*********************************************************************72
c
cc DGEMV computes y := alpha * A * x + beta * y for general matrix A.
c
c  Discussion:
c
...
c    0 <= N.
c
c    Input, double precision ALPHA, the scalar multiplier for A * x.
c
c    Input, double precision A(LDA,N).  The M x N subarray contains
c    the matrix A.
c
c    Input, integer  LDA, the the first dimension of A as declared
c    in the calling routine.  max ( 1, M ) <= LDA.
c
c    Input, double precision X(*), an array containing the vector to be 
...
c    X.  INCX must not be zero.
c
c    Input, double precision BETA, the scalar multiplier for Y.
c
c    Input/output, double precision Y(*), an array containing the vector to
...
c
      if ( ( m .eq. 0 ) .or. 
     &     ( n .eq. 0 ) .or. 
     &     ( ( alpha .eq. 0.0D+00 ) .and. ( beta .eq. 1.0D+00 ) ) ) then
        return
      end if
c
c  Set LENX and LENY, the lengths of the vectors x and y, and set
