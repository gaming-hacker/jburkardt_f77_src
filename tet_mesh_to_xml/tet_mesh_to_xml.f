...
c*********************************************************************72
c
cc MAIN is the main program for TET_MESH_TO_XML.
c
c  Discussion:
c
c    TET_MESH_TO_XML converts a tet mesh, using 4 or 10 node
c    tetrahedral elements, to a DOLFIN XML mesh file.
c
c  Usage:
c
c    tet_mesh_to_xml prefix
c
c    where
c
c   * prefix_nodes.txt contains the node coordinates;
c   * prefix_elements.txt contains the element definitions;
c   * prefix.xml the DOLFIN XML mesh file.
c
c  Licensing:
c
...
c
      ...
      call timestamp ( )
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'TET_MESH_TO_XML'
      write ( *, '(a)' ) '  FORTRAN77 version'
      write ( *, '(a)' ) '  Convert a linear or quadratic tet mesh' //
     &  ' to DOLFIN XML mesh format.'

      write ( *, '(a)' ) '  Read "prefix"_nodes.txt, node coordinates.'
      write ( *, '(a)' ) '  Read "prefix"_elements.txt, 4 or 10 ' // 
     &  'node element definitions.'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) '  Create "prefix".xml, a corresponding ' // 
     &  'DOLFIN XML mesh file.'
c
...
c
      ...
      if ( dim_num .ne. 3 ) then
        write ( *, * ) ' '
        write ( *, '(a)' ) 'TET_MESH_TO_XML - Fatal error!'
...
c
     ...
     &  element_node )
c
c  Write out the XML version of the data.
c
      call xml_write ( xml_filename, dim_num, node_num, node_xyz,
...
c
      ...
      subroutine mesh_base_zero ( node_num, element_order, 
     &  element_num, element_node )
...
c
     ...
     &  d, month(m), y, h, ':', n, ':', s, '.', mm, ampm
      ...
      subroutine xml_write ( xml_filename, dim_num, node_num, node_xyz, 
     &  element_order, element_num, element_node )

c*********************************************************************72
c
cc XML_WRITE writes the triangulation data as a DOLFIN XML mesh file.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
