...
c*********************************************************************72
c
cc MAIN is the main program for PATTERSON_RULE.
c
c  Discussion:
c
c    This program looks up a standard Gauss-Patterson quadrature rule
c    and writes it to a file.
c
c  Usage:
c
c    patterson_rule order a b filename
c
c    where
c
c    * ORDER is the number of points in the rule, and must be
c      1, 3, 7, 15, 31, 63, 127, 255 or 511;
c    * A is the left endpoint;
c    * B is the right endpoint;
c    * FILENAME is the "root name" of the output files.
c
c  Licensing:
c
...
c*********************************************************************72
c
cc ORDER_CHECK checks the value of ORDER.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c  Parameters:
c
c    Input, integer ORDER, the requested order.
c
c    Output, logical ORDER_CHECK, is TRUE if the requested order is acceptable.
c
      implicit none
...
c  Discussion:
c
c    The integration interval is [ -1, 1 ].
c
c    The weight function is w(x) = 1.0.
c
...
c    rule.
c
c    Each subsequent rule is nested in a similar way.  Rules are available
c    of orders 1, 3, 7, 15, 31, 63, 127, 255 and 511.
c
c    The data for N = 511 was supplied by Dirk Laurie.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c  Parameters:
c
c    Input, real ( kind = 8 ) A, B, the endpoints of the new interval.
c
