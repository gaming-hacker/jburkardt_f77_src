...
c*********************************************************************72
c
cc GRAY_MEDIAN_NEWS uses a median NEWS filter on a gray scale image to remove noise.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c  Parameters:
c
c    Input, integer M, N, the number of rows and columns of pixels.
c
c    Input, integer GRAY(M,N), the noisy grayscale data.
c
c    Output, integer GRAY2(M,N), the grayscale data for the 
c    filtered image.
c
      implicit none
...
c  Get an odd number of data points, 
c
      do i = 2, m - 1
        j = 1
        p(1) = gray(i-1,j  )
