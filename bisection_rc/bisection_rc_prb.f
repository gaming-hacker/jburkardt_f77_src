...
c*********************************************************************72
c
cc MAIN is the main program for BISECTION_RC_PRB.
c
c  Location:
c
c    http://people.sc.fsu.edu/~jburkardt/f77_src/bisection_rc/bisection_rc_test.f
c
c  Discussion:
c
c    BISECTION_RC_PRB tests the BISECTION_RC library.
c
c  Licensing:
c
...
c*********************************************************************72
c
cc TEST01 tests BISECTION_RC, evaluating the function in a separate routine.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      double precision b
      double precision dx
      double precision dx_tol
      double precision f01 
      external f01
      double precision fx
      double precision fx_tol
      integer it
      integer it_max
      integer job
      double precision x
...
10    continue
        ...
        if ( abs ( fx ) .le. fx_tol ) then
          write ( *, '(a)' ) ' '
          write ( *, '(a)' ) '  Function is small.'
          go to 20
        end if

        if ( dx .le. dx_tol ) then
          write ( *, '(a)' ) ' '
...
c*********************************************************************72
c
cc F01 evaluates the function f(x) = cos ( x ) - x which is zero around 0.74
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c    Output, double precision F01, the function value.
c
      implicit none
...
c*********************************************************************72
c
cc TEST02 tests BISECTION_RC, evaluating the function within the routine.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
10    continue
        ...
        fx = cos ( 100.0D+00 * x ) 
     &    - 4.0D+00 * erf ( 30.0D+00 * x - 10.0D+00 )
...
c*********************************************************************72
c
cc TEST03 tests BISECTION_RC, to invert the cardioid CDF.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
      ...
      parameter ( alpha = 0.0D+00 )
      double precision b
      double precision beta
      parameter ( beta = 0.25D+00 )
     ...
     &  '  The cardioid probability density function has a'
      ...
      write ( *, '(a)' ) 
     &  '    CDF(X) = ' //
     &  '( pi + x - alpha + 2 beta * sin ( x - alpha ) ) / ( 2 * pi )'
      write ( *, '(a)' ) 
     ...
     &  '  CDF(X) is the probability that a random sample will have'
      ...
      write ( *, '(a)' ) '  determine the value X where ' //
     &  'the Cardioid CDF is exactly 0.75.'
...
10    continue
        ...
        cdf = ( r8_pi + x - alpha + 2.0D+00 * beta * sin ( x - alpha ) ) 
     &    / ( 2.0D+00 * r8_pi )
        fx = cdf - 0.75D+00
...
20    continue
      ...
      write ( *, '(a)' ) ' '
      cdf = ( r8_pi + a - alpha + 2.0D+00 * beta * sin ( a - alpha ) ) 
     &  / ( 2.0D+00 * r8_pi )
      fx = cdf - 0.75D+00
      write ( *, '(a,g14.6,a,g14.6)' ) '  A = ', a, ' F(A) = ', fx
      cdf = ( r8_pi + x - alpha + 2.0D+00 * beta * sin ( x - alpha ) ) 
     &  / ( 2.0D+00 * r8_pi )
      fx = cdf - 0.75D+00
      write ( *, '(a,g14.6,a,g14.6)' ) '  X = ', x, ' F(X) = ', fx
      cdf = ( r8_pi + b - alpha + 2.0D+00 * beta * sin ( b - alpha ) ) 
     &  / ( 2.0D+00 * r8_pi )
      fx = cdf - 0.75D+00
      write ( *, '(a,g14.6,a,g14.6)' ) '  B = ', b, ' F(B) = ', fx
      ...
      write ( *, '(a)' ) ''
      cdf = ( r8_pi + x - alpha + 2.0D+00 * beta 
     &  * sin ( x - alpha ) ) / ( 2.0D+00 * r8_pi )
...
c*********************************************************************72
c
cc TEST04 tests BISECTION_RC for the pipe freezing problem.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
     ...
     &  '  As a function of depth x and time t, the soil temperature'
      ...
      write ( *, '(a)' ) 
     &  '    ( T(x,t) - Tc ) / ( Ti - Tc ) = erf ( 0.5 * x ' //
     &  '/ sqrt ( alpha * t ) ).'
      write ( *, '(a)' ) '  where:'
      write ( *, '(a)' ) '    Ti =  20 degrees centigrade,'
      write ( *, '(a)' ) '    Tc = -15 degrees centigrade,'
      write ( *, '(a)' ) '    alpha = 0.000000138 meter^2 ' // 
     &  '/ second, thermal conductivity;'
      write ( *, '(a)' ) '    and erf() is the error function.'
      write ( *, '(a)' ) '  Water freezes at 0 degrees centigrade.'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) 
     &  '  What depth x in meters must a water pipe be buried so that'
      write ( *, '(a)' ) 
     &  '  it will not freeze even if this cold snap lasts for 60 days?'
c
...
c*********************************************************************72
c
cc TEST05 tests BISECTION_RC for Kepler's problem.
c
c  Discussion:
c
c    Kepler's equation has the form:
c
c      X = M + E * sin ( X )
c
c    X represents the eccentric anomaly of a planet, the angle between the
c    perihelion (the point on the orbit nearest to the sun) through the sun 
c    to the center of the ellipse, and the line from the center of the ellipse
c    to the planet.
c
c    There are two parameters, E and M:
c
...
c      angular velocity equal to the average angular velocity of the true
c      planet.  M is usually between 0 and 180 degrees, but can have any value.
c
...
c
      ...
      write ( *, '(a)' ) '    X = M + E * sin ( X )'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) '  X represents the eccentric anomaly ' //
     &  'of a planet, the angle between the'
      write ( *, '(a)' ) '  perihelion (the point on the orbit ' //
     &  'nearest to the sun) through the sun'
      write ( *, '(a)' ) '  to the center of the ellipse, and the ' //
     &  'line from the center of the ellipse'
      write ( *, '(a)' ) '  to the planet.'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) '  There are two parameters, E and M:'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) '  * E is the eccentricity of the orbit, ' //
     &  'which should be between 0 and 1.0;'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) '  * M is the angle from the perihelion ' //
     &  'made by a fictitious planet traveling'
      write ( *, '(a)' ) '    on a circular orbit centered at the ' //
     &  'sun, and traveling at a constant'
      write ( *, '(a)' ) '    angular velocity equal to the ' //
     &  'average angular velocity of the true'
      write ( *, '(a)' ) '    planet.  M is usually between 0 and ' //
     &  '180 degrees, but can have any value.'
      write ( *, '(a)' ) ''
      write ( *, '(a)' ) 
...
20    continue
      ...
      write ( *, '(a)' ) '  In Degrees:'
      write ( *, '(a)' ) ''
      fx = ( ad - md ) * r8_pi / 180.0D+00 - e * 
     &  sin ( ad * r8_pi / 180.0D+00 );
      write ( *, '(a,g14.6,a,g14.6)' ) '  A = ', ad, ', F(A) = ', fx
      fx = ( xd - md ) * r8_pi / 180.0D+00 - e * 
     &  sin ( xd * r8_pi / 180.0D+00 );
      write ( *, '(a,g14.6,a,g14.6)' ) '  X = ', xd, ', F(X) = ', fx
      fx = ( bd - md ) * r8_pi / 180.0D+00 - e 
     &  * sin ( bd * r8_pi / 180.0D+00);
