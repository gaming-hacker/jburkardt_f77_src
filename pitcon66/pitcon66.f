...
c
c    Input/output, integer IWORK(LIW), the integer work storage array.
c
c    Input, integer LIW, the size of IWORK.
c
c    Input, integer NVAR, the number of variables.
c
c    Input/output, double precision RWORK(LRW), the double precision work array.
c
...
c
      ...
      ltc = 29 + 2 * nvar + 1
      lwk = 29 + 3 * nvar + 1
      if ( iwork(1) .lt. (-10) ) then
        iwork(1) = -2
      end if
      if ( 5 .lt. iwork(1) ) then
        iwork(1)=0
      end if
c
c  2.  Preparations.
...
c  Parameters:
c
c    Input,  EPS, a tolerance used for shifting the X values. 
c    A value of the square root of the machine precision is 
c    usually appropriate.
c
c    Output, double precision FCOL(NEQN), the last column of the approximate
c    jacobian, which is allowed to be "full".  This comprises
c    matrix entries FPRIME(1,NVAR) through FPRIME(NEQN,NVAR).
c
c    Input/output, double precision FPAR(*), user parameter vector, not
c    touched by this routine, but passed on to user routines.
c
c    Output, double precision FPRIME(NBAND,NEQN), is the array into which the
...
c
c    Output, double precision FROW(NVAR), storage for the last (augmenting) row
...
c
c           FPAR   Input/output, double precision FPAR(*), array of user parameters.
c
c           IPAR   Input/output, integer IPAR(*), array of user parameters.
c
c           X      Input, double precision X(NVAR), the point at which function evaluation
c                  is required.
c
c           FVEC   Output, double precision FVEC(NVAR), the value of the function at point
...
c
c    Output, integer IERROR, error flag.  A nonzero value means that
c    there was an error in the user routine FX, or in BANJAC itself.
c    In either case, the jacobian has not been computed.
c
c    Input, integer IPAR(*), a user parameter vector passed to FX.
...
c
c    Input, integer IPC, the index of the current continuation parameter,
c    which is needed to determine the form of FROW.
c
c    Input, integer IWORK(LIW), work and statistics vector.  Only
c    required here so that we can count the number of function
c    evaluations.
c
c    Input, integer JAC, the user requested jacobian option.  For
...
c
c    Input, integer NBAND, the first dimension of the jacobian matrix
c    FPRIME, NBAND=ML+MU+1.
c
c    Input, integer NEQN, the number of equations, equal to NVAR-1.
c
c    Input, integer NVAR, the number of variables.
c
c    Input, double precision X(NVAR), the point at which the jacobian is desired.
c
c    Work arrays, double precision XTEMP(NVAR), WORK1(NVAR), WORK2(NVAR).
c
      implicit none
...
c  Parameters:
c
c    Output, double precision DETS, the sign of the determinant of the full 
c    matrix A.
c
c  FX     Input, EXTERNAL FX, the name of the routine which evaluates the
c         function.
...
c           NVAR   Input, integer NVAR, number of variables.
c
c           FPAR   Input/output, double precision FPAR(*), user parameters.
c
c           IPAR   Input/output, integer IPAR(*), user parameters.
c
c           X      Input, double precision X(NVAR), the point at which function
c                  evaluation is required.
c
c           FVEC   Output, double precision FVEC(NVAR), value of the function at
c                  X.  Only the first NVAR-1 entries of FVEC are to be set by
c                  the routine.  PITCON sets the final value itself.
c
c           IERROR Output, integer IERROR, error flag.  FX should set this to 0
...
c           integer nvar
c
c           double precision fpar(*)
c           integer ierror
c           integer ipar(*)
c           double precision rwork(*)
c           double precision x(nvar)
c
c           ml=ipar(1)
c           mu=ipar(2)
c           nband=2*ml+mu+1
...
c
c  FPAR   Input/output, double precision FPAR(*), a user defined parameter array.
c
...
c
c    Workspace, double precision RWORK(LRW).  RWORK contains workspace used
...
c
c    Input, double precision X(NVAR), the point at which the jacobian is to
c    be evaluated.
c
c    Input/output, double precision Y(NVAR).  Right hand side/solution vector.
...
c
      ...
      if(lrlst.gt.lrw)then
        ...
        if(iwrite.ge.1)then
          write(*,*)'BANSLV - Fatal error!'
          write(*,*)
     &      '  Insufficient double precision workspace in RWORK!'
...
c
      if(one+rwork(ldfl+nvar-1).eq. 0.0D+00 ) then
        ierror=3
      ...
      if(one+rwork(ldfl+nvar-1).lt. 0.0D+00 )dets=-dets
      if(job.eq.2)return
c
c  Solve the system.
c
...
c
      if(rwork(1) .le. 0.0D+00 ) rwork(1)=sqrt(rwork(8))
      if(rwork(2) .le. 0.0D+00 ) rwork(2)=sqrt(rwork(8))
...
c  Parameters:
c
c    Input, double precision EPS, a tolerance to be used for shifting the X 
...
c
c    Input, double precision FPAR(*),  available for the user
c    to communicate with the FX routine.
c
c    Output, double precision FPRIME(NVAR,NVAR), the array into which the
...
c           end
c
c    Output, integer IERROR, error return flag.  A nonzero value
c    indicates that an error occurred.
c
c    Input, integer IPAR(*), an integer vector, available for the user
c    to communicate with the FX routine.
c
c    Input, integer IPC, the index of the current continuation parameter,
...
c    of the jacobian.
c
c    Input/output, integer IWORK(LIW), an array containing pointers
...
c
c    Input, integer JAC, the jacobian option.
...
c    Input, integer NVAR, the number of variables.
c
c    Input, double precision X(NVAR), the point at which the jacobian is to be estimated.
c
c    Workspace, double precision WORK1(NVAR), WORK2(NVAR).
c
      implicit none
...
c        integer nvar
c
c        double precision a(nvar,nvar)
c        double precision fpar(*)
c        integer ierr
c        integer ipar(*)
c        double precision x(nvar)
...
c  Parameters:
c
c    Output, double precision DETS, the sign of the determinant of the matrix
c    in equation (1).
c
...
c
c    Input, rdouble precision FPAR(*), an array which allows the user to pass
...
c    Input, integer NVAR, the number of variables.
c
c    Input/output, double precision RWORK(*).  RWORK contains various scalars,
...
c    Input, integer LRW, the dimension of RWORK.
c
c    Input, double precision X(NVAR), the point at which the jacobian is to be
c    evaluated.
c
c    Input/output, double precision Y(NVAR).  On input, Y may contain the right
     ...
     &      'Insufficient integer workspace in IWORK!'
      ...
      if(lrlst.gt.lrw)then
        ...
        if(iwrite.ge.1)then
          write(*,*)'DENSLV - Fatal error!'
          write(*,*)
     &      'DENSLV - Insufficient workspace in RWORK!'
...
c
      ...
      if(det.gt.0.0D+00)then
        dets=one
      elseif(det.lt.0.0D+00)then
        dets=-one
      end if
...
c
c    Input, integer LDAB, the leading dimension of A.
c
...
c  Parameters:
c
c    Input, integer M, the number of rows and columns of the matrix A.
c
c    Input, double precision A(LDA,M), the factors L and U as computed by DGETRF.
c
c    Input, integer LDA, the leading dimension of the array A.  LDA >= max(1,M).
c
c    Input, integer IPIVOT(M), as computed by DGETRF.
c
...
c
      ...
      if(iwrite.ge.2)then
        write(*,*)
     &    'LIMIT  - For S= 0.0, Tan(X(S))(Lim)=',rwork(27)
        write(*,*)
...
c
        if(snl.le.0.0D+00)then
          skale=0.0D+00
        else
          skale=sn/snl
          if(skale.le.0.0D+00)skale=0.0D+00
...
c  Parameters:
c
c    Input/output, double precision A.  A is one endpoint of the current 
...
c
c    Input/output, double precision FA.  On the very first call only, the 
...
c
c    Input/output, double precision B.  B is one endpoint of the current 
...
c
c    Input/output, double precision FB.  On the very first call only, the 
...
c
c    Output, double precision U, the current approximation to the root.  After every
c    call to ROOT, U will contain the routine's best approximation to
c    the location of the root.
c
c    Input, double precision FU.  On the first call, the user should not set FU.
...
c
c    Input/output, integer KOUNT.  On the first call, set KOUNT to zero.
c    Thereafter, the program will update KOUNT, which counts the number
c    of calls made to ROOT.
c
c    Output, integer IFLAG, reports the status of the search.
...
c
c    Input, double precision EPMACH, the relative machine precision.
c
      implicit none
...
c
      if(kount.le.0)then
        ...
        if((fa.gt.0.0D+00.and.fb.gt.0.0D+00)
     &    .or.(fa.lt.0.0D+00.and.fb.lt.0.0D+00))then
...
c  Parameters:
c
c    Input/output, integer IWORK(LIW), the integer work array.
c
c    Input, integer LIW, the size of IWORK.
c
c    Input, integer LRW, the size of RWORK.
...
c
      ...
      if ( curv .eq. 0.0D+00 ) then
        curv = rwork(16)
      end if

      if ( rwork(22) .eq. 0.0D+00 ) then
        curvx = rwork(16) 
      else
        curvx = rwork(16)
...
c  of the parameter.
c
      if ( 0 .lt. iwork(18) ) then
        htan = min ( htan, ( rwork(20) - one ) * rwork(21) / two )
      end if

      if ( iwork(3) .ne. 1 )then
        temp = one + ( one - rwork(25) / rwork(24) ) 
     &    * ( htan / two ) / rwork(21)
        htan = htan * temp
      end if
c
