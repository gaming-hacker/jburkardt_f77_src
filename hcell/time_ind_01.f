...
!***********************************************************************
! 
!! TIME_IND solves the steady Navier-Stokes equations in an H-cell region.
!
!
!  Discussion:
!
...
!
      ...
      double precision re_no, delta_re
!
...
!
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'TIME_IND_01:'
      write ( *, '(a)' ) '  Solve the steady state Navier Stokes fluid flow'
...
!
        re_no = 1.0d0
        write(*,*) "get an initial guess at Re = 1"
     ...
     &     nsteps, nsim, maxnd, maxel, rdel, alpha, beta, iflag )
        if( iflag .ne. 0 ) then
            write(*,*) "Failed to convergence at Re=1"
            stop
        end if
!
!  Continue in Reynolds number
        delta_re = 100.0d0
        re_no=0.0d0
!
        do iter = 1, n_con
...
! 
 500          continue       
     ...
     &         nsteps, nsim, maxnd, maxel, rdel, alpha, beta, iflag )
               if ( iflag .ne. 0 ) then
                   re_no = re_no - delta_re
                   delta_re = delta_re / 2.0d0
                   write(*,*) "reducing stepsize of RE to ", delta_re
                   If ( delta_re .lt. 0.0001d0) then
...
!
        ...
        do i = 1, np
          ...
          if ( 0 < unk_u ) then
            diff = diff + ( g(unk_u) - f(unk_u) )**2/ dfloat(neqn)
          end if

          unk_v = indx(i,2)

          if ( 0 < unk_v ) then
            diff = diff + ( g(unk_v) - f(unk_v) )**2/ dfloat(neqn)
          end if
        ...
        if ( iter .ge. 4) then
            if ( diff .ge. error_save) then
                write(*,*) "error growing - Newton fails"
                return
            end if
        end if
