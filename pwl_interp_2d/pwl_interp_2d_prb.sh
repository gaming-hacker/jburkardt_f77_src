#!/bin/bash
#
gfortran -c pwl_interp_2d_prb.f
if [ $? -ne 0 ]; then
  echo "Errors compiling pwl_interp_2d_prb.f"
  exit
fi
#
gfortran pwl_interp_2d_prb.o -L$HOME/libf77 -lpwl_interp_2d -ltest_interp_2d -lr8lib
if [ $? -ne 0 ]; then
