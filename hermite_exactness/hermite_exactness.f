...
c*********************************************************************72
c
cc MAIN is the main program for HERMITE_EXACTNESS.
c
c  Discussion:
c
c    This program investigates a standard Gauss-Hermite quadrature rule
c    by using it to integrate monomials over (-oo,+oo), and comparing the
...
c    * the OPTION (unweighted/physicist weight/probabilist weight)
c
c    OPTION indicates the weight function and normalization:
c    0, Integral ( -oo < x < +oo ) x^n exp(-x*x)               dx.
c    1, Integral ( -oo < x < +oo ) x^n exp(-x*x)               dx.
c    2, Integral ( -oo < x < +oo ) x^n exp(-x*x/2)             dx.
c    3, Integral ( -oo < x < +oo ) x^n exp(-x*x)   / sqrt (pi) dx.
c    4, Integral ( -oo < x < +oo ) x^n exp(-x*x/2) / sqrt(2pi) dx.
c
c  Licensing:
c
...
c
      if ( 3 .le. arg_num ) then
        ...
        write ( *, '(a)' ) 'HERMITE_EXACTNESS:'
        write ( *, '(a)' ) 
     &    '  OPTION specifies the weight function rho(x):'
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) '  0, rho(x) = 1'
      ...
      if ( option .lt. 0 .or. 4 .lt. option ) then
        write ( *, '(a)' ) ' '
...
c
      ...
      if ( option .eq. 0 ) then
        ...
        write ( *, '(a)' ) 
     &    '  OPTION = 1, the physicist weighted rule for:'
        write ( *, '(a)' ) 
     &    '  Integral ( -oo < x < +oo ) f(x) * exp(-x*x) dx'
      else if ( option .eq. 2 ) then
        write ( *, '(a)' ) 
     &    '  OPTION = 2, the probabilist weighted rule for:'
        write ( *, '(a)' ) 
     &    '  Integral ( -oo < x < +oo ) f(x) * exp(-x*x/2) dx'
...
c
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '  A Gauss-Hermite rule exactly'
      write ( *, '(a,i8)' )
     &  '  integrates monomials through degree = ', 2 * order - 1
      write ( *, '(a)' ) ' '
      ...
      do degree = 0, degree_max
        ...
        call hermite_monomial_quadrature ( degree, order, option, w, x, 
     &    quad_error )
...
c  Discussion:
c
c    H(n,1) = Integral ( -oo < x < +oo ) x^n exp(-x^2) dx
c    H(n,1) is 0 for n odd.
c    H(n,1) = (n-1)!! * sqrt(pi) / 2^(n/2) for n even.
c
c    H(n,2) = Integral ( -oo < x < +oo ) x^n exp(-x^2/2) dx
c    H(n,2) is 0 for n odd.
c    H(n,2) = (n-1)!! * sqrt(2*pi) for n even.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c    0 <= N.
c
c    Input, integer OPTION, the integral has the form:
c    0, Integral ( -oo < x < +oo ) x^n exp(-x*x)               dx.
c    1, Integral ( -oo < x < +oo ) x^n exp(-x*x)               dx.
c    2, Integral ( -oo < x < +oo ) x^n exp(-x*x/2)             dx.
c    3, Integral ( -oo < x < +oo ) x^n exp(-x*x)   / sqrt (pi) dx.
...
c
     ...
     &    / 2.0D+00 ** ( n / 2 )
      ...
      subroutine hermite_monomial_quadrature ( expon, order, option, 
     &  w, x, quad_error )

c*********************************************************************72
c
cc HERMITE_MONOMIAL_QUADRATURE applies a quadrature rule to a monomial.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license.
...
c
c  Evaluate the unweighted monomial at the quadrature points.
c
      if ( option .eq. 0 ) then
        do i = 1, order
          value(i) = exp ( - x(i)**2 ) * x(i)**expon
        end do
      else if ( option .eq. 1 ) then
        do i = 1, order
          value(i) = x(i)**expon
