...
c*********************************************************************72
c
cc MAIN is the main program for TEMPLATES_PRB.
c
c  Discussion:
c
c    TEMPLATES_PRB tests the TEMPLATES library.
c
...
*     ..
*     .. Array Declarations ..
      REAL               X( MAXDIM,NSUBS ), B( MAXDIM ), X0( MAXDIM ),
     $                   WORK( MAXLEN )
      LOGICAL            LTEST( NSUBS )
...
*     .. External Routines ..
     ...
     $                   PSOLVEQ, PSOLVETRANSQ, BACKSOLVE, SNSYCHK,
     $                   SSPDCHK
*
...
*
      CALL SSPDCHK( X, LDX, B, X0, WORK, LDW, PFORM, MATVEC,
...
*
      CALL SNSYCHK( X, LDX, B, X0, WORK, LDW, PFORM, MATVEC,
     $              MATVECTRANS, PSOLVE, PSOLVETRANS, PSOLVEQ,
     $              PSOLVETRANSQ, BACKSOLVE, TOL, SCALEDTOL, LTEST, 
...
*
      END
*     ===============================================================
      REAL             FUNCTION SCALEDRESID( ANORM, N, X, RK, TOL )
*
*     Returns |B-A*X| / ( |A||X|*N*TOL ), using the infinity norm.
*
      INTEGER            N, ISAMAX
      REAL               ANORM, TOL, XNORM, RESNORM,
     $                   X( * ), RK( * )
*
      XNORM   = ABS( X( ISAMAX( N, X, 1 ) ) )
      RESNORM = ABS( RK( ISAMAX( N, RK, 1 ) ) )
*
...
*
      END
*     ===========================================================
      REAL             FUNCTION MATNORM( N, A, LDA )
*
*     Compute infinity norm of matrix A.
*
      INTEGER            N, LDA, I, J
      REAL               ROWSUM, ZERO, TEMP, A( LDA,* )
...
*
      ...
      REAL                SNRM2, SCALEDRESID, SRESID( 9 )
      CHARACTER*9         METHOD( 9 )
      LOGICAL             LSAME, LSAMEN
*
      EXTERNAL            SCOPY, SGEMV, SNRM2
*
*     .. Executable Statements ..
*
      METHOD( 1 ) = 'CG       '
...
*
      ...
      DO 10 I = FIRSTALG, NUMALG
         IF ( RESID( I ).NE.ZERO ) THEN
            CALL SCOPY( N, B, 1, RK, 1 )
            CALL SGEMV('N', N, N, -ONE, A, LDA, X( 1,I ), 1, ONE,
     $                  RK, 1 )
