...
c
      ...
      if ( x .lt. 0.0D+00 .or. 1.0D+00 .lt. x ) then
        write ( *, '(a)' ) ''
        write ( *, '(a)' ) 'BETAIN - Warning!'
        write ( *, '(a)' ) '  X is not between 0 and 1.'
        ifault = 2
        stop 1
      end if
c
...
c  Discussion:
c
c    The accuracy exponent SAE was loosened from -37 to -30, because
c    the code would not otherwise accept the results of an iteration
c    with p = 0.3, q = 3.0, alpha = 0.2.
c
c  Modified:
c
c    25 September 2014
...
c  Local Parameters:
c
c    Local, double precision SAE, accuracy is requested to about 10^SAE.
c
      implicit none
...
c
      ...
      if ( alpha .lt. 0.0D+00 .or. 1.0D+00 .lt. alpha ) then
        write ( *, '(a)' ) ''
        write ( *, '(a)' ) 'XINBTA - Fatal error!'
        write ( *, '(a)' ) '  ALPHA not between 0.0 and 1.0'
        stop 1
      end if
c
...
c
      ...
      if ( value .lt. 0.0001D+00 ) then
        value = 0.0001D+00
      end if

      if ( 0.9999D+00 .lt. value ) then
        value = 0.9999D+00
      end if

      iex = max ( 
     &  - 5.0D+00 / pp ** 2 - 1.0D+00 / a ** 0.2D+00 - 13.0D+00, 
     &  sae )
...
10    continue
      ...
      if ( ifault .ne. 0 ) then
        ...
        write ( *, '(a,i4)' ) '  BETAIN returns IFAULT = ', ifault
        stop 1
      end if
...
30    continue
      ...
      go to 20
c
c  Check whether current estimate is acceptable.
c  The change "VALUE = TX" was suggested by Ivan Ukhov.
c
40    continue

      if ( prev .le. acu .or. y * y <= acu ) then
        value = tx
        go to 50
      end if
