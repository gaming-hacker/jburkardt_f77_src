...
c*********************************************************************72
c
cc MAIN is the main program for PYRAMID_EXACTNESS.
c
c  Discussion:
c
c    This program investigates the polynomial exactness of a quadrature
c    rule for the pyramid.
c
...
c
      ...
      integer t
      double precision v(max_order)
      double precision w(max_order)
      double precision x(dim_num,max_order)
...
c  Read the X file.
c
      call r8mat_header_read ( quad_x_filename, dim_num2, order )

      write ( *, '(a)' ) ' '
      write ( *, '(a,i8)' ) '  Spatial dimension = ', dim_num2
...
c
c      product ( 1 .le. I .le. 3 ) X(I)^EXPON(I)
c
